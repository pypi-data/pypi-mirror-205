    ...
    def predict(self,texts : Union[List[str],Tuple[str],str], 
       ...
       '''
        ...
        for idx,_input in enumerate(input_list):
            ...
            group_id.extend([idx] * len(_input))
        ...
        _results = list(map(lambda input,output : self.postprocess(input, output, **kwargs),inputs, outputs))
        
        results = []
        last_id = 0
        entities,labels = [],[]
        for result,_id in zip(_results,group_id):
            if _id <= last_id:
                entities.extend(result[0])
                labels.extend(result[1])
            else:
                results.append([entities,labels])
                entities,labels = [],[]
                last_id = _id
