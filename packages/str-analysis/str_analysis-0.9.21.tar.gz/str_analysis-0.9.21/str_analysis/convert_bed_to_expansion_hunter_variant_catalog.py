...
def main():
    ...
    p.add_argument("bed_path", help="Input .bed file")
    args = p.parse_args()
    ...
    process_bed_file(args.bed_path, args.output_file, verbose=args.verbose)


def process_bed_file(bed_path, output_file_path, verbose=False):
    print(f"Parsing {bed_path}")
    json_records = []
    existing_locus_ids = set()
    counter = collections.defaultdict(int)
    with (gzip.open if bed_path.endswith("gz") else open)(bed_path, "rt") as f:
