...
# run program
#noclif: boolean. Skip clif translation and start with p9 files?
#test: boolean.  run a complex test suite (compare several heuristics) on the input'
#norun: skips consistency checking if True. (used for updating the p9 files)
#tidy: delete intermediary files upon completion

#mode: 
#   "check" to run a simple consistency check
#   "lemma" if there are lemma to be proven.
#   "vam" to run Vampire

def prove(filepath, mode="check", noclif=False, test=False, norun=False, tidy=True):
