...
class ReasonerProcess(multiprocessing.Process):
    ...
    def enforce_limits (self, pid):
        limit = int(macleod_eureka.Filemgt.read_config('system', 'memory_limit')) # read custom memory limit from configuration file
        if limit is None:
...
def raceProcesses (reasoners):
    ...
    from macleod_eureka.clif_things.ClifModuleSet import ClifModuleSet
    ...
    while num_running>0:
        ...
        while not results.empty():
            ...
            if r.terminatedWithError():
                ...
                if r.output==macleod_eureka.Ontology.INCONSISTENT:
                    if r.isProver():
                        ...
                        status = 'INCONSISTENT'
                elif r.output==macleod_eureka.Ontology.CONSISTENT:
                    if r.isProver():
