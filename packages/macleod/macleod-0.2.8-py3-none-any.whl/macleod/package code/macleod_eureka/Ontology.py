...
import macleod_eureka.ReasonerSet
import macleod_eureka.dl.owl
import macleod_eureka.logical.axiom

import macleod_eureka.Filemgt
import macleod_eureka.Process
import macleod_eureka.dl.filters
import macleod_eureka.dl.translation
...
class Ontology(object):
    ...
    def __init__(self, name, basepath=None, resolve=False, preserve_conditionals = True):
        ...
        macleod_eureka.dl.translation.reset_seen()
        ...
        if basepath is None:
            self.basepath = macleod_eureka.Filemgt.get_ontology_basepath()
        else:
    ...
    def resolve_imports(self):
        ...
        import macleod_eureka.parsing.parser as Parser
    ...
    def add_axiom(self, logical):
        ...
        self.axioms.append(macleod_eureka.logical.axiom.Axiom(logical))
    ...
    def add_conjecture(self, logical):
        ...
        self.conjecture.append(macleod_eureka.logical.axiom.Axiom(logical))
    ...
    def analyze_ontology(self):
        ...
        from macleod_eureka.logical.symbol import Function
    ...
    def add_predicate_satisfiability_axiom(self, symbol, positive_polarity=True):
        ...
        from macleod_eureka.logical.connective import Conjunction
        from macleod_eureka.logical.quantifier import Existential
        from macleod_eureka.logical.negation import Negation
        from macleod_eureka.logical.axiom import Axiom
    ...
    def get_output_filename(self, output_type, out=False):
        ...
        if self.resolve:
            ending += macleod_eureka.Filemgt.read_config('output', 'all_ending')

        ending += macleod_eureka.Filemgt.read_config(output_type, 'ending')

        if out:
            ending += macleod_eureka.Filemgt.read_config("output", 'ending')
            folder_name = macleod_eureka.Filemgt.read_config("output", 'folder')
        else:
            folder_name = macleod_eureka.Filemgt.read_config(output_type, 'folder')

        output_filename = macleod_eureka.Filemgt.get_full_path(module_name,
                                                folder=folder_name,
                                                ending=ending)
    ...
    def check_consistency (self, options_files = None):
        ...
        reasoners = macleod_eureka.ReasonerSet.ReasonerSet()
        ...
        reasoners = macleod_eureka.Process.raceProcesses(reasoners)
    ...
    def to_owl(self, profile):
        ...
        if self.owl is None:
            ...
            for axiom, path in axioms:
                ...
                pcnf_sentences = macleod_eureka.dl.translation.translate_owl(pcnf)
                ...
                for pruned in pcnf_sentences:
                    ...
                    tmp_axiom = macleod_eureka.logical.axiom.Axiom(pruned)
                    pattern_set = macleod_eureka.dl.filters.filter_axiom(tmp_axiom)
                ...
                for extraction in extractions:
                    if macleod_eureka.dl.translation.produce_construct(extraction, onto):
                        self.owl_axioms += 1


                for extra in pcnf.extra_sentences:
                    for extra_pruned in macleod_eureka.dl.translation.translate_owl(extra):
                        tmp_axiom = macleod_eureka.logical.axiom.Axiom(extra_pruned)
                        pattern_set = macleod_eureka.dl.filters.filter_axiom(tmp_axiom)
