...
class FunLibCSVReader:
    ...
    def read_csv_funclib_create_hashmap(self, gs: GatewaySIGMA = None):
        df = pd.read_csv(os.path.join(Path(__file__).parent.parent, 'CFUN_mapping.csv'),
                         names=['SIGMA_key', 'File_name'])
        funlib_map = gs.gateway.jvm.java.util.HashMap()
        for index, row in df.iterrows():
            print(row['SIGMA_key'], row['File_name'])
            funlib_map.put(row['SIGMA_key'], row['File_name'])
        return funlib_map
