...
class LocIdSerialTransportThread(Thread):
    ...
    def __init__(self, device_port: str = None):
        ...
        self._user_prox_callback = None
        self._user_min_prox_changed_callback = None
    ...
    def on_user_proximity_changed(self, callback):
        """" Sets a user proximity handler """
        self._user_prox_callback = callback

    # def on_user_min_prox_changed(self, callback):
    #     """" Sets a callback for minimal user proximity changes """
    #     self._user_min_prox_changed_callback = callback

    def _on_user_connected(self, user_id):
        logging.info("on_user_connected: %s", user_id)
        if self._user_connected_callback:
            self._user_connected_callback(self, user_id)
        pass

    def _on_user_disconnected(self, user_id):
    ...
    def run(self):
        while not self._stop_rx_thread_evt.is_set():
            ...
            # jsonrpc notifications
                ...
                elif method == 'locid.user.prox_changed' and self._user_prox_callback is not None:
                    user_id = params['user_id']
                    new_prox = params['prox']
                    old_prox = params['old']
                    self._user_prox_callback(user_id, new_prox, old_prox)
