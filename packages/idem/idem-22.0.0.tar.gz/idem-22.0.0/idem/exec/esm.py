...
async def remove(
    ...
    Command: idem exec esm.remove provider=[provider] tag=[tag-in-esm] <profile=[]>
    For example: idem exec esm.remove tag="aws.iam.role_|-Create AWS IAM role ReadOnly01_|-ReadOnly01_|-"  provider=aws profile=cloud-1
    """
    # Get the acct data from the kwargs and fallback to the acct_data in the current runtime
    if acct_data is None:
        acct_data = await hub.idem.acct.data(
    ...
    try:
        ...
        if tag in state:
            ret = state.pop(tag)
            comment = f"Clearing resource with tag '{tag}' from ESM on provider '{provider}' using profile '{profile}'"
...
async def show(
    ...
    Command: idem exec esm.show provider=[provider] <profile=[]>
    For example: idem exec esm.show provider=aws profile=cloud-1
    """
    # Get the ESM ctx
    ctx = await hub.idem.acct.ctx(
        f"esm.{provider}",
