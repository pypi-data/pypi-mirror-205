...
class WebsocketClient:
    ...
    def send(self, channel: str, message: Union(str, object), headers: Union(dict, None) = None):
    ...
    def wait_message(self, channel: str, timeout: int = 30, headers: Union(object,None) = None) -> Union(object, str, int, float):
        connection = f"wss://{self._server}?channel={channel}"
        headers = headers if headers else {}
        with connect(connection, additional_headers=headers) as websocket:
            try:
