...
def test__error__empty_list() -> None:
	with pytest.raises(ValueError, match='I cannot infer the type from an empty list'):
		Config('l', [])
...
def test__help_text_error_and_set() -> None:
	...
	cf = ConfigFile(appname='test')
	cf.set_ui_callback(ui_callback)
	with pytest.raises(AttributeError, match="No help given for Path"):
		cf.save()

	Set.set_help_for_type(Path, 'The path to a file')
	fn = cf.save()
	with open(fn, 'rt') as f:
		assert f.read() == """\
# Data types
