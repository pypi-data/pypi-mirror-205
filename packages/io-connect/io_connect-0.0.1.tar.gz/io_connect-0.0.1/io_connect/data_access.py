...
class DataAccess:
    ...
    def get_caliberation(self, device_id, metadata, df):
        ...
        for (value1, value2) in zip(sensor_id_list, sensor_name_list):
            ...
            if 'm' in df_meta.columns and 'c' in df_meta.columns:
                ...
                df[str(value2)] = df[str(value2)].replace('BAD 255', '-99999').replace('-', '99999') \
                    .replace('BAD undefined', '-99999').replace('BAD 0', '-99999')
                ...
                if 'max' in df_meta.columns:
                    max_value = int(str(df_meta.iloc[0]['max']).replace('-', '99999')
                                    .replace('1000000000000000000000000000', '99999').replace('100000000000', '99999'))
    ...
    def fetch_data(self, device_id, start_time, end_time=datetime.now(), sensors=None, echo=True):
        metadata = {}
        if sensors is None:
            metadata = DataAccess.get_device_metadata(self, device_id)
    ...
    def data_query(self, device_id, start_time, end_time=datetime.now(), cal=True, bands=None):
        """
        ...
        try:
            ...
            if flag:
                ...
                if last_date.date() != end_time.date():
                    df1 = DataAccess.fetch_data(self, device_id, start_time=str(start_date), end_time=end_time,sensors=None, echo=True)
                    df = pd.concat([df, df1])
                else:
                    df1 = DataAccess.fetch_data(self, device_id, start_time=str(last_date), end_time=end_time,sensors=None, echo=True)
                    df = pd.concat([df, df1])
                df.reset_index(drop=True, inplace=True)
            else:
                ...
                if device_id in device_list:
                    df = DataAccess.fetch_data(self, device_id, start_time, end_time, sensors=None,echo=True)
                else:
