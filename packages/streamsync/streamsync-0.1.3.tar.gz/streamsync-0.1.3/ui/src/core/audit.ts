...
export function auditComponents(components: ComponentMap) {
	Object.entries(components).forEach(([componentId, component]) => {
		if (componentId !== "root" && !components[component.parentId]) {
			console.warn(`Component ${component.id} (${component.type}). Orphan component.`);
		}
		auditPositions(component, components);
...
function auditPositions(component: Component, components: ComponentMap) {
	...
	const positionfulChildren = Object.values(components).filter(c => c.parentId === component.id && c.position !== -2);
	let positionSum = 0;
	positionfulChildren.forEach(c => {
		positionSum += c.position;
	});

	const arithmeticProgression = ((positionfulChildren.length-1) * (positionfulChildren.length )) / 2;
	if (arithmeticProgression !== positionSum) {
		console.error(`Component ${component.id} (${component.type}). Invalid children positions.`, positionfulChildren);
	}
}