...
class OpenAPIPresenter(ISchemaPresenter):
    ...
    def _get_prop_schema(self, type_: Type) -> dict:
        for parser in self.components_properties_parsers:
            if parser.check(type_):
                return parser.parse(type_)
        raise ValueError(f'Cannot found OpenAPI component property parser for type = {type_.name}')
    ...
    def _components(self, models: dict[str, Type]) -> dict:
        ...
        for model_path, model in models.items():
            ...
            for prop_name, prop in model.model_fields.items():
                prop_schema = self._get_prop_schema(prop.type)
    ...
    def _type_ref(self, annotation: Annotation | Type) -> dict:
        ...
        return self._get_prop_schema(t)
