...
class Data:
    ...
    def __init__(self, dataset, shuffle=False, batch_size=64,  whole_batch=False, n_packs=1, n_batches=-1) -> None:
        """
        Класс даных. Его итератор выдаёт данные батчами.
        ...
        ------------   
            * dataset (tensor or list of tensors):
                данные
            * shuffle (bool = False):
                перемешивать или нет данные        
            * batch_size (int = 64):
                размер минибатча        
            * whole_batch (bool = False):
                брать только батчи размера batch_size
            * n_packs (int = 1)
                разбить весть датасет на n_packs паков
            * n_batches (int = -1)
                выдать только n_batches первых батчей
