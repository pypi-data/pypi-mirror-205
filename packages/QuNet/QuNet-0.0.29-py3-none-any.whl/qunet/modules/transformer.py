...
class SelfAttention(nn.Module):
    ...
    def create(self):
        cfg = self.cfg
        assert cfg.E > 0 and cfg.H > 0, f"must be E and H in cfg:{cfg.get_str()}"
...
class FFT(nn.Module):
    """
    see: "FNet: Mixing Tokens with Fourier Transforms" 1
    """
    def __init__(self,  *args, **kvargs) -> None:
        """
        FFT Block from FNet
        ...
        Args:         
            ...
            * `res=1` - kind of skip-connections (0: none, 1: usial, 2: train one for all E, 3: train for each E)
            * `after = 1` - after fft2: (1) take Re, (2) take Im, (3) Re**2+Im**2 
        """
        super().__init__()
...
class  TransformerBlock(nn.Module):
    def __init__(self,  *args, **kvargs) -> None:
        ...
        Args:         
            ...
            * `res=1`  - kind of skip-connections (0: none, 1: usial, 2: train one for all E, 3: train for each E)
            * `causal = False` - kind of cauual attention mask (True: GPT, False: Bert)
            * 'T_max = 2048'- maximum number of tokens (needed for causal==True)
        """
        super().__init__()
