...
class Trainer:
    ...
    def plot(self, *views):
        ...
        """
        if len(views) > 0 and  isinstance(views[0], Config):
            plot_history(self.hist, views[0])     
        else:
    ...
    def save(self, fname, folder="", model = None, optim=None, info=""):
    ...
    def load(self, fname, ClassModel, load_optimizer=False):
        try:
            ...
            if load_optimizer and self.optim and 'optimizer' in state:
            ...
            print(f"May be wrong model in path '{fname}'")
            return None
