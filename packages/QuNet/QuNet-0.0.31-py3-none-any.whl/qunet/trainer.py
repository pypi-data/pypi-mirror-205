...
class Trainer:
    ...
    def fit(self,  epochs=None,  samples=None,
            ...
            period_call:int = 0, period_reload: int = 0,
            monitor = [],
            patience = None,
            callback = None):        
        """
        ...
        ------------                
            ...
            period_call (int=0):
                callback custom function call period
            callback (function):
                custom function called with period_info
            ...
            period_reload (int=0):
                period in epochs after which the reload of data_trn and data_val method is called
        ...
        for epoch in range(1, epochs+1):
            if period_reload > 0 and epoch % period_reload == 0:
                if hasattr(self.data.trn, "reload"):
                    self.data.trn.reload(True,  epoch, self.hist, self.best)
                if hasattr(self.data.val, "reload"):
                    self.data.val.reload(False,  epoch, self.hist, self.best)
