...
class DenseLayer_TFLITE(DenseLayer):
    ...
    def forward_pass(self, **kwargs):
        ...
        if(self.idx_multiply is None):
            cuda.synchronize()
            matmul_tflite_occorenza[self.griddim, self.blockdim](self.results, A_global_mem,self.weights,self.k_mul,self.k_filtri,self.k_bias,self.M,128,128,self.activation,self.output_mul,self.output_offset,self.occorenza)
        else:
    ...
    def load_weights(self, **kwargs):
        ...
        if self.enable_gpu:
            ...
            #todo accorciare
            self.weights = cuda.to_device(np.array(self.weights,dtype=np.int8))
