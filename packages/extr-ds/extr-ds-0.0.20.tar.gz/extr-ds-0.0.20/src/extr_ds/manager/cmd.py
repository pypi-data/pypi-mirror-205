...
def annotate() -> None:
    def get_redacted_templates() -> Set[str]:
        ...
        redacted_templates_file_path = os.path.join(WORKSPACE, '4', 'dev-ents-redacted.txt')
...
def save() -> None:
    dataset = set(load_data(os.path.join(WORKSPACE, '3', 'ents.txt')))
    append_data(os.path.join(WORKSPACE, '4', 'ents.txt'), dataset)

    redacted_dataset = set(load_data(os.path.join(WORKSPACE, '3', 'ents-redacted.txt')))
