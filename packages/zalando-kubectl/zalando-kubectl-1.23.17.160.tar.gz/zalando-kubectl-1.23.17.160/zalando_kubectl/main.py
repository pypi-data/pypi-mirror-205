...
@click.pass_obj
def configure(env: Environment, cluster_registry, okta_auth):
    """Set the Cluster Registry and/or Okta OIDC Issuer URL."""
    if not any([cluster_registry, okta_auth]):
        print("Missing arguments. Provide Cluster Registry and/or Okta OIDC Issuer URL.")
...
@click.option("--okta/--no-okta", default=False, help="Login with Okta")
@click.pass_obj
def do_login(env: Environment, cluster, okta):
    ...
    token = auth_token()
    if okta:
        token = kube_config.get_auth_token(env, cluster)
...
def latest_socat_image():
    ...
    return "{}/{}/{}:{}".format(docker_registry, team, artifact, image["tags"][-1])
...
@click.pass_obj
def list_cluster_requests(env: Environment, output):
