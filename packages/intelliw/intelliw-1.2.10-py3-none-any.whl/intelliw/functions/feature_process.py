...
def feature_process(data, config: list, stage: str) -> Iterable:
    ...
    """
    alg_yaml_file, model_yaml_file = gl.get(
        "alg_yaml_file"), gl.get("model_yaml_file")
    ...
    if processor_mode == "infer":
        ...
        if target_data is not None:
            ...
            # TODO 暂时兼容后面删除
            gl.set("final_target_col", target_cols[-1]["col"])
...
class NumericFeature(Feature):
    ...
    def do_process(self, data_frame):
        ...
        if self.mode == 'train':
            ...
            if not (self.check_repair_config['miss_repair'] in ['1', '2', '3', '4', '5', '6'] or type(self.check_repair_config['miss_repair']) == dict):
                raise FeatureProcessException(
