        ...
        def _init_meta(self):
            ...
            if gl.get("model_type") != 9:
                ...
                if isinstance(target_metadata, list) and len(target_metadata) > dataset_idx:
                    target_info = target_metadata[dataset_idx]
                    target_col = target_info["target_col"]
                    if target_col > len(self.meta):
                        raise DatasetException(
                            f"target column index:{target_col}, There are {len(self.meta)} columns in the source csv, index out of range")
                    self.meta[target_col] = {
                        'code': target_info["target_name"]}
