...
class CocoProcess:
    ...
    def __gen_config(cls, images, annotations, prefix):
        ...
        for i in images:
            ...
            i_config[i['file_name']] = {'image': i, 'annotation': a_map[i['id']]}
        return i_config
...
class DataSourceIwImgData(AbstractDataSource):
    ...
    def reader(self, page_size=10000, offset=0, limit=0, transform_function=None, dataset_type='train_set'):
        r = self.__Reader(self.input_address, self.ds_id, self.ds_type, self.total(), dataset_type, page_size,
                          transform_function)
        r.dataset_no = self.dataset_no
        return r

    def download_images(self, images, transform_function=None, dataset_type='train_set'):
        r = self.reader(transform_function=transform_function, dataset_type=dataset_type)
        r.dataset_no = self.dataset_no
        r.set_download(images)
        return r()

    class __Reader:
        def __init__(self, input_address, ds_id, ds_type, total, process_type, page_size=100,
        ...
        def get_data_bar(self):
            """数据拉取进度条"""
            try:
                proportion = round((self.total_read / self.total_data) * 100, 2)
                logger.info(
        ...
        def __next__(self):
            ...
            if self.total_read >= self.total_data:
                logger.info(f'图片数据集[{self.dataset_no}]元数据下载完成，准备下载图片，共读取原始数据 {self.total_read} 条')
                raise StopIteration
            ...
            try:
                with ThreadPoolExecutor(max_workers=self.worker) as executor:
                    ...
                    for f in futures:
                        page = f.result()
                        data_result.extend(page['data']['content'])
                ...
                logger.exception(f"图片数据集[{self.dataset_no}]数据源读取失败, input_address: [{self.ds_id}]")
        ...
        def set_download(self, page):
            logger = _get_framework_logger()
            prefix = str(self.dataset_no) if DataSourceIwImgData.dataset_count > 1 else ""
            ...
            with ThreadPoolExecutor(max_workers=self.worker) as executor:
                futures = [executor.submit(self.__download, p, prefix) for p in page]
                for f in as_completed(futures):
                    try:
                        f.result()
                        success_count += 1
                        if success_count % 10 == 0:
                            logger.info(f"图片数据集[{self.dataset_no}]下载中: {success_count}/{self.total_data}")
                    except Exception as e:
                        err_count += 1
                        err_msg = e
                        logger.error(
