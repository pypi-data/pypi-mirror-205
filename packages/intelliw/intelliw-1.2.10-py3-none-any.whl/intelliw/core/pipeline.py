...
from intelliw.datasets.datasets import DataSets, AlgorithmsType
...
class Pipeline:
    def __init__(self, reporter: Tuple[str, Recorder] = None, perodic_interval=-1):
    ...
    def _get_train_kwargs(self, reader, column_meta):
        ...
        kwargs = {
            ...
            "column_meta": gl.pop("column_meta") if gl.get("column_meta") is not None else [i["code"] for i in
                                                                                            column_meta],
        ...
        # 时间序列参数
        if gl.get("model_type") == AlgorithmsType.TIME_SERIES:
    ...
    def transform_call_chains(self, funcs, src):
        try:
            for _, item in enumerate(funcs):
                ...
                if func == functions.select_columns and self._not_select_columns():
                    continue
                src = self.transform_call_wrapper(src, func, params)
            return src
        except Exception as e:
            msg = message.CommonResponse(
                500, "transforms", "特征工程函数 {} 出错".format(name))
    ...
    def _not_select_columns(self):
        ...
        if fp and fp.get(Const.pre_train) and self.model_type != AlgorithmsType.TIME_SERIES:
            return True
        return False
    ...
    def prepare_transforms(self, cfgs):
        def init_dict(f):
            return dict() if f is None else f

        def init_list(f):
            return list() if f is None else f
                    ...
                    for item in functions_cfg:
                        ...
                        if key == functions.Const.feature_process:
                            ...
                            if func is not None:
                                ...
                                _t = {}
                                if can_split:
                                    split_transforms = init_dict(split_transforms)
                                    _t = split_transforms
                                else:
                                    alldate_transforms = init_dict(alldate_transforms)
    ...
    def _raise_exception(self, msg: message.CommonResponse, e: Exception = None, is_framework_error=True):
        ...
        if hasattr(e, "btype"):
            msg.other['businessType'] = e.btype()
