...
#==========================================
    def __init__(self,input_dim, output_dim, emb_dim, hid_dim, n_layers, dropout, device, tokenizer):
        ...
        # Encoder
        self.encoder = nn.LSTM(input_size=emb_dim, hidden_size=(hid_dim//2),
                               num_layers=1, dropout=dropout, bidirectional=True, batch_first=True)
        self.hx_dense = nn.Linear(hid_dim, hid_dim)

        # Decoder
        self.decoder_cell = nn.LSTMCell(input_size=hid_dim, hidden_size=hid_dim, device=device)
    ...
    def forward(self, input_ids, labels, mode:str = "train"):
        embed = self.embedding(input_ids)

        # Encoder
        enc_out, hn = self.encoder(embed)
        ...
        for t in range(seq_len):
            ...
            if "eval" == mode:
                ...
                mutable_pron_ids = self._get_score_handling_list(batch_size,
                                                                 self.vocab_size, mutable_pron_list)
        ...
        """
         # LSTM
        # dec_out, _ = self.decoder(enc_out, hn)

        # FFN
        # logits = self.fc_layer(dec_out)
        """
        return output
    ...
    def _decode_batch_sentences(self, input_ids):
        ...
        for input_item in input_ids.tolist():
            ...
            for eojeol in decoded_sent:
                if eojeol in ["[CLS]", "[SEP]", "[UNK]", "[PAD]", "[MASK]", "<N>", "</N>"]:
    ...
    def _get_mutable_pron_list(self, time_step: int, batch_size: int, origin_sent: List[List[str]]):
        ...
        for b_idx in range(batch_size):
            ...
            if origin_char in ["[CLS]", "[SEP]", "[PAD]", "[UNK]", "[MASK]", "<N>", "</N>"]:
