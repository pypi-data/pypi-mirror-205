...
class PostgresTargetConnector(C8Connector):
    ...
    def config(self) -> list[ConfigProperty]:
        ...
        return [
            ...
            ConfigProperty('connect_timeout', 'Connection Timeout (secs)', ConfigAttributeType.INT, False, False,
...
async def process_messages(event_loop, message_queue, state, flushed_state, flush_lock, schemas, key_properties,
    ...
    while True:
        LOGGER.info("reached process is :")
...
def flush_streams(
    ...
    LOGGER.info("flush stream return :")
...
def setup_flush_task(config, filter_streams=None):
...
async def read_stdin() -> AsyncIterable[str]:
    ...
    async def read_stdin_impl():
        input_messages = io.TextIOWrapper(sys.stdin.buffer, encoding='utf-8')
        while True:
            line = await loop.run_in_executor(None, input_messages.readline)
            if not line:
                break
            yield line.strip()

    return read_stdin_impl()
...
async def main_impl():
    ...
    try:
        ...
        LOGGER.info("Queue:")
        LOGGER.info("flush task setup reaching1")
        message_queue = setup_flush_task(config)
        LOGGER.info("flush task setup reaching")
        lines = await read_stdin()
        async for line in lines:
            LOGGER.info("persist lines reaching")
