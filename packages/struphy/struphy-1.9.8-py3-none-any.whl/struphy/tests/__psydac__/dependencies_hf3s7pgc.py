...
def assemble_matrix_hf3s7pgc(global_test_basis_v_1, global_trial_basis_u_1, global_span_v_1, global_x1, test_v_p1, trial_u_p1, n_element_1, k1, pad1, g_mat_u_v_hf3s7pgc, b01, e01):

    from numpy import array, zeros, zeros_like, floor
    local_x1 = zeros_like(global_x1[0,:])
    
    l_mat_u_v_hf3s7pgc = zeros((3, 5), dtype='float64')
        ...
        for i_basis_1 in range(0, 3, 1):
            for j_basis_1 in range(0, 3, 1):
                ...
                for i_quad_1 in range(0, 3, 1):
                    ...
                    u = u_1
                    u_x1 = u_1_x1
                    contribution_v_u_hf3s7pgc += u*v
                
                l_mat_u_v_hf3s7pgc[i_basis_1,2 - i_basis_1 + j_basis_1] = contribution_v_u_hf3s7pgc
            
        
        g_mat_u_v_hf3s7pgc[pad1 + span_v_1 - test_v_p1:1 + pad1 + span_v_1,:] += l_mat_u_v_hf3s7pgc[:,:]
