...
@pytest.mark.parametrize(
    ("response", "has_tier_warning", "has_location_warning"),
    [
        pytest.param(
        ...
        pytest.param(
            ...
            False,
            True,
            id="EnterpriseCentralZone",
        ),
        pytest.param(
            ("200 OK", "zonal_filestore.json"),
            True,
            True,
            id="NonEnterpriseCentralZone",
        ),
    ],
)
def test_verify_filestore_warn(
    ...
    _, err = capsys.readouterr()
    assert ("ENTERPRISE" in err) == has_tier_warning
