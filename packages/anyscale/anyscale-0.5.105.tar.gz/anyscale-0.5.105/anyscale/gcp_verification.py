...
def verify_filestore(
    ...
    try:
        ...
        return False
    # file_store 1.6.0: https://cloud.google.com/python/docs/reference/file/latest/google.cloud.filestore_v1.types.Instance.Tier
    if file_store.tier != Instance.Tier.ENTERPRISE:
        logger.internal.warning(
            f"Filestore is running with tier {file_store.tier.name}, {Instance.Tier.ENTERPRISE.name} is the suggested tier."
        )
        if strict:
            return False
