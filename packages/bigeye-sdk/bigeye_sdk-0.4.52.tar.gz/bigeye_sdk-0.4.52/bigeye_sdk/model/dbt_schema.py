...
class SimpleDbtColumn(YamlModel):
    type = "column"
    name: str = ""
        ...
        if values["tests"]:
            for test in values["tests"]:
                ...
                if isinstance(test, dict) and list(test.keys())[0] == "accepted_values":
                    params.append(
                        SimpleMetricParameter(key="list", string_value=",".join(test["accepted_values"]["values"]))
                    )
                    ...
                    threshold = SimpleConstantThreshold(type="CONSTANT", upper_bound=1, lower_bound=1)
                    ...
                    params.extend(
                        [
                            column_to_check,
                            SimpleMetricParameter(key="lookup_column", string_value=test["relationships"]["field"]),
                            SimpleMetricParameter(key="lookup_table", string_value=test["relationships"]["to"][5:-2])
                        ]
                    )
                    name = "dbt_test_relationships"
                    threshold = SimpleConstantThreshold(type="CONSTANT", upper_bound=1, lower_bound=1)
                    mt = SimpleTemplateMetric(template_id=0, aggregation_type=SimpleAggregationType.PERCENT)
                elif test in SimpleDbtTestToMetricType.__members__:

                    t = SimpleDbtTestToMetricType[test.lower()]
                    name = f"dbt_test_{test.lower()}"
                    threshold = SimpleConstantThreshold(type="CONSTANT", upper_bound=0, lower_bound=0)
                    mt = SimplePredefinedMetric(predefined_metric=t.value)
                ...
                smd = SimpleMetricDefinition(metric_type=mt,
                                             metric_name=name,
