...
def change_column_name(data):
    new_name_map = {
        ...
        # tick
        'tradingphasecode': 'trading_phase_code','maxpx': 'max', 'minpx': 'min',
    ...
    if isinstance(result, list):
        ...
        for handle_data in result_list:
            ...
            for cost in cost_list:
                ...
                df_cost_choose['NumberOfSharesPercent'].values[-1] = df_cost_choose['NumberOfSharesPercent'].values[-1] - (
        ...
        column_list = ["last", "prev_close", "avg_cost", "max_cost", "min_cost", "winner_rate",
                       "diversity", "pre_winner_rate", "restricted_avg_cost", "restricted_max_cost", "restricted_min_cost",
        ...
        all_result = df_all_data[['htsc_code', 'time', 'exchange', 'last', 'prev_close', 'total_share', 'a_total_share', 'a_listed_share', 'listed_share', 'restricted_share', 'cost_5pct', 'cost_15pct', 'cost_50pct', 'cost_85pct', 'cost_95pct', 'avg_cost', 'max_cost', 'min_cost', 'winner_rate', 'diversity', 'pre_winner_rate', 'restricted_avg_cost', 'restricted_max_cost', 'restricted_min_cost', 'large_shareholders_avg_cost', 'large_shareholders_total_share', 'large_shareholders_total_share_pct']]
...
def get_tick_handle(htsc_code, security_type, trading_day):
    ...
    exchange_suffix_map = {'SH': '101', 'SZ': '105'}
    ...
    cal_params = {"EXCHANGE_CODE": exchange_code, "START_DATE": start_time, "END_DATE": end_time, "IS_TRADING_DAY": 1}
    ...
    for trading_date in trading_day_list:
        ...
        if isinstance(result, list):
            ...
            if security_type == 'stock':  # 股票
                ...
                c_result = df_result[['htsc_code', 'time', 'trading_phase_code', 'exchange', 'security_type',
                      'max', 'min', 'prev_close', 'num_trades', 'volume', 'value', 'last', 'open', 'high', 'low',
                      'iopv', 'close']]
            ...
            divisor_list = ['max', 'min', 'prev_close', 'last', 'open', 'high', 'low', 'close', 'iopv', 'settle', 'pre_settle']
            ...
            if security_type != 'index':
                ...
                df_result["BuyPriceQueue"] = df_result["BuyPriceQueue"].apply(lambda x: list(map(lambda n: float(n)/10000, x)))
                df_result["SellPriceQueue"] = df_result["SellPriceQueue"].apply(lambda x: list(map(lambda n: float(n)/10000, x)))

                df_buy_price_queue = df_result['BuyPriceQueue'].apply(pd.Series, index=['bid' + str(i) for i in range(1,count)])
                df_buy_order_qty_queue = df_result['BuyOrderQtyQueue'].apply(pd.Series, index=['bid_size' + str(i) for i in range(1,count)])
                df_sell_price_queue = df_result['SellPriceQueue'].apply(pd.Series, index=['ask' + str(i) for i in range(1,count)])
                df_sell_order_qty_queue = df_result['SellOrderQtyQueue'].apply(pd.Series, index=['ask_size' + str(i) for i in range(1,count)])

                c_result = pd.concat([c_result, df_buy_price_queue, df_buy_order_qty_queue, df_sell_price_queue, df_sell_order_qty_queue], axis=1)
