...
#     attribute. To ensure a reproducible bootstrap, you must set the
#     `random_state` attribute after the bootstrap has been created. See
#     the example below. Note that `random_state` is a reserved keyword and any
#     variable passed using this keyword must be an instance of `RandomState`.
#     Attributes:
#       data: tuple, Two-element tuple with the pos_data in the first position
#         and kw_data in the second (pos_data, kw_data). Derived from
#         `IIDBootstrap`.
...
#             performance on a suite of tasks with multiple runs/episodes. The
#             inputs are assumed to be of the shape `(num_runs, num_tasks, ..)`.
#           random_state: If specified, ensures reproducibility in uncertainty
#             estimates.
#           task_bootstrap: Whether to perform bootstrapping (a) over runs or (b)
#             over both runs and tasks. Defaults to False which corresponds to
#             (a). (a) captures the statistical uncertainty in the aggregate
#             performance if the experiment is repeated using a different set
#             of runs (e.g., changing seeds) on the same set of tasks. (b)
#             captures the sensitivity of the aggregate performance to a given
#             task and provides the performance estimate if we had used a larger
#             unknown population of tasks.
...
#         """Update independent sampling indices for the
#         next bootstrap iteration."""

#         pos_indices = [
#             self._get_indices(
#                 self._num_arg_items[i],
#                 self._args_shapes[i],
