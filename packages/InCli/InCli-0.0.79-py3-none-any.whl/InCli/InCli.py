...
def option_vte(args):
    help = f"""
        ...
        {utils.CYELLOW}-vte a6R7a000001k7t7EAA {utils.CEND} - Displays the JSON for the provided Id.
            {utils.CYELLOW}-file{utils.CEND} Saves the JSON to a file. """ 
    ...
    if check_modifiers('-vte',args,["-file","-limit"],help) == False: return
...
def option_ipa(args):
    help = f"""
        ...
        {utils.CYELLOW}-ipa a2J7a000002UakuEAC {utils.CEND} - Displays the attachment for the provided Id.
            {utils.CYELLOW}-file{utils.CEND} Saves the attachment to a file. """ 
    
    if '-hu' in args: 
        return help
    if '-h' in args: return

    if check_modifiers('-ipa',args,["-file","-limit","-nf","-from","-to","-around","-minutes","-contains","-orderNumber"],help) == False: return
    ...
    ip_attachments.print_attachments(limit=limit,only_not_finished=nf,date1=mfrom,date2=mto,contains=contains)
...
def option_ipe(args):
    ...
    if check_modifiers('-ipe',args,["-file","-limit","-last"],help) == False: return
    ...
    return VlocityErrorLog.print_error_list(limit)
...
def option_cc(args):
    ...
    if check_modifiers('-cc',args,["-code","-list","-basket","-account"],help) == False: return
...
def option_logs(args):
    ...
    LOG Parsing
        Common modifiers:
            {utils.CYELLOW}-debuglevel M{utils.CEND} values S,M,L. To control how big the log files are. The more information is logged the longer time it takes. 
                    S displays basic information and it is fast. M adds the stack trace, is the default. L can help detecting if there is an exception in the manage package. 
            {utils.CYELLOW}-allLimits{utils.CEND} displays all limits (SQOL rows...)
            {utils.CYELLOW}-limitinfo {utils.CEND}--> prints out the log line when the limits (CPU, SOQL) are parsed.    
            {utils.CYELLOW}-file {utils.CEND}--> creates 2 files for the parsed logs, txt and html format. 
            {utils.CYELLOW}-level {utils.CEND}--> outputs the parsed line up to the specified level. 
        ...
        {utils.CYELLOW}-logs -folder path {utils.CEND}--> parses the files saved locally in order of creation(download) {utils.CGREEN}InCli -logs -folder /a/b/.../ {utils.CEND} 
 ...
 """
    ...
    if check_modifiers('-logs',args,["-last","-tail","-store","-deletelogs","-limit","-limitinfo","-auto","-folder","-level","-loguser","-where","-inputfile","-file","-error","-noScreen","-allLimits","-debuglevel"],help) == False: return

    logId = getParam(args,'-logs')    
    last = getParam(args,'-last')  
    limit = getParam(args,'-limit')    
    limitInfo = True if '-limitinfo' in args else False   
    allLimits = True if '-allLimits' in args else False   
    debuglevel = getParam(args,'-debuglevel')    
    if debuglevel == None: debuglevel = 'M'
    else: 
        if debuglevel not in ['S','L','M']: 
            print('debuglevel must be set to S, M or L. Deafulting to M')
            debuglevel = 'M'
    ...
    noScreen  = True if '-noScreen' in args else False
