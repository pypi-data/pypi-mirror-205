...
def initial_matching(fnn_model, ref, tgt, k_ptrs):
    ...
    for ref_i in range(ref.shape[0]):
        ...
        corr[:, ref_i] = np.reshape(fnn_model.predict([ref_x_flat_batch, tgt_x_flat_batch], batch_size=32),
                                    tgt.shape[0])
...
def transform_cells(img3d, vectors3d):
    ...
    for label in range(1, img3d.max() + 1):
        ...
        v1 = vectors3d[label - 1, 0]
        v2 = vectors3d[label - 1, 1]
        v3 = vectors3d[label - 1, 2]

        if v1 >= 0:
            idx_1_start = 0
            idx_1_end = shape[0] - v1
        else:
            idx_1_start = -v1
            idx_1_end = shape[0]
        if v2 >= 0:
            idx_2_start = 0
            idx_2_end = shape[1] - v2
        else:
            idx_2_start = -v2
            idx_2_end = shape[1]
        if v3 >= 0:
            idx_3_start = 0
            idx_3_end = shape[2] - v3
        else:
            idx_3_start = -v3
            idx_3_end = shape[2]
        ...
        output[idx_label[0] + idx_1_start + v1, idx_label[1] + idx_2_start + v2, idx_label[2] + idx_3_start + v3] = \
        image_temp[idx_label]
        mask[idx_label[0] + idx_1_start + v1, idx_label[1] + idx_2_start + v2,
             idx_label[2] + idx_3_start + v3] = mask[idx_label[0] + idx_1_start + v1,
                                                     idx_label[1] + idx_2_start + v2, idx_label[
                                                         2] + idx_3_start + v3] + 1
