...
from eia.utils.facets import get_facets_gasoline_price
...
class CrudeOilPrice(object):
    ...
    def get_weekly_retail_price(self):

        facets: List[str] = get_facets_gasoline_price()
        for state,series in facets.items():
            current_endpoint: str = f"https://api.eia.gov/v2/petroleum/pri/gnd/data/?api_key={API_KEY}&frequency=weekly&data[0]=value&facets[series][]={series}&sort[0][column]=period&sort[0][direction]=desc&offset=0&length=5000"
