...
class CrudeOilProduction(object):
    ...
    def get_crude_oil_production(self):

        facets: Dict = get_facets(facet_name="petroleum/crd/crpdn")
        self.all_productions = facets
        for state,series in facets.items():
            endpoint: str = f"https://api.eia.gov/v2/petroleum/crd/crpdn/data/?api_key={API_KEY}&frequency={self.frequency}&data[0]=value&facets[series][]={series}&sort[0][column]=period&sort[0][direction]=desc&offset=0&length=5000"
