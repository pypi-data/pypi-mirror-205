...
from vecdb.collections import dataset
from vecdb.utils import document
from vecdb.utils import keyphrase


class Field:
    def __init__(self, dataset: dataset.Dataset, field: str):
    ...
    def bulk_update_keyphrases(self, updates: List[Union[keyphrase.Keyphrase, dict]]):
...
class ClusterField(Field):
    ...
    def insert_centroids(self, centroid_documents: Union[List[Dict[str, Any]], document.DocumentList]):
        if isinstance(centroid_documents, document.DocumentList):
...
class KeyphraseField(Field):
    ...
    def update_keyphrase(
        self,
        keyphrase_id: str,
        keyphhprase_: Union[keyphrase.Keyphrase, str],
        frequency: int = 0,
        ancestors: list = None,
        parents: list = None,
        metadata: dict = None,
        ...
        if isinstance(update, keyphrase.Keyphrase):
