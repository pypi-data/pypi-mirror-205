...
class IconRpcHelper:
    ...
    def sign_tx(self, wallet=None, payload=None):
        ...
        _balance = self.get_balance(address=self.wallet.get('address'))
        pawn.console.log(f"<{self.wallet.get('address')}>'s Balance = {_balance} {self.network_info.symbol}")

        if not _balance or _balance == 0:
            return self.exit_on_failure(f"Insufficient Balance. 0 {self.network_info.symbol}")
