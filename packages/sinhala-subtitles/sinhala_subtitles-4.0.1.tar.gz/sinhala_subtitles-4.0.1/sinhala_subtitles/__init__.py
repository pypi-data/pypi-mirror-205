...
def bulk_cineru(bulk_url, path=None):
    response = get(bulk_url)
    soup = BeautifulSoup(response.text, 'html.parser')
    if path:
        path = path + "/" + soup.title.get_text()
    else:
...
def bulk_baiscope(bulk_url, path=None):
    response = get(bulk_url)
    soup = BeautifulSoup(response.text, 'html.parser')
    if path:
        path = path + "/" + soup.title.get_text()
...
class SinhalaSubtitles:
    ...
    def bulk(bulk_url: str, download_path=None) -> bool:
        """
        ...
        if 'cineru' in bulk_url:
            return bulk_cineru(bulk_url, download_path)
        elif 'baiscopelk' in bulk_url:
            return bulk_baiscope(bulk_url, download_path)
        else:
            print('Wrong url.')
            return False
