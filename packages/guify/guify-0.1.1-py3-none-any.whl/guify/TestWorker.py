...
class TestWorker(Thread):
    ...
    def _run_test(self, name, cls):
        ...
        try:
            ...
            cls.prompt_user = self.set_prompt
            if cls.initial_prompt:
                self.set_prompt(cls.initial_prompt)

            # Stop running if halt flag is True
            if self.halt.is_set():
                return False
            result = cls._run(**params)
            ...
            log.error(f"Exception {exc} in {name}")
    ...
    def start(self, tests: list = None, parameters: dict = None) -> None:
        ...
        required_params = set()
        required_params.union(set(self.additional_params))
