...
class TFTrainer:
    ...
    def setup_comet(self):
        ...
        if experiment is not None:
            experiment._set_model_graph(self.model, framework="adapter_transformers")
            experiment._log_parameters(self.args, prefix="args/", framework="adapter_transformers")
            experiment._log_parameters(self.model.config, prefix="config/", framework="adapter_transformers")
        ...
        if is_comet_available():
            ...
            if experiment is not None:
                experiment._log_metrics(
                    logs, step=self.global_step, epoch=self.epoch_logging, framework="adapter_transformers"
                )
