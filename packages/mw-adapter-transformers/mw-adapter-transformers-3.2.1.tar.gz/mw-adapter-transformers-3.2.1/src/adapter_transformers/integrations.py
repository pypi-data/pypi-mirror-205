...
def run_hp_search_ray(trainer, n_trials: int, direction: str, **kwargs) -> BestRun:
    ...
    def _objective(trial, local_trainer, checkpoint_dir=None):
        try:
            from adapter_transformers.utils.notebook import NotebookProgressCallback
...
class CometCallback(TrainerCallback):
    ...
    def setup(self, args, state, model):
        ...
        if state.is_world_process_zero:
            ...
            if comet_mode == "ONLINE":
                ...
                experiment.log_other("Created from", "adapter_transformers")
            ...
            if experiment is not None:
                experiment._set_model_graph(model, framework="adapter_transformers")
                experiment._log_parameters(args, prefix="args/", framework="adapter_transformers")
                if hasattr(model, "config"):
                    experiment._log_parameters(model.config, prefix="config/", framework="adapter_transformers")
    ...
    def on_log(self, args, state, control, model=None, logs=None, **kwargs):
        ...
        if state.is_world_process_zero:
            ...
            if experiment is not None:
                experiment._log_metrics(logs, step=state.global_step, epoch=state.epoch, framework="adapter_transformers")
