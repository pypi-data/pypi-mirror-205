...
from adapter_transformers.utils.doc import add_code_sample_docstrings
...
class FlavaIntermediate(nn.Module):
    ...
    # Copied from adapter_transformers.models.vit.modeling_vit.ViTIntermediate.forward
...
class FlavaOutput(nn.Module):
    ...
    # Copied from adapter_transformers.models.vit.modeling_vit.ViTOutput.forward
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor) -> torch.Tensor:
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
...
class FlavaModel(FlavaPreTrainedModel):
    ...
    def get_text_features(
        ...
        ```python
        >>> from adapter_transformers import AutoProcessor, FlavaModel
...
class FlavaImageCodebook(FlavaPreTrainedModel):
    ...
    def get_codebook_indices(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        >>> from PIL import Image
        >>> import requests
        >>> from adapter_transformers import AutoImageProcessor, FlavaImageCodebook
...
class FlavaForPreTraining(FlavaPreTrainedModel):
    ...
    def forward(
        ...
        >>> from PIL import Image
        >>> import requests
        >>> from adapter_transformers import FlavaForPreTraining, AutoProcessor
