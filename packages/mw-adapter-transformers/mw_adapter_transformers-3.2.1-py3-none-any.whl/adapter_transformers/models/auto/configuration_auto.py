...
class _LazyConfigMapping(OrderedDict):
    ...
    def __getitem__(self, key):
        ...
        if module_name not in self._modules:
            self._modules[module_name] = importlib.import_module(f".{module_name}", "adapter_transformers.models")
        if hasattr(self._modules[module_name], value):
            return getattr(self._modules[module_name], value)

        # Some of the mappings have entries model_type -> config of another model type. In that case we try to grab the
        # object at the top level.
        transformers_module = importlib.import_module("adapter_transformers")
...
class _LazyLoadAllMappings(OrderedDict):
    ...
    def _initialize(self):
        ...
        for model_type, map_name in self._mapping.items():
            ...
            module = importlib.import_module(f".{module_name}", "adapter_transformers.models")
...
class AutoConfig:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        ```python
        >>> from adapter_transformers import AutoConfig
