...
def get_task(model: str, use_auth_token: Optional[str] = None) -> str:
    ...
    if getattr(info, "library_name", "adapter_transformers") != "adapter_transformers":
...
def clean_custom_task(task_info):
    ...
    task_info["pt"] = tuple(getattr(adapter_transformers, c) for c in pt_class_names)
    tf_class_names = task_info.get("tf", ())
    if isinstance(tf_class_names, str):
        tf_class_names = [tf_class_names]
    task_info["tf"] = tuple(getattr(adapter_transformers, c) for c in tf_class_names)
...
def pipeline(
    ...
    **kwargs,
    ...
    ```python
    >>> from adapter_transformers import pipeline, AutoModelForTokenClassification, AutoTokenizer
