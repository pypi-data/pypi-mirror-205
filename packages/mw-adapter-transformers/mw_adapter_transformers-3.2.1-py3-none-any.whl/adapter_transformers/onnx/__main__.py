...
def export_with_optimum(args):
    if is_optimum_available():
        ...
        if parsed_optimum_version < version.parse(MIN_OPTIMUM_VERSION):
            raise RuntimeError(
                f"adapter_transformers.onnx requires optimum >= {MIN_OPTIMUM_VERSION} but {optimum_version} is installed. You "
                "can upgrade optimum by running: pip install -U optimum[exporters]"
            )
    else:
        raise RuntimeError(
            "adapter_transformers.onnx requires optimum to run, you can install the library by running: pip install "
            "optimum[exporters]"
        )
    cmd_line = [
        sys.executable,
    ...
    logger.info(
        ...
        "adapter_transformers.onnx is deprecated, and will be removed in v5. You can find more information here: "
...
if __name__ == "__main__":
    logger = logging.get_logger("adapter_transformers.onnx")  # pylint: disable=invalid-name
