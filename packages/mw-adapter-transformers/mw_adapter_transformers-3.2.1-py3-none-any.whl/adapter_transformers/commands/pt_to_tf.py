...
class PTtoTFCommand(BaseTransformersCLICommand):
    ...
    def __init__(
        ...
        *args
        ...
        self._logger = logging.get_logger("adapter_transformers-cli/pt_to_tf")
    ...
    def run(self):
        ...
        if architectures is None:  # No architecture defined -- use auto classes
            pt_class = getattr(import_module("adapter_transformers"), "AutoModel")
            tf_class = getattr(import_module("adapter_transformers"), "TFAutoModel")
            ...
            pt_class = getattr(import_module("adapter_transformers"), architectures[0])
            try:
                tf_class = getattr(import_module("adapter_transformers"), "TF" + architectures[0])
