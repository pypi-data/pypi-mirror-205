...
def sprint_60m(data, sfreq: float = 400., start=1.0, acc_frame=True,
               inplace=False):
    """
    Calculate 10m sprint test outcomes measures.
    ...
    n60 = int(len(data["dist_x"]))
    for val10 in range(0, n60):
        if data["dist_x"][val10] > 10:
            ten_value = val10
            break
    for val40 in range(0, n60):
        if data["dist_x"][val40] > 40:
            forty_value = val40
            break
    for val60 in range(0, n60):
        if data["dist_x"][val60] > 60:
            end_value = val60
            break
    ...
    outcomes_sprint = dict()
    outcomes_sprint['time_10m'] = ten_value / sfreq
    outcomes_sprint['time_40m'] = forty_value / sfreq
    outcomes_sprint['time_60m'] = end_value / sfreq
    outcomes_sprint['vel_10m_peak'] = np.max(data["vel"][:ten_value])
    outcomes_sprint['vel_40m_peak'] = np.max(data["vel"][ten_value:forty_value])
    outcomes_sprint['vel_60m_peak'] = np.max(data["vel"][forty_value:end_value])
    outcomes_sprint['pos_vel_peak'] = data["dist_x"][data["vel"].idxmax()]
    outcomes_sprint['vel_mean'] = np.mean(data["vel"])
    outcomes_sprint['vel_peak'] = np.max(data["vel"])
    outcomes_sprint['acc_10m_peak'] = np.max(data["acc"][:ten_value])
    outcomes_sprint['acc_40m_peak'] = np.max(data["acc"][ten_value:forty_value])
    outcomes_sprint['acc_60m_peak'] = np.max(data["acc"][forty_value:end_value])
    outcomes_sprint['acc_peak'] = np.max(data["acc"])
    outcomes_sprint['pos_acc_peak'] = data["dist_x"][data["acc"].idxmax()]
    outcomes_sprint['n_pushes'] = n_pushes
    outcomes_sprint['dist_push1'] = data["dist_x"][push_ind[0]]
