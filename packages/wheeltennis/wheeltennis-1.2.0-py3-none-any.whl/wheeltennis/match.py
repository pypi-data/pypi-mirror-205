...
def speed_zones(sessiondata, percentage=False, top_speed=4):
...
def turns(sessiondata):  # Rotations are based on rotational velocity now, so quick rotations, not actual rotations
    sfreq = 1 / sessiondata['frame']['time'].diff().mean()
