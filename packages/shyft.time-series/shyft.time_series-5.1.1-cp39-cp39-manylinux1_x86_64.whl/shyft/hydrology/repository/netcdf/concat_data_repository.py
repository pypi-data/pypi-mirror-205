...
from ....time_series import (Calendar, utctime_now, UtcPeriod, TimeAxis, UtcTimeVector, POINT_INSTANT_VALUE,
                             GeoGridSpec, GeoPointVector, GeoPoint, time)
from ... import shyftdata_dir
...
import warnings
from ..interfaces import ForecastSelectionCriteria
...
class ConcatDataRepository(interfaces.GeoGridTsRepository):
    """
    ...
    def __init__(self, epsg: str, filename: str, nb_lead_intervals_to_drop: int = 0,
        ...
        with Dataset(self._filename) as dataset:
            ...
            self.epsg_code = dataset.variables['crs'].epsg_code
            self.grid_coords = {'x': np.array(dataset.variables["x"]), 'y': np.array(dataset.variables["y"])}
            if 'z' in dataset.variables:
                self.grid_coords['z'] = np.array(dataset.variables['z'])

    @property
    def shyft_cs(self) -> str:
        shyft_cs = f"+init=EPSG:{self._epsg}"
        if not utm_proj(shyft_cs):
            raise ConcatDataRepositoryError(f"Coordinate system EPSG:{self._epsg} is not UTM-type")
        return shyft_cs
    ...
    @property
    def grid_spec(self) -> GeoGridSpec:
        source_epsg = self.epsg_code.split(':')
        epsg_str = source_epsg[-1] if source_epsg else self.shyft_cs.split(':')[-1]
        gps = [GeoPoint(float(x), float(y), float(z)) for (x, y, z) in zip(
            self.grid_coords['x'], self.grid_coords['y'], self.grid_coords.get('z', 0*self.grid_coords['x']))]
        return GeoGridSpec(int(epsg_str), GeoPointVector(gps))

    @property
    def data_period(self) -> UtcPeriod:
        return UtcPeriod(int(self.time[0]), int(self.time[0] + self.lead_times_in_sec[-1]))

    @property
    def start_times(self) -> List[time]:
        return [time(int(t)) for t in self.time]
    ...
    def _get_data_from_dataset(self, dataset: Dataset, input_source_types: List[str],
        ...
        extracted_data = self._transform_raw(raw_data, time, np.array(lead_times_in_sec[lead_time_slice]), concat)
