...
enum Error {
  ...
  MAX_REALIZATION,
  INVALID_NAME,
  DUMPLICATED_ACTION_NAME,
  INVALID_SECONDARY_TYPE,
  CUSTOM,
  __END__
};
...
template <class... TA> std::string Emsg(Error e, const TA &...args) {
  ...
  case Error::INVALID_NAME:
    return fmt::format("invalid name '{}', {}", args..., "a valid name is less than 12 characters and can only contain characters in a-z, 1-5 and '.'");
  case Error::DUMPLICATED_ACTION_NAME:
    return fmt::format("dumplicated name '{}'", args...);
  case Error::INVALID_SECONDARY_TYPE:
    return fmt::format("invalid secondary type '{}'", args...);
  case Error::CUSTOM:
