...
NLOHMANN_DEFINE_ORDERED_TYPE_NON_INTRUSIVE(ABIVariant,
  name,
  types);
...
struct ABIGenerator {
  ...
  ABI abi;
  bool hasContractClass = false;
  vector<FunctionStmt*> actionStmts;

  ABIGenerator();
  string parseABIType(string id);
  void addTable(std::shared_ptr<SimplifyContext> ctx, ClassStmt *stmt, std::vector<Param>& args);
  bool addAction(std::shared_ptr<SimplifyContext> ctx, FunctionStmt *stmt);
  void addStruct(ClassStmt *stmt, std::vector<Param>& args);
  std::vector<string> getRealizationTypes(std::vector<ExprPtr>& typeParams);
...
bool extract_attr_name(string attr, vector<ExprPtr>& decorators, string& name);
bool extract_attr_name(string attr, vector<ExprPtr>& decorators, int& index, string& name);
bool is_main_contract(vector<ExprPtr>& decorators);
bool is_contract_class(vector<ExprPtr>& decorators);
bool is_notify_action(vector<ExprPtr>& decorators);

bool hasContractClass();
void setContractClass(bool hasContractClass);
