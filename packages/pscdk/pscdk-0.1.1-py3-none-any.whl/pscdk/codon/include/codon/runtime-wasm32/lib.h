...
extern "C" {
  ...
  void prints_l( const char* cstr, uint32_t len);
  void prints_l2( const char* cstr, uint32_t len);
...
SEQ_FUNC void *seq_calloc(seq_int_t m, seq_int_t n);
SEQ_FUNC void *seq_calloc_atomic(seq_int_t m, seq_int_t n);
...
typedef uint64_t _Unwind_Reason_Code;

struct _Unwind_Exception {
  uint64_t exception_class;
  void (*exception_cleanup)(_Unwind_Reason_Code reason, _Unwind_Exception *expToDelete);
};
...
SEQ_FUNC seq_str_t seq_str_uint(seq_int_t n, seq_int_t len, char *format, bool *error);
SEQ_FUNC seq_str_t seq_str_float(double f, seq_int_t len, char *format, bool *error);
SEQ_FUNC seq_str_t seq_str_ptr(void *p, seq_int_t len, char *format, bool *error);
...
void printBacktraceFrameString(uintptr_t pc, const std::string &func = "",
