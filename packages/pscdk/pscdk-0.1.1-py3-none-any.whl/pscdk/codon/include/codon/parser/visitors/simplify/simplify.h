...
public: // Convenience transformators
  ...
  void prepareTransform(StmtPtr &stmt);
...
private: // Node simplification rules
  ...
  StmtPtr codegenMagic(const std::string &, const ExprPtr &, const std::vector<Param> &,
                       bool);
  StmtPtr codegenTableMethod(const std::string &op, const ExprPtr &typExpr,
                                        const std::vector<Param> &allArgs);
  StmtPtr generateNewMITableMethod(const std::string &tableName, const ExprPtr &typExpr,
                                        const std::vector<Param> &allArgs);
  std::shared_ptr<FunctionStmt> generateApply(std::shared_ptr<std::vector<StmtPtr>> preamble, std::shared_ptr<SimplifyContext> ctx, ClassStmt *cls, std::vector<FunctionStmt*> actionStmts);
