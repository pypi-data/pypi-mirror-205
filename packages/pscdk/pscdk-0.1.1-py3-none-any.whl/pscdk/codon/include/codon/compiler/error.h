...
template <class... TA> std::string Emsg(Error e, const TA &...args) {
  ...
  case Error::INVALID_NAME:
    return fmt::format("invalid name '{}', {}", args..., "a valid name can only contain characters in a-z, 0-5 and '.'");
...
template <class... TA>
void E(Error e, const codon::SrcInfo &o, const TA &...args) {
  auto msg = Emsg(e, args...);
  raise_error((int)e, o, msg);
}

template <class... TA>
void E(Error e, const codon::SrcInfo &o = codon::SrcInfo()) {
  auto msg = Emsg(e);
  raise_error((int)e, o, msg);
}
