...
from tala.domain_orchestration import OrchestratedDomainBundle
...
def create_orchestrated_domain_bundle(args):
    ...
    odb = OrchestratedDomainBundle(args, logger).pickle_view
    _store_odb(odb, args.output)


def serialize_odb(odb):
    zipped_odb = zlib.compress(dill.dumps(odb))
