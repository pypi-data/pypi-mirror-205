...
class TransactionsProductSettings(BaseModel):
    ...
    extend_history: Optional[StrictBool] = Field(
        False,
        description="If this is set to true, we will attempt to get as much history as possible. This is not guaranteed to work for all institutions. By default at least 30 days is retrieved (although sometimes this can be more).",
    )
    __properties = ["required", "prefetch", "extend_history"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> TransactionsProductSettings:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> TransactionsProductSettings:
        """Create an instance of TransactionsProductSettings from a dict"""
        if obj is None:
            return None

        if type(obj) is not dict:
            return TransactionsProductSettings.parse_obj(obj)

        _obj = TransactionsProductSettings.parse_obj(
            {
                ...
                "extend_history": obj.get("extend_history") if obj.get("extend_history") is not None else False,
            }
        )
        return _obj
