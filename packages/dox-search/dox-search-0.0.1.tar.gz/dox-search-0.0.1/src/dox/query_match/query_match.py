        ...
        for keyword_match in self.keyword_matches:
            for table, attribute, keywords in keyword_match.value_mappings():
                ...
                for keyword in keywords:
                    # print('Score for keyword: {}'.format(keyword))
                    if isinstance(keyword, frozenset):
                        ...
                        tf = value_index.get_tf(weight_scheme,keyword,table,attribute,metrics['max_frequency'])
                        iaf = value_index.get_iaf(weight_scheme, keyword)
                        tf_iaf_sum+= (tf*iaf)
