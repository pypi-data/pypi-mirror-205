...
class Session(sqlalchemy.db.Model, Table):
    ...
    @classmethod
    def user_by_session_token(cls, session_token: str) -> User | None:
        session = cls.find_by(session_token=session_token)
        if session is not None:
            return session.user()

    @classmethod
    def user_by_driver_token(cls, driver_token: str) -> User | None:
        session = cls.find_by(driver_token=driver_token)
        if session is not None:
            return session.user()

    @classmethod
    def user_by_security_token(cls, security_token: str) -> User | None:
        session = cls.find_by(security_token=security_token)
        if session is not None:
            return session.user()

    @classmethod
    def user_by_api_token(cls, api_token: str) -> User | None:
    ...
    def user(self) -> User | None:
        return User.find_by(xuid=self.xuid)
