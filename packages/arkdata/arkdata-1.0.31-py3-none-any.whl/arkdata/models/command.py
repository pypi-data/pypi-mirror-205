...
class Command(sqlalchemy.db.Model, Table):
    ...
    @classmethod
    def queued_commands(cls, service_id: str = None, server_name: str = None) -> list:
        if server_name is None and service_id is None:
            return []
        if service_id:
            return cls.find_all_by(service_id=service_id, executed=False)
        return cls.find_all_by(server_name=server_name, executed=False)

    @classmethod
    def completed_commands(cls, service_id: str = None, server_name: str = None) -> list:
        if server_name is None and service_id is None:
            return []
        if service_id:
            return cls.find_all_by(service_id=service_id, executed=True)
        return cls.find_all_by(server_name=server_name, executed=True)
