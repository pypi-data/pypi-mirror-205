...
def run_ml_gpr(file_i, file_v, ml_gpr_config_file, flag_config, eor_bins_list, ps_conf,
    ...
    if file_noise is not None:
        ...
        if ml_gpr_config.kern.noise.estimate_baseline_noise_from_stokes == 'V':
            noise_cube_df = v_cube
        elif ml_gpr_config.kern.noise.estimate_baseline_noise_from_stokes == 'I':
            noise_cube_df = i_cube
        else:
            print('Error: estimate_baseline_noise_from_stokes must be I or V')
            return
        if ml_gpr_config.kern.noise.estimate_baseline_noise_remove_n_pca > 0:
            from ps_eor import fgfit

            fitter = fgfit.PcaForegroundFit(ml_gpr_config.kern.noise.estimate_baseline_noise_remove_n_pca)
            noise_cube_df = fitter.run(noise_cube_df, noise_cube_df).sub

        noise_cube_df = noise_cube_df.make_diff_cube()

        w = i_cube.weights.copy()
        w.scale_with_noise_cube(noise_cube_df, sefd_poly_fit_deg=ps_conf.empirical_weighting_polyfit_deg,
                                sefd_filter_n_bins=ps_conf.empirical_weighting_n_bins)
        sefd = v_cube.make_diff_cube().estimate_sefd()
...
@click.argument('instru', type=click.Choice(['ska_low', 'nenufar', 'nenufar_80', 'nenufar_52', 'lofar_hba', 'a12_hba',
                                             'a12_lba', 'mwa1', 'hera', 'hera_56', 'hera_120', 'hera_208', 'hera_320'], 
...
def simu_noise_img(uv_simu_h5, total_time_hour, out_filename, min_weight):
    ...
    simu_gridded = obssimu.TelescopGridded.load(uv_simu_h5)
    freqs = simu_gridded.weights.freqs
    uv_simu = simu_gridded.get_tel_gridded_uv(freqs[0], freqs[-1])
...
def simu_noise_ps_zrange(uv_simu_h5, total_time_hour, k, out_filename, bandwidth, delta_z, dk_over_k,
    ...
    for z in zs:
        ...
        uv_simu_z = simu_gridded.get_tel_gridded_uv(mfreq - bandwidth / 2 * 1e6, mfreq + bandwidth / 2 * 1e6)
