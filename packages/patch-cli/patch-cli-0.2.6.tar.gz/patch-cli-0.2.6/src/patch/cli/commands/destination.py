...
def mutate_create_destination(client, console, create_destination_input):
    gql_query = client.prepare_mutation('createDestination', input=create_destination_input)
    with console.status("[bold yellow]Creating destination ...[/bold yellow]") as _status:
        gql_result = gql_query.execute()
    return gql_result


def create_destination_from_file(_client, _console, config_file):
...
def create(patch_ctx: PatchClickContext, config, interactive):
    ...
    create_destination_input = None
    if interactive:
        destination_config = ConfigInteractive(console, config)
        config_values = destination_config.resolve_config(field_specs)
        result_destination = destination_config.send_to_gql(patch_ctx, config_values, spec=DestinationConfigSpec)
        console.print(f"[green]Succeeded[/green]")
        console.print(f"Destination Name: [yellow]{result_destination.name}[/yellow]")
    else:
        create_destination_input = create_destination_from_file(client, console, config)
        if create_destination_input is not None:
            mutate_create_destination(client, console, create_destination_input)
            console.print("[bold green]Destination submitted![/bold green] "
                        "Your destination is being processed, and will be ready soon!")
