...
class InputAnalysisResult:
    ...
    worth_watching = TypeCheckedProperty(bool)

    def __init__(self, input_value, value, ceval_results, worth_watching):
        if (
            ...
            self.value = value
            self.worth_watching = worth_watching
...
def initialize_input_tracing():
    # This function is necessary for now because we have to conditionally import agent_lib.modules.LIB_CONTRAST
...
def check_method_tampering(input_value, worth_watching):
    evaluations = []
    ...
    return [InputAnalysisResult(input_value, input_value, c_eval_res, worth_watching)]
...
def map_result_and_free_eval_result(
    ret, results, results_len, name, value, is_valid_return, evaluations, worth_watching
):
    if ret == 0 and bool(results) and results_len.value > 0:
        for i in range(results_len.value):
            evaluations.append(
                InputAnalysisResult(name, value, results[i], worth_watching)
            )
