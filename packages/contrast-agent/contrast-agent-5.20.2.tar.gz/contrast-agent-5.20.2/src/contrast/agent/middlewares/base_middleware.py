...
class BaseMiddleware(RoutesMixin):
    ...
    def __init__(self):
        ...
        already (e.g. by the runner) and then loads the settings and reporting
        client.
        ...
        if not agent_state.is_initialized():
            agent_state.initialize(runner=False)

        self.settings = agent_state.get_settings()
        if not self.is_agent_enabled():
            return

        self.reporting_client = agent_state.get_reporting_client()
    ...
    def prefilter(self):
        """
        Prefilter - AKA input analysis - is performed mostly with agent-lib but partly
        in the agent.
        ...
        2. agent-lib returns input analysis to use for later sink / infilter analysis,
    ...
    def agent_prefilter(self):
        """
        Prefilter for any rules that do not yet use agent-lib
        """
        with scope.contrast_scope():
    ...
    def postfilter_defend(self, context):
        ...
        if self.settings.config.is_agent_lib_enabled:
            current_user_input_analysis = context.user_input_analysis
            ...
            if context.user_input_analysis:
                for rule in context.user_input_analysis:
                    if rule.score >= 90:
                        attack = BaseRule().build_or_append_attack(rule)
                        context.attacks.append(attack)
            current_user_input_analysis.extend(context.user_input_analysis)
            context.user_input_analysis = current_user_input_analysis
        for rule in rules.values():
