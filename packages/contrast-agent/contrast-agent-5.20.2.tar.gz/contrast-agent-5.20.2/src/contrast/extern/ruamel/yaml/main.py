...
from contrast.extern.ruamel.yaml.compat import StringIO, BytesIO, with_metaclass, nprint, nprintf  # NOQA
...
from contrast.extern.ruamel.yaml.comments import CommentedMap, CommentedSeq, C_PRE
...
class YAML:
    def __init__(self, *, typ=None, pure=False, output=None, plug_ins=None):  # input=None,
        ...
        self.Resolver = yaml.resolver.VersionedResolver  # type: Any
        ...
        if 'rt' in self.typ:
            ...
            self.Emitter = (
                yaml.emitter.Emitter if pure or CEmitter is None else CEmitter
            )
            self.Representer = yaml.representer.SafeRepresenter
            self.Parser = yaml.parser.Parser if pure or CParser is None else CParser
            self.Composer = yaml.composer.Composer
            self.Constructor = yaml.constructor.SafeConstructor
        elif 'base' in self.typ:
            self.Emitter = yaml.emitter.Emitter
            self.Representer = yaml.representer.BaseRepresenter
            self.Parser = yaml.parser.Parser if pure or CParser is None else CParser
            self.Composer = yaml.composer.Composer
            self.Constructor = yaml.constructor.BaseConstructor
        elif 'unsafe' in self.typ:
            self.Emitter = (
                yaml.emitter.Emitter if pure or CEmitter is None else CEmitter
            )
            self.Representer = yaml.representer.Representer
            self.Parser = yaml.parser.Parser if pure or CParser is None else CParser
            self.Composer = yaml.composer.Composer
            self.Constructor = yaml.constructor.Constructor
        elif 'rtsc' in self.typ:
            self.default_flow_style = False
            # no optimized rt-dumper yet
            self.Emitter = yaml.emitter.Emitter
            self.Serializer = yaml.serializer.Serializer
            self.Representer = yaml.representer.RoundTripRepresenter
            self.Scanner = yaml.scanner.RoundTripScannerSC
            # no optimized rt-parser yet
            self.Parser = yaml.parser.RoundTripParserSC
            self.Composer = yaml.composer.Composer
            self.Constructor = yaml.constructor.RoundTripConstructor
        ...
        if setup_rt:
            ...
            self.Serializer = yaml.serializer.Serializer
            self.Representer = yaml.representer.RoundTripRepresenter
            self.Scanner = yaml.scanner.RoundTripScanner
            # no optimized rt-parser yet
            self.Parser = yaml.parser.RoundTripParser
            self.Composer = yaml.composer.Composer
            self.Constructor = yaml.constructor.RoundTripConstructor
        del setup_rt
        self.stream = None
    ...
    def get_constructor_parser(self, stream):
        ...
        if self.Parser is not CParser:
            if self.Reader is None:
                self.Reader = yaml.reader.Reader
            if self.Scanner is None:
                self.Scanner = yaml.scanner.Scanner
            self.reader.stream = stream
        else:
            if self.Reader is not None:
                if self.Scanner is None:
                    self.Scanner = yaml.scanner.Scanner
                self.Parser = yaml.parser.Parser
                self.reader.stream = stream
                ...
                # is actually used
                rslvr = self.Resolver
                # if rslvr is yaml.resolver.VersionedResolver:
                #     rslvr = yaml.resolver.Resolver
...
def warn_deprecation(fun, method, arg=''):
    ...
    from contrast.extern.ruamel.yaml.compat import _F
...
def add_implicit_resolver(
    ...
    if Loader:
        if hasattr(Loader, 'add_implicit_resolver'):
            ...
            Loader, (BaseLoader, SafeLoader, yaml.loader.Loader, RoundTripLoader)
        ):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
    if Dumper:
        if hasattr(Dumper, 'add_implicit_resolver'):
            ...
            Dumper, (BaseDumper, SafeDumper, yaml.dumper.Dumper, RoundTripDumper)
        ):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
...
def add_multi_constructor(tag_prefix, multi_constructor, Loader=None, constructor=Constructor):
    ...
    if Loader is None:
        ...
        elif issubclass(Loader, yaml.loader.Loader):
            Constructor.add_multi_constructor(tag_prefix, multi_constructor)
        elif issubclass(Loader, RoundTripLoader):
