...
http_provider = [
    ...
    #        explicit dependency on `smartpip-providers-http` which needs to be pulled in for them.
...
def get_provider_package_from_package_id(package_id: str) -> str:
    ...
    return f"smartpip-providers-{package_suffix}"
    ...
    def parse_config_files(self, *args, **kwargs) -> None:
        ...
        if os.getenv(INSTALL_PROVIDERS_FROM_SOURCES) == 'true':
            self.install_requires = [
                req for req in self.install_requires if not req.startswith('smartpip-providers-')
            ]
...
def replace_extra_requirement_with_provider_packages(extra: str, providers: List[str]) -> None:
    ...
    { 'salesforce': ['smartpip-providers-salesforce'] }

    And then, 'smartpip-providers-salesforce' package has those 'install_requires' dependencies:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            airflow_provider_packages = [
                ...
                if package_line.startswith("smartpip-providers")
            ]
...
def do_setup() -> None:
    ...
    setup(
        ...
        download_url=('https://www.smartchart.cn/dist/airflow/' + version),
        cmdclass={
