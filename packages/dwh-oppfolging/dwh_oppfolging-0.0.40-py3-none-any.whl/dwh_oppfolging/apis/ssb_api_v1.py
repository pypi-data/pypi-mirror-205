...
def _get_all_versions_for_classification(classification_id: int, force_global_modified_date: bool = False):
    ...
    patch = {"lastModified": data["lastModified"]} if force_global_modified_date else {}
    versions = [_build_version_struct(entry | patch) for entry in data["versions"]]
...
def _get_all_records_in_classification_version(
    ...
    records = [
        ...
        for classification_item in data["classificationItems"]
    ]
...
def _get_all_records_in_correspondence(
    ...
    records = [
        ...
        for correspondence_map in data["correspondenceMaps"]
    ]
    return records
...
def get_latest_records_for_classification(
    ...
    classification_id: int,
    force_global_modified_date: bool = False
):
    """
    ...
    Note!: The SSB api has a bug where their modified dates are not set correctly in the version endpoint.
    Set `force_global_modified_date` to use the classifications global last modified date as a workaround.
    The downside of this is that one cannot tell which versions have been changed,
    so older or unchanged versions may still be returned.
    """
    for version in _get_all_versions_for_classification(classification_id, force_global_modified_date):
