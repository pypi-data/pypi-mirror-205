...
class ParametricFaceModel:
    def __init__(self, 
                #bfm_folder=os.path.join(cwd, "BFM"), 
                folder_name = 'deep3dmm',
                ckpt_name = 'BFM.tar.xz',
                force=False,
                recenter=True,
                camera_distance=10.,
                init_lit=np.array([
        ...
        bfm_name = check_ckpt_exist(root, ckpt_name = ckpt_name, url_id = url_id, force = force)
        bfm_folder = os.path.expanduser(os.path.join(root, ckpt_name.split(".")[0]))

        if not os.path.exists(bfm_folder):
            os.system(f"tar -xvf {bfm_name} -C {root}")
