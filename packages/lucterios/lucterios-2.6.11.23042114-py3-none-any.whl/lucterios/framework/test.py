...
def add_user(username, is_superuser=False, last_login=timezone.now()):
    user = LucteriosUser.objects.create_user(
        username=username, password=username, last_login=last_login)
    user.first_name = username
    user.last_name = username.upper()
    user.email = "%s@lucterios.org" % username
    user.is_staff = False
...
class XmlClient(Client):
    ...
    def call(self, path, data, method, **extra):
        ...
        if method in ('get', 'delete'):
            ...
            data = None
        return request_action(path, data, content_type=MULTIPART_CONTENT, HTTP_ACCEPT_LANGUAGE=self.language, **extra)
...
class LucteriosTestAbstract(object):
    ...
    def call_ex(self, path, data, is_client, status_expected=200, **extra):
        if is_client is False:
            self.assertEqual('/' + self.factory.xfer.url_text, path)
            self.response = self.factory.call(path, data)
        else:
            self.response = self.client.call(path, data, is_client if isinstance(is_client, str) else 'post', **extra)
        self.clean_resp()
        ...
        if isinstance(selects, dict):
            ...
            try:
                for case in values['case']:
                    vid = case[0]
                    self.assertIn(vid, selects.keys(), str(case))
                    self.assertEqual(selects[vid], case[1], str(case))
            except Exception:
                print(values['case'])
                raise
        else:
