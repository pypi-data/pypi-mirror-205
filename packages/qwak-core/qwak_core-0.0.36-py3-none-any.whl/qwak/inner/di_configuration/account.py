...
class UserAccountConfiguration:
    ...
    # Field names in claims section
    QWAK_GRPC_API = "grpc-api"
    QWAK_DATA_API = "data-api"
    QWAK_MODELS_API = "models-api"
    BI_API_KEY = "bi-api-key"
    QWAK_BUCKET = "qwak-bucket"
    QWAK_PARTNER_ID = "qwak-partner-id"
    ...
    def _get_claims(self):
        try:
            claims = os.environ.get("QWAK_CLAIMS")
            if not claims:
                user_account = self.get_user_config()
                client = self._auth_client(api_key=user_account.api_key)
                try:
