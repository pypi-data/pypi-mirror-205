...
def _get_db_config(db_code):
    db_type, db_name = _infer_db_type_name(db_code=db_code)
    host, port, user, password, database = None, None, None, None, None

    if db_type is not None:
        user = ecfg.get_env_config().get(f'{db_name}.user',
                                         os.getenv(f'{db_name}.user',
                                                   os.getenv(f'{db_type.value}_USER', None)))
        password = ecfg.get_env_config().get(f'{db_name}.password',
                                             os.getenv(f'{db_name}.password',
                                                       os.getenv(f'{db_type.value}_PASSWORD', None)))
        port = ecfg.get_env_config().get(f'{db_name}.port', None)

        host = ecfg.get_env_config().get(f'{db_name}.host',
                                         ecfg.get_env_config().get(db_code,
                                                                   os.getenv(f'{db_name}.host',
                                                                             os.getenv(f'{db_type.value}_HOST', db_code))))
        if db_type==DBType.POSTGRES:
            database = db_name

        # if db_type == DBType.REDSHIFT:
        #     host = ecfg.get_env_config().get(prop='{}.host')
        #     if port is None:
        #         port = ecfg.get_env_config().get(prop='redshift.port', default=5439)
    else:
        host = db_code
...
def read_sql(sql,
    ...
    >>> import qtc.utils.db_utils as dbu
    >>> dbu.read_sql(sql="SELECT * FROM ANTH_REFERENCE.trd.RESTRICTIONS (NOLOCK) " \
                          "WHERE DATA_DATE='2022-05-04'",
                     host='ANTHSYSEQ-DB').head()
        DATA_DATE      CODE  TICKER COUNTRY  TYPE MODE SOURCE           REASON              ENTRY_TIME
    0  2022-05-04  64261093  600764      CN  HOLD  AUT    BAM  BAM Restriction 2022-05-04 05:34:51.330
    1  2022-05-04  64296926  688981      CN  HOLD  AUT    BAM  BAM Restriction 2022-05-04 05:34:51.330
    2  2022-05-04  64366361  003009      CN  HOLD  AUT    BAM  BAM Restriction 2022-05-04 05:34:51.330
    3  2022-05-04  64618032  003009      CN  HOLD  AUT    BAM  BAM Restriction 2022-05-04 05:34:51.330
    4  2022-05-04  64680269  601728      CN  HOLD  AUT    BAM  BAM Restriction 2022-05-04 05:34:51.330
    """
