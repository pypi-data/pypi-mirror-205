...
plot_grouped_HPD95 <- function(data_file, plot_width, plot_height, plot_file){
  ...
  ggplot(dat, aes(x = pollut, y = or, ymin = lcl, ymax = ucl)) +
    geom_pointrange(aes(col = factor(lag)),
                    position=position_dodge(width=0.35),  # control distance between groups
                    linewidth = 0.9,  # line width
                    size=0.6,         # size of shape
                    shape=16) +       # shape
    ylab("95% HPD CI") +
    scale_color_discrete(name = "Lag") +
    xlab("") +
    theme_bw() +
