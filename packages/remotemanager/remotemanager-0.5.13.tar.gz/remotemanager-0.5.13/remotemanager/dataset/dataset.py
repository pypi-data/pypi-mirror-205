...
class Dataset(SendableMixin, LoggingMixin):
    ...
    def _run(self, force: bool = False, dry_run: bool = False, **run_args) -> None:
        ...
        for remote, lines in master_scripts.items():
            ...
            if asynchronous and self.submitter == "bash":
                # adds the content of the stderr back into stderr
                lines.append(
...
\n"""
    ...
    def fetch_results(
        ...
        runfile_mtimes = self._check_for_resultfiles()

        if not any(runfile_mtimes.values()):
            self._logger.info("no valid results found, exiting early")
            Quiet.state = False
            return

        self._logger.info("present result files:")
        self._logger.info(format_iterable(runfile_mtimes))
        for runner in self.runners:
            if runfile_mtimes[runner.resultfile.remote]:
        ...
        if not all(self._is_finished(runfile_mtimes)) and raise_if_not_finished:
    ...
    def _is_finished(self, files: dict = None, verbose: bool = True) -> list:
        ...
        for runner in self.runners:
            ...
            mtime = files[runner.resultfile.remote]
            ...
            self._logger.debug(
                ...
                f"mtime {mtime} vs runner submit time "
    ...
    def wait(self, interval: int = 10, verbose: bool = False) -> None:
        """
