...
class GlobeController {
  constructor(config) {
    ...
    $("<canvas>").attr({id: this.name + "overlaycanvas"})
                 ...
                 .css({position: "absolute", top:30, left:0, opacity:this.config.overlay.opacity})
                 .appendTo(this.parentDiv);

    $("<canvas>").attr({id: this.name + "colorflowcanvas"})
                 .attr({width: this.width + "px", height: this.height + "px"})
                 .css({position: "absolute", top:30, left:0, display:"none"})
                 .appendTo(this.parentDiv);

    $("<canvas>").attr({id: this.name + "particlescanvas"})
                 .attr({width: this.width + "px", height: this.height + "px"})
                 .css({position: "absolute", top:30, left:0, opacity:this.config.flow.opacity})
                 .appendTo(this.parentDiv);

    $("<canvas>").attr({id: this.name + "mapcanvas"})
                 .attr({width: this.width + "px", height: this.height + "px"})
                 .css({position: "absolute", top:30, left:0, opacity: this.config.map.opacity})
                 .appendTo(this.parentDiv);
    ...
    $("<canvas>").attr({id: this.name + "legendcanvas"})
     ...
     this.pointsSvg = d3.select(this.config.parentDivName)
                              ...
                              .style("position","absolute").style("top","30px")
...
buildProfileChart(divname, profileData, labelData){
  ...
  drawColorLegend(mode="overlay") {
    ...
    var textsize = 30
    // Define Sizes
    var titleHeight = 1*textsize;
    if (mode == "overlay" && this.config.overlay.title) {  titleHeight += 1*textsize; }
    if (mode == "overlay" && this.config.overlay.unit)  {  titleHeight += 1*textsize; }
    if (mode == "colorflow" && this.config.flow.title) {  titleHeight += 1*textsize; }
    if (mode == "colorflow" && this.config.flow.unit)  {  titleHeight += 1*textsize; }
    if (this.config.title)  {  titleHeight = 3*textsize; }
    if (this.config.subtitle)  {  titleHeight = 3*textsize; }

    var boxheight = this.height - 2*textsize - titleHeight;
    var boxwidth = 1*textsize;
    var legendx = 1*textsize;
    if (mode == "colorflow") {
      legendx = this.width - 2*textsize;
    }
    var legendy = 1.25*titleHeight;
    boxheight *= 1
    ...
    var tickNumber = Math.min(10,Math.round(boxheight/(5*textsize)));
    var ticks = scale.scale.ticks(tickNumber);
    legendContext.font = textsize + "px Lato";
    var maxTextWidth = 0;
    ...
    if (mode == "overlay") {
      legendContext.fillRect(0,titleHeight,boxwidth+legendx+maxTextWidth+1*textsize,this.height);
    }
    if (mode == "colorflow") {
      legendContext.fillRect(legendx-boxwidth-maxTextWidth,titleHeight,this.width,this.height);
    }
    ...
    if (mode == "overlay") {
      ...
      if (this.config.overlay.title) {
        legendContext.font = "bold " + textsize + "px Lato";
        legendContext.fillText(this.config.overlay.title, legendx, 1*textsize);
      }
      if (this.config.overlay.unit) {
        legendContext.font = "italic " + textsize + "px Lato";
        legendContext.fillText("("+this.config.overlay.unit+")", legendx, 2.2*textsize);
      }
    }
    if (mode == "colorflow") {
      ...
      if (this.config.flow.title) {
        ...
        legendContext.font = "bold " + textsize + "px Lato";
        legendContext.fillText(flowtitle, legendx + boxwidth, 1*textsize);
      }
      if (this.config.flow.unit) {
        legendContext.font = "italic " + textsize + "px Lato";
        legendContext.fillText("("+this.config.flow.unit+")", legendx + boxwidth, 2.2*textsize);
      }
    }
    if (this.config.title) {
      legendContext.textAlign="center";
      legendContext.font = "bold " + textsize + "px Lato";
      legendContext.fillText(this.config.title, this.width/2, 1*textsize);
    }
    if (this.config.subtitle) {
      legendContext.textAlign="center";
      legendContext.font = textsize + "px Lato";
      legendContext.fillText(this.config.subtitle, this.width/2, 2.2*textsize);
    }
    ...
    for (var itick in ticks) {
      ...
      if (mode == "overlay") {
        ...
        legendContext.fillText(tickstext[itick],
            legendx + boxwidth + 0.5*textsize,
            ypos + 0.25*textsize);
        legendContext.beginPath();
        legendContext.moveTo(legendx,ypos);
        legendContext.lineTo(legendx + boxwidth + 0.25*textsize,ypos);
        legendContext.stroke();
      }
      if (mode == "colorflow") {
        legendContext.textAlign="right";
        legendContext.fillText(tickstext[itick],
            legendx - 0.5*textsize,
            ypos + 0.25*textsize);
        legendContext.beginPath();
        if (scale.allSameColors) {
          legendContext.moveTo(legendx - 0.25*textsize,ypos);
...
class VideoExport {
  ...
  buildFrames(save){
    ...
    if (this.parentGlobe.config.export.overlay) {
      ...
      renderctx.drawImage(document.getElementById(this.parentGlobe.name+"overlaycanvas"),0,30);
    }
    if (this.parentGlobe.config.export.particles) {
      renderctx.globalAlpha = $("#"+this.parentGlobe.name+"particlescanvas").css("opacity");
      renderctx.drawImage(document.getElementById(this.parentGlobe.name+"particlescanvas"),0,30);
    }
    if (this.parentGlobe.config.export.map) {
      ...
      renderctx.drawImage(document.getElementById(this.parentGlobe.name+"mapcanvas"),0,30);
