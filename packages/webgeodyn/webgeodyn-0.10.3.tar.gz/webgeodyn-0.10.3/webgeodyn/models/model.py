...
class Model:
    ...
    def __init__(self, dataDirectory=None, dataFormat='default', nth=90, nph=180, normalisation=semiNormalisedSchmidt,
                 keepRealisations=True, state_type='analysed', export=True):
        """
        ...
        :param export: whether the model should be available for download (export = True) or not (export = False)
        :type expot: bool
        """
        self.nth = nth
        self.nph = nph
        ...
        if dataDirectory is not None:
            try:
                ...
                # Old pygeodyn files have the analysis keyword instead of analysed, give it another try with the key analysis instead of analysed, or vice versa.
                if state_type == 'analysis':
                    state_type = 'analysed'
                elif state_type == 'analysed':
                    state_type = 'analysis'
                else:
                    raise ke
    ...
    def addMeasure(self, measureName, measureType, lmax, units=None, data=None, rmsdata=None, times=None):
        ...
        # change name measure
        if measureName == "EF":
            measureName = "EXT_DIPOLE"
            measureType = "EXT_DIPOLE"
    ...
    def loadDataFromFile(self, keepRealisations, state_type='analysed'):
        ...
        if self.dataFormat is not None:
            ...
            except TypeError: # state_type is a valid argument only for pygeodyn hdf5 simulations for the moment
                load_fct(self.dataDirectory, self, keepRealisations)

            print('Finished loading model data with format', self.dataFormat)
        # Else raise an error
        else:
