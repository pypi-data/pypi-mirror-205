...
from .cells import DistortionSupercell, DistortionCluster
...
class Harmonic:
    ...
    def mass_freq_weighted_coordinates(self):
        ...
        # convert to units of zero-point displacement
        conversion = 1.0e10 / np.sqrt(AMU) * np.sqrt(H_PCK / (2 * np.pi) / self.omega)
    ...
    def from_vasp_phonopy(cls, poscar, force_sets, ext_masses=None, trans=True,
                          distortion_expansion=None, distortion_cutoff=None,
                          distortion_poscar=None, force_expansion=(1, 1, 1),
                          q_points=None):
        ...
        if distortion_expansion is not None:
            cluster = DistortionSupercell.from_poscar(
                distortion_poscar, distortion_expansion)
        elif distortion_cutoff is not None:
            cluster = DistortionCluster.from_poscar(
                distortion_poscar, distortion_cutoff)
        ...
        def evaluate_modes(q_point, freqs, vecs):
            ...
            unitcell_displacements = vecs.T.reshape(nmodes, natoms, 3) / \
                np.sqrt(masses)[np.newaxis, :, np.newaxis]
            ...
            displacements = np.tile(unitcell_displacements, (1, n_cell, 1)) * \
                phase[np.newaxis, :, np.newaxis] / np.sqrt(len(q_points))

            nmodes = freqs.size

            real_modes = cls(freqs, displacements.real, coords=cart,
                             band_indices=np.arange(1, nmodes + 1),
                             q_points=np.tile(q_point, (nmodes, 1)))

            imag_modes = cls(freqs, displacements.imag, coords=cart,
                             band_indices=np.arange(1, nmodes + 1),
                             q_points=np.tile(q_point, (nmodes, 1)))
            # print(np.linalg.norm(displacements.imag), np.linalg.norm(displacements.real))

            return real_modes + imag_modes
