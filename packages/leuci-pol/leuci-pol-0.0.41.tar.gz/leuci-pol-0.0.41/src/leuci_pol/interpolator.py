        ...
        for z in range(buffer+1):
            for y in range(-1*buffer, self._M+buffer):
                for x in range(-1*buffer, self._F+buffer):
                    ...
                    p = self.get_fms(u_f, u_m, u_s)
                    padded[x,y,z] = p                            
        for z in range(zza,zza+buffer+1):
            for y in range(-1*buffer, self._M+buffer):
                for x in range(-1*buffer, self._F+buffer):
                    u_f, u_m, u_s = self.get_adjusted_fms(x-buffer,y-buffer,z-buffer)
                    p = self.get_fms(u_f, u_m, u_s)
        ...
        for y in range(buffer+1):
            for z in range(-1*buffer, self._S+buffer):
                for x in range(-1*buffer, self._F+buffer):
                    ...
                    p = self.get_fms(u_f, u_m, u_s)
                    padded[x,y,z] = p                            
        for y in range(yya,yya+buffer+1):
            for z in range(-1*buffer, self._S+buffer):
                for x in range(-1*buffer, self._F+buffer):
                    u_f, u_m, u_s = self.get_adjusted_fms(x-buffer,y-buffer,z-buffer)
                    p = self.get_fms(u_f, u_m, u_s)
        ...
        for x in range(buffer+1):
            for z in range(-1*buffer, self._S+buffer):
                for y in range(-1*buffer, self._M+buffer):
                    ...
                    p = self.get_fms(u_f, u_m, u_s)
                    padded[x,y,z] = p                            
        for x in range(xxa,xxa+buffer+1):
            for z in range(-1*buffer, self._S+buffer):
                for y in range(-1*buffer, self._M+buffer):
                    u_f, u_m, u_s = self.get_adjusted_fms(x-buffer,y-buffer,z-buffer)
                    p = self.get_fms(u_f, u_m, u_s)
...
class Numpest(Interpolator):                
    ...
    def get_value(self, x, y, z):                
        ...
        return self.interp_regular_grid(u_f, u_m, u_s)
    ## implement abstract interface #########################################
    def get_radient(self, x, y, z):
        return self.get_radient_numerical(x,y,z)
    ...
    def get_laplacian(self, x, y, z):
        return self.get_laplacian_numerical(x,y,z)
    ## iplement abstract interface ###########################################    
    def interp_regular_grid(self,x,y,z):
        # dimensional check        
        #if np.shape(self._npy) != (len(x_volume), len(y_volume), len(z_volume)):
        #    print(self._F,self._M,self._S,np.shape(self._npy),(len(x_volume),len(y_volume), len(z_volume)))
        ...
        volume_needed = fn(np.array([x,y,z]))[0]
...
class Linear(Interpolator):                
    ...
    def get_value(self, x, y, z):                
        u_f, u_m, u_s = self.get_adjusted_fms(x,y,z)
        return self.trilin_interp_regular_grid(u_f, u_m, u_s)
    ## implement abstract interface #########################################
    def get_radient(self, x, y, z):
        return self.get_radient_numerical(x,y,z)
    ...
    def get_laplacian(self, x, y, z):
        return self.get_laplacian_numerical(x,y,z)
    ## iplement abstract interface ###########################################    
    def trilin_interp_regular_grid(self,x,y,z):
        # dimensional check        
        #if np.shape(self._npy) != (len(x_volume), len(y_volume), len(z_volume)):
        #    print(self._F,self._M,self._S,np.shape(self._npy),(len(x_volume),len(y_volume), len(z_volume)))
        #    raise ValueError(f'dimension mismatch, volume must be a ({len(x_volume)}, {len(y_volume)}, {len(z_volume)}) list or numpy.ndarray')
