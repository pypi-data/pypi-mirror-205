...
"""
** Properties of a ``movia.core.filter.basic.cut.FilterCut``. **
----------------------------------------------------------------
"""
    ...
    ** Allows to view and modify the properties of a filter of type ``FilterTruncate``.
    ...
    def init_limit(self, grid_layout, ref_span=0):
        ...
        ** Displays and allows to modify the limits. **
        ...
        self.limit_textboxs = [None for _ in range(len(limits))]
    ...
    def update_limit(self, text, limit_index):
        ...
        ** Check that the limit is correct and update all the limits. **
        ...
        if not text and limit_index >= 1 and limit_index + 1 == len(self.state["limits"]):
            ...
            limits = list(map(fractions.Fraction, new_limits))
