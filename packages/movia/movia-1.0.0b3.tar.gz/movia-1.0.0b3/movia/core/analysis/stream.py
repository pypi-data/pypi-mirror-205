...
from movia.core.exceptions import MissingInformation, MissingStreamError



def _decode_duration_ffmpeg(filename: str, index: int, accurate: bool) -> fractions.Fraction:
    """
    ** Extract the duration by the complete decoding of the stream. **
    ...
    Examples
    --------
    >>> from movia.core.analysis.stream import _decode_duration_ffmpeg
    >>> _decode_duration_ffmpeg("movia/examples/audio.ogg", 0, accurate=True)
    Fraction(16, 1)
    >>> _decode_duration_ffmpeg("movia/examples/video.mp4", 0, accurate=True)
    Fraction(16, 1)
    >>> _decode_duration_ffmpeg("movia/examples/intro.mkv", 0, accurate=True)
    Fraction(9809, 1000)
    >>>
    """
    # get context
    ...
    data = data[index]
    time_base = fractions.Fraction(data["time_base"])
    ...
    for duration_key in ("duration", "pkt_duration"):
        ...
        for duration_key in ("duration_time", "pkt_duration_time"):
            if (duration := last.get(duration_key, "N/A")) != "N/A":
                duration = fractions.Fraction(duration)
                break
...
) -> np.ndarray[typing.Optional[fractions.Fraction]]:
    """
    ** Retrieve the exact position of the frames in a stream. **

    Examples
    --------
    >>> from movia.core.analysis.stream import _decode_timestamps_ffmpeg
    >>> _decode_timestamps_ffmpeg("movia/examples/audio.ogg", 0) # doctest: +ELLIPSIS
    array([Fraction(0, 1), Fraction(4, 125), Fraction(8, 125),
           ...
           ...
           Fraction(1968, 125), Fraction(1972, 125), Fraction(1976, 125),
           Fraction(396, 25), Fraction(1984, 125), Fraction(1988, 125),
           Fraction(1992, 125), Fraction(1996, 125)], dtype=object)
    >>> _decode_timestamps_ffmpeg("movia/examples/video.mp4", 0) # doctest: +ELLIPSIS
    ...
    if (headers := get_metadata(filename).get("streams", [])) and index < len(headers):
        ...
        if "time_base" in headers:
            time_base = fractions.Fraction(headers["time_base"])
    else:
        time_base = 0
        ...
        for time_key in ("best_effort_timestamp", "pts", "pkt_pts", "dts", "pkt_dts"):
            ...
            for time_key in (
                ...
                if (time := data.get(time_key, "N/A")) != "N/A":
                    timestamps.append(fractions.Fraction(time))
                    break
            else:
...
def _estimate_duration_ffmpeg(filename: str, index: int, *, _indirect=True) -> fractions.Fraction:
    """
    ** Extract the duration from the metadata. **
    ...
    Examples
    --------
    >>> from movia.core.analysis.stream import _estimate_duration_ffmpeg
    >>> _estimate_duration_ffmpeg("movia/examples/audio.ogg", 0)
    Fraction(16, 1)
    >>> _estimate_duration_ffmpeg("movia/examples/video.mp4", 0)
    Fraction(16, 1)
    >>>
    """
    def parse(duration: typing.Union[numbers.Real, str]) -> typing.Optional[fractions.Fraction]:
        if isinstance(duration, numbers.Real):
            return fractions.Fraction(duration)
        if (match := re.fullmatch(r"(?P<h>\d+):(?P<m>\d\d):(?P<s>\d*\.\d+)", duration)):
            return 3600*int(match["h"]) + 60*int(match["m"]) + fractions.Fraction(match["s"])
        try:
            return fractions.Fraction(duration)
        except ValueError:
            return None
    ...
    if (stream_meta := metadata.get("streams", [])) and len(stream_meta) > index:
        ...
        if "duration_ts" in stream_meta and "time_base" in stream_meta:
            return stream_meta["duration_ts"] * fractions.Fraction(stream_meta["time_base"])
        if "duration" in stream_meta:
            return fractions.Fraction(stream_meta["duration"])
    ...
    if (format_meta := metadata.get("format", None)) is not None:
        if "duration" in format_meta:
            return fractions.Fraction(format_meta["duration"])
...
def _estimate_len_ffmpeg(filename: str, index: int, *, _indirect=True) -> int:
    ...
    ** Extract the number of frames or samples from the metadata. **
    ...
    Examples
    --------
    >>> from movia.core.analysis.stream import _estimate_len_ffmpeg
    >>> _estimate_len_ffmpeg("movia/examples/audio.ogg", 0)
    256000
    >>> _estimate_len_ffmpeg("movia/examples/video.mp4", 0)
    400
    >>>
    """
    metadata = get_metadata(filename)
...
def _estimate_rate_ffmpeg(filename: str, index: int, *, _indirect=True) -> fractions.Fraction:
    """
    ** Retrieves via ffmpeg, the metadata concerning the fps or the framerate. **
    ...
    >>> _estimate_rate_ffmpeg("movia/examples/audio.ogg", 0)
    Fraction(16000, 1)
    >>> _estimate_rate_ffmpeg("movia/examples/video.mp4", 0)
    Fraction(25, 1)
    >>>
    """
    metadata = get_metadata(filename)
    ...
    for rate_key in ("r_frame_rate", "sample_rate", "avg_frame_rate"):
        if rate_key in stream_meta:
            try:
                return fractions.Fraction(stream_meta[rate_key])
...
def _map_index_rel_to_abs(filename: str, index: int, stream_type: str) -> int:
    ...
    ** Relative stream index to absolute stream index. **
    ...
    >>> _map_index_rel_to_abs("movia/examples/intro.mkv", 0, "video")
    0
    >>> _map_index_rel_to_abs("movia/examples/intro.mkv", 1, "video")
    1
    >>> _map_index_rel_to_abs("movia/examples/intro.mkv", 0, "audio")
    2
    >>> _map_index_rel_to_abs("movia/examples/intro.mkv", 1, "audio")
    3
    >>>
    """
...
def get_metadata(
    ...
    ** Call ``ffprobe`` and parse the result as a dictionary. **
    ...
    >>> from movia.core.analysis.stream import get_metadata
    >>> pprint(get_metadata("movia/examples/audio.ogg")) # doctest: +ELLIPSIS
    {'format': {'bit_rate': '26166',
                'duration': '16.000000',
                'filename': 'movia/examples/audio.ogg',
...
def get_slices_metadata(
    ...
    ** Get the packets informations for all streams. **
    ...
    --------
    >>> from pprint import pprint
    >>> from movia.core.analysis.stream import get_slices_metadata
    >>> headers, data = get_slices_metadata("movia/examples/audio.ogg", slice_type="packet")
    >>> pprint(headers)
    [['codec_type',
      'dts',
      'dts_time',
      ...
      'stream_index']]
    >>> pprint(data)
    [array([['audio', '-512', '-0.032000', ..., 'Metadata Update', '116', '0'],
           ['audio', '0', '0.000000', ..., 'N/A', '109', '0'],
           ['audio', '512', '0.032000', ..., 'N/A', '87', '0'],
           ...,
           ['audio', '254464', '15.904000', ..., 'N/A', '103', '0'],
           ['audio', '254976', '15.936000', ..., 'N/A', '118', '0'],
           ['audio', '255488', '15.968000', ..., 'N/A', '118', '0']],
          dtype='<U15')]
    >>> headers, data = get_slices_metadata("movia/examples/video.mp4", slice_type="packet")
    >>> pprint(headers)
    [['codec_type',
      'dts',
      'dts_time',
    ...
    with subprocess.Popen(
        ...
        times_base = [fractions.Fraction(s["time_base"]) for s in data if "time_base" in s]
        ...
        with tqdm.tqdm(
            ...
            while (data := child.stdout.readline()) or child.poll() is None:
                ...
                if not set(data).issubset(headers[stream_index]):
                    ...
                    infos[stream_index] = [
                        parse_data(d, sort_all_h, sort_old_h) for d in infos[stream_index]
                    ]
...
def get_streams_type(
    ...
    ** Retrieves in order the stream types present in the file. **
    ...
    >>> get_streams_type("movia/__main__.py", ignore_errors=True)
    []
    >>>
    """
    filename = pathlib.Path(filename)
