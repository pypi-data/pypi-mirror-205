...
from movia.core.analysis.stream import _decode_timestamps_ffmpeg, _map_index_rel_to_abs
from movia.core.exceptions import MissingStreamError



def _decode_timestamps_cv2(
    ...
    ** Retrieve from cv2 the position of the frames in the video. **
    ...
    Examples
    --------
    >>> from movia.core.analysis.video.properties.timestamps import _decode_timestamps_cv2
    >>> _decode_timestamps_cv2("movia/examples/video.mp4", 0) # doctest: +ELLIPSIS
    ...
    while True:
        ...
        if pos_curr == 0.0 and pos_list:
            ...
            pos_list.append(fractions.Fraction(round(pos_curr), 1000))
    cap.release()
...
def _interpolate(
    sequence: np.ndarray[typing.Optional[fractions.Fraction]]
) -> np.ndarray[typing.Optional[fractions.Fraction]]:
    """
    ** Interpolates a numpy vector to replace the None with a consistent value. **
    ...
    Returns
    -------
    interp : np.ndarray[typing.Optional[fractions.Fraction]]
    ...
    new_vals = np.vectorize(lambda v: fractions.Fraction(v).limit_denominator(1001))(new_vals)
...
def get_timestamps_video(
    ...
    ** Recover the date of appearance of the frames. **
    ...
    Returns
    -------
    dates : typing.Optional[fractions.Fraction]
        The numpy 1d list containing the dates in seconds, encoded in fractions.Fraction.
    ...
    Examples
    --------
    >>> from movia.core.analysis.video.properties.timestamps import get_timestamps_video
    >>> get_timestamps_video("movia/examples/video.mp4") # doctest: +ELLIPSIS
    array([Fraction(0, 1), Fraction(1, 25), Fraction(2, 25), Fraction(3, 25),
           Fraction(4, 25), Fraction(1, 5), Fraction(6, 25), Fraction(7, 25),
           Fraction(8, 25), Fraction(9, 25), Fraction(2, 5), Fraction(11, 25),
           ...
           ...
           Fraction(396, 25), Fraction(397, 25), Fraction(398, 25),
           Fraction(399, 25)], dtype=object)
    >>> get_timestamps_video("movia/examples/intro.mkv") # doctest: +ELLIPSIS
