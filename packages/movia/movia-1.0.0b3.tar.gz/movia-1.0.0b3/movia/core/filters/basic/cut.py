...
class FilterCut(Filter):
    ...
    ** Splits the stream at the given positions. **
    ...
    Attributes
    ----------
    limits : list[fractions.Fraction]
    ...
    def __init__(self, in_streams: typing.Iterable[Stream], *limits: numbers.Real):
        ...
        in_streams : typing.Iterable[Stream]
            Forwarded to ``movia.core.filters.basic.cut.FilterCut``.
        limits : numbers.Real
        ...
        self._limits = list(map(fractions.Fraction, limits))
    ...
    def setstate(self, in_streams: typing.Iterable[Stream], state: dict) -> None:
        assert set(state) == {"limits"}, set(state)-{"limits"}
        limits = map(fractions.Fraction, state["limits"])
...
class _StreamAudioCut(StreamAudioWrapper):
    ...
    ** Select a slice of an audio stream. **
    ...
    def _snapshot(self, timestamp: fractions.Fraction, rate: int, samples: int) -> FrameAudio:
        if timestamp + fractions.Fraction(samples, rate) > self.l_max or timestamp < self.l_min:
