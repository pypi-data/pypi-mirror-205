        ...
        with av.open(
            ...
            for stream, settings in zip(self.in_streams, self.streams_settings):
                if stream.type == "audio":
                    ...
                    stream_av = container_av.add_stream(settings["codec"], rate)
                    ...
                    stream_av.options = settings["options"]
                    stream_av.height, stream_av.width = stream.height, stream.width
                    # stream_av.pix_fmt = None
                    streams_av.append(stream_av)
                else:
            ...
            with tqdm.tqdm(
                ...
                for index, frame in scheduler(list(self.in_streams), rates):
