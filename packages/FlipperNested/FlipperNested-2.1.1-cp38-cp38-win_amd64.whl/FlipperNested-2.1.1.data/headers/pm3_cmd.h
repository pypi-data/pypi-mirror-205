...
#define PM3_SQUIT               2
// Success, transfer nonces            pm3_old:        Sending nonces back to client
...
// Operation not supported by device    client/pm3_old: probably only on pm3_old once client becomes universal
#define PM3_EDEVNOTSUPP        -3
// Operation timed out                  client:     no response in time from pm3_old
#define PM3_ETIMEOUT           -4
// Operation aborted (by user)          client/pm3_old: kbd/button pressed
#define PM3_EOPABORTED         -5
// Not (yet) implemented                client/pm3_old: TBD place holder
#define PM3_ENOTIMPL           -6
// Error while RF transmission          client/pm3_old: fail between pm3_old & card
#define PM3_ERFTRANS           -7
// Input / output error                 pm3_old:        error in client frame reception
#define PM3_EIO                -8
// Buffer overflow                      client/pm3_old: specified buffer too large for the operation
#define PM3_EOVFLOW            -9
// Software error                       client/pm3_old: e.g. error in parsing some data
#define PM3_ESOFT             -10
// Flash error                          client/pm3_old: error in RDV4 Flash operation
#define PM3_EFLASH            -11
// Memory allocation error              client:     error in memory allocation (maybe also for pm3_old BigBuff?)
#define PM3_EMALLOC           -12
// File error                           client:     error related to file access on host
#define PM3_EFILE             -13
// Generic TTY error
#define PM3_ENOTTY            -14
// Initialization error                 pm3_old:        error related to trying to initialize the pm3_old / fpga for different operations
#define PM3_EINIT             -15
// Expected a different answer error    client/pm3_old: error when expecting one answer and got another one
#define PM3_EWRONGANSWER      -16
// Memory out-of-bounds error           client/pm3_old: error when a read/write is outside the expected array
#define PM3_EOUTOFBOUND       -17
// exchange with card error             client/pm3_old: error when cant get answer from card or got an incorrect answer
...
// execute pm3_old cmd failed               client/pm3_old: when one of our pm3_old cmd tries and fails. opposite from PM3_SUCCESS
#define PM3_EFAILED           -21
// partial success                      client/pm3_old: when trying to dump a tag and fails on some blocks.  Partial dump.
#define PM3_EPARTIAL          -22
// tearoff occurred                      client/pm3_old: when a tearoff hook was called and a tearoff actually happened
#define PM3_ETEAROFF          -23

// Got bad CRC                          client/pm3_old: error in transfer of data,  crc mismatch.
#define PM3_ECRC              -24

// STATIC Nonce detect                  pm3_old:  when collecting nonces for hardnested
#define PM3_ESTATIC_NONCE     -25

// No data                              pm3_old:        no data available, no host frame available (not really an error)
