...
def get_ticker_financial_info(zqdm_list,start_date,end_date,with_stock_price=False):
    ...
    for i in range(int(np.ceil(len(zqdm_list) / max_length))):
        ...
        if(len(df1)>0):
            ...
            if(len(df3)==0):
                df3=pd.DataFrame(columns=['INNERCODE','TOTALMV','DIVIDENDRATIO','TRADINGDAY','PE','PB','PCFTTM','PEG','PCF'])
...
class Style_analysis:
    ...
    def save_style_property2db(self,asofdate, time_length=3,if_prv=False,fre='Q'):
        ...
        jjdm_list=list(set(util.get_stock_funds_pool(asofdate,2))
                       .intersection(style_exp['jjdm'].unique()) )
        ...
        for jjdm in jjdm_list:
            ...
            balance_exp_list.append(tempdf['均衡'].mean())
...
class General_holding:
    ...
    @staticmethod
    def read_hld_list_fromdb(start_date,end_date,jjdm_list,keyholdonly=False):
    ...
    def get_fund_jjjzc(jjdm_list,start_date,end_date):
        ...
        sql="select jjdm,jsrq,jjjzc,hbzjzjb,zqzszzjb from st_fund.t_st_gm_zcpz where jjdm in ({2}) and jsrq>='{0}' and jsrq<='{1}'"\
    ...
    def read_hld_style_fromstock(hld,start_date,end_date,if_prv=False):
        ...
        if(new_threshold):
            ...
            for i in range(len(hld_date_list)):
                ...
                #localdb.execute('delete from stock_style_lable')
                tempdf.to_sql('stock_style_lable', index=False, con=localdb, if_exists='append')
    ...
    @staticmethod
    def get_the_initial_constrains(jj_zjh_industry_data,jjdm,last_hld_key,last_hld,current_hld):
        ...
        if (industry_constrains.loc['制造业']['zjbl'] > 0):
            # get the 制造业 change
            change = pd.merge(last_hld_key[last_hld_key['jjdm'] == jjdm].groupby('zjh_industry')['zjbl'].sum(),
                              current_hld[current_hld['jjdm'] == jjdm].groupby('zjh_industry')['zjbl'].sum(),
                              how='outer', on='zjh_industry').fillna(0)
            change['change'] = change['zjbl_y'] - change['zjbl_x']

            # get the last holding distribution
            last_dis = last_hld[last_hld['jjdm'] == jjdm].groupby('zjh_industry')['zjbl'].sum()
            last_dis = pd.merge(last_dis, change[['change']], how='outer', on='zjh_industry').fillna(0)
            last_dis['zjbl'] = last_dis['zjbl'] + last_dis['change']
            last_dis.reset_index(inplace=True)
            last_dis = \
                last_dis[last_dis['zjh_industry'].isin(['大金融', '消费', 'TMT', '周期', '制造'])][['zjh_industry', 'zjbl']]
            last_dis['zjbl'] = last_dis['zjbl'] / last_dis['zjbl'].sum() * industry_constrains.loc['制造业']['zjbl']

            industry_constrains = \
                pd.concat([industry_constrains, last_dis.set_index('zjh_industry')], axis=0).drop('制造业', axis=0)
    ...
    @staticmethod
    def insert_zq_with_constrains(industry_constrains,overlap_hld,first_holding):

        for zqdm in overlap_hld['zqdm'].tolist():
            ...
            if (industry_constrains.loc[tempdf['zjh_industry']]['zjbl'].iloc[0] > 0):
                if (tempdf['zjbl'].iloc[0] > industry_constrains.loc[tempdf['zjh_industry']]['zjbl'].iloc[0]):
                    tempdf['zjbl'] = [industry_constrains.loc[tempdf['zjh_industry'].iloc[0]]['zjbl']]
                ...
                for industry in not_filled_industry_list:
                    ...
                    if (industry_constrains.loc[industry]['zjbl'] > 0):
                        if (tempdf['zjbl'].iloc[0]> industry_constrains.loc[tempdf['zjh_industry'].iloc[0]]['zjbl']):
                            tempdf['zjbl'].iloc[0] = industry_constrains.loc[industry]['zjbl']
    ...
    def key_holding_expantation(self,jjdm_list,asofdate,last_date,
                                regression_strat_date,regression_end_date):
        ...
        current_hld = self.read_hld_list_fromdb(asofdate[0:6]+'01', asofdate,
                                            ['000294', '000549', '001028', '001071', '001072', '001104',
       '001445', '001532', '001581', '001694', '002179', '002181',
       '004263', '005136', '005521', '005630', '005695', '006121',
       '006122', '006154', '006768', '006879', '007460', '008133',
       '008290', '008359', '008371', '008635', '009200', '009714',
       '010385', '010554', '010787', '010792', '011128', '011144',
       '011238', '011251', '040001', '040004', '040005', '040007',
       '040008', '040011', '040015', '040016', '040020', '040025',
       '040035', '160421', '160425', '519002', '519909'], True)
       ...
       '011238', '011251', '040001', '040004', '040005', '040007',
       '040008', '040011', '040015', '040016', '040020', '040025',
       '040035', '160421', '160425', '519002', '519909'], False)
        ...
        jj_ret_data = \
            self.get_fund_daily_return(['000294', '000549', '001028', '001071', '001072', '001104',
       '001445', '001532', '001581', '001694', '002179', '002181',
       '004263', '005136', '005521', '005630', '005695', '006121',
       '006122', '006154', '006768', '006879', '007460', '008133',
       '008290', '008359', '008371', '008635', '009200', '009714',
       ...
       '011238', '011251', '040001', '040004', '040005', '040007',
       '040008', '040011', '040015', '040016', '040020', '040025',
       '040035', '160421', '160425', '519002', '519909'], regression_strat_date, regression_end_date)


        fund_jjjzc=self.get_fund_jjjzc(['000294', '000549', '001028', '001071', '001072', '001104',
       '001445', '001532', '001581', '001694', '002179', '002181',
       '004263', '005136', '005521', '005630', '005695', '006121',
       '006122', '006154', '006768', '006879', '007460', '008133',
       '008290', '008359', '008371', '008635', '009200', '009714',
       ...
       '011238', '011251', '040001', '040004', '040005', '040007',
       '040008', '040011', '040015', '040016', '040020', '040025',
       '040035', '160421', '160425', '519002', '519909'],
        ...
        for jjdm in jjdm_list:
            ...
            #get the initial industry constrains
            industry_constrains=\
                self.get_the_initial_constrains(jj_zjh_industry_data,jjdm,last_hld_key,last_hld,current_hld)

            #update the constrains by published key holding
            adjusted_weight=\
                (current_hld[current_hld['jjdm']==jjdm].groupby('zjh_industry')['zjbl'].sum()).to_frame('adjusted_zjbl')
            ...
            first_holding,industry_constrains=\
                self.insert_zq_with_constrains(industry_constrains, overlap_hld, first_holding)
            ...
            temp_last_hld.loc[temp_last_hld['spjg'].notnull(),'zjbl']=\
                temp_last_hld.loc[temp_last_hld['spjg'].notnull()]['spjg']*temp_last_hld.loc[temp_last_hld['spjg'].notnull()]['ccsl']/temp_last_hld.loc[temp_last_hld['spjg'].notnull()]['jjjzc']
            ...
            first_holding,industry_constrains=\
                self.insert_zq_with_constrains(industry_constrains, temp_last_hld, first_holding)
            first_holding=pd.concat(first_holding,axis=0)


            not_filled_industry_list=\
                industry_constrains[industry_constrains['zjbl']>0].sort_values('zjbl',ascending=False).index.tolist()
            not_filled_industry_list.remove('合计')
            ...
            if(len(not_filled_industry_list)>0):
                ...
                # company holding
                company_jj_list=\
                    jj_company_info[jj_company_info['glrm']==jj_company_info[jj_company_info['jjdm']==jjdm]['glrm'].iloc[0]]['jjdm'].tolist()
                ...
                company_holding=pd.merge(company_holding.groupby('zqdm')[['zjbl']].mean(),company_holding[['zqdm','zjh_industry','yjxymc']]
                                         ,how='left',on='zqdm')[['zqdm','zjbl','zjh_industry','yjxymc']].drop_duplicates('zqdm')
                ...
                company_holding, totalnum, not_filled_industry_list,industry_constrains, regression_holding=\
                    self.insert_zq_for_regression_pool(company_holding,totalnum,not_filled_industry_list,
                                      industry_constrains,regression_holding)


                #get the all mutual holding if necessary
                not_filled_industry_list = \
                    industry_constrains[industry_constrains['zjbl'] > 0].sort_values('zjbl',
                ...
                if (len(not_filled_industry_list) > 0):
                    ...
                    market_holding = pd.merge(market_holding.groupby('zqdm')[['zjbl']].mean(),
                                               market_holding[['zqdm', 'zjh_industry', 'yjxymc']]
                                               , how='left', on='zqdm')[
                    ...
                    market_holding, totalnum, not_filled_industry_list, industry_constrains, regression_holding = \
                        self.insert_zq_for_regression_pool(market_holding, totalnum, not_filled_industry_list,
                                                           industry_constrains, regression_holding)

            regression_holding=pd.concat(regression_holding,axis=0)

            temp_regress_data=\
                return_data[return_data['zqdm'].isin(list(set(first_holding['zqdm'].tolist()+regression_holding['zqdm'].tolist())))]
...
if __name__ == '__main__':
    ...
    #
    # data=pd.read_csv(r"C:\Users\xuhuai.zhe\Downloads\theme_ret.csv")
    # data.to_sql('nav_theme_ret_new',con=localdb,index=False,if_exists='append')
    gh=General_holding()
    asofdate='20221230'
    gh.key_holding_expantation(util.get_stock_funds_pool('20220930') ,'20220930','20220630','20220901','20221031')

    # gh.save_style_indexhistory2db()
    # gh.save_size_indexhistory2db()
    # gh.save_industry_financial_stats2db(start_date='20220630',end_date='20221230')
    ...
    #     temp_jjdm_list=jjdm_list[i*trunk_size:(i+1)*trunk_size]
        ...
        # industry_hld_list=\
        #     gh.save_industry_exp2db(temp_jjdm_list,start_date='20220630',end_date='20221231')
        #
        #
        # for j in range(0,len(industry_hld_list)-1):
        #
        #     industry_hld_list[j].to_sql('885001_hbs_industry_class{0}_exp'.format(j + 1),
        #                                                             index=False,
        #                                 if_exists='append', con=localdb)
        #
        # print('{} done'.format(i))
    ...
    # gh.save_style_exp2db(jjdm_list,start_date='20220630'
    #                      ,end_date='20221231',if_885001=False)
    # table_name=''
    ...
    # jjdm_list.sort()
    #
    # jjdm_list = util.get_stock_funds_pool('20230331')
    # gh.save_holding_trading_2db(jjdm_list, '20191231', '20221231')
    # gh.get_holding_trading_analysis('20221230')
    # gh.get_hld_property(jjdm_list,'20191231', '20231301')
    # gh.ticker_contribution(jjdm_list, '20191201', '20221231')
    # gh.ticker_contribution(jjdm_list, '20201231', '20211231')
