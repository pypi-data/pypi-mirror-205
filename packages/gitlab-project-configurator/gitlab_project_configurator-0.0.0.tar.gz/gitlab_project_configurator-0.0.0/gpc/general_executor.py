...
class GpcGeneralExecutor(GpcConfigValidator):
    ...
    def _check_warnings(self):
        if self._warnings and self.params.mode == RunMode.DRY_RUN:
            ...
            for path, warnings in self._warnings.items():
                click.secho(f"  >> Configuration for path {path} has warnings.", fg="red")
    ...
    def project_executor(
        ...
        try:
            ...
            if rule_exec.warnings:
                self._warnings[project] = rule_exec.warnings
        except Exception as e:
    ...
    def __generate_report_file(self):
        if self.params.report_file:
            ...
            with open(self.params.report_file, "w", encoding="utf-8") as f:
