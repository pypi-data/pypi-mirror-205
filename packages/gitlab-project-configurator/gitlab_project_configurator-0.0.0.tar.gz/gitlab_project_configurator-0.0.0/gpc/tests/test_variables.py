...
def test_variables(monkeypatch, keep_variables, mocker, fake_gitlab, fake_project):
    ...
    if keep_variables:
        ...
        assert change_variables.differences.get("ENV_2").get("after").get("value") == "e****2"
        ...
        assert change_variables.after[1].value == "env_2"
        assert change_variables.after[1].value_hidden == "e****2"
...
@pytest.mark.parametrize("value", ["env_1", "12345678\n"], ids=["with_underscore", "with_return"])
def test_variables_ko_masked(value, mocker, fake_gitlab, fake_project):
    ...
    executor = get_executor(p, VariablesSettingExecutor)
    assert (
        executor.error_message == "The 'ENV_1' value does not respect the requirements"
