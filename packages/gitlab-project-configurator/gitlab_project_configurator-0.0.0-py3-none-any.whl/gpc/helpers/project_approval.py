...
class ProjectApprovalRules:
    ...
    def __init__(self, project: Project):
        ...
        self._approval_rules_list = None

    @property
    def approval_rules(self):
        if self._approvalrules_config is None:
            self._approvalrules_config = (
                self._approvalrules_manager.list(retry_transient_errors=True) or [None]
            )[0]
    ...
    def set_approvers(  # noqa: C901
        ...
        if not premium and self.approval_rules:
            self._approvalrules_manager.delete(self.approval_rules.id, retry_transient_errors=True)
            self._approvalrules_config = None
            self._approvalrules_manager.create(config, retry_transient_errors=True)
        try:
            ...
            if e.response_code == 400:
                self._approvalrules_manager.update(
