    ...
    `}_renderDone(){return this._error?this._renderMessage("👀",this._error,!0):h`
      ...
      ${this._installed?h`
          ...
          `:h`
            ...
            ></mwc-button>
          `}
    `}firstUpdated(e){super.firstUpdated(e),U().then((e=>{this._hasWifiSecrets=e}))}updated(e){if(super.updated(e),e.has("_state")||e.has("_hasWifiSecrets")){const e=this.shadowRoot.querySelector("mwc-textfield, mwc-radio, mwc-button");e&&e.updateComplete.then((()=>e.focus()))}if("pick_board"==this._state){if(e.has("_state")){(e=>x(`./boards/${e}`))(this._platform.toLowerCase()).then((e=>{this._supportedBoards=e,this._busy=!1}))}if(e.has("_busy")&&!this._busy){const e=this.shadowRoot.querySelector("select");e&&e.focus()}}}async _handleBasicConfigSubmit(){const e=this._inputName,t=e.reportValidity(),c=!!this._hasWifiSecrets||this._inputSSID.reportValidity();if(!t)return void e.focus();if(!c)return void this._inputSSID.focus();const o=e.value;this._data.name=o,this._hasWifiSecrets||(this._wifi={ssid:this._inputSSID.value,password:this._inputPassword.value}),setTimeout((()=>{this._state=T&&H?"connect_webserial":"pick_platform"}),0)}_handleUseRecommendedCheckbox(e){this._useRecommended=e.target.checked}_handlePickBoardSelect(e){this._board=e.target.value}async _handlePickPlatformClick(e){this._supportedBoards=void 0,this._platform=e.currentTarget.platform,this._board=this._platformData().defaultBoard,this._useRecommended&&null!==this._board?await this._handlePickBoardSubmit():(this._busy=!0,this._state="pick_board")}async _handlePickBoardSubmit(){if(this._board){this._data.board=this._board,this._busy=!0;try{this._wifi&&await W(this._wifi.ssid,this._wifi.password);const e=await w(this._data);this._configFilename=e.configuration,$(),this._apiKey=await S(this._configFilename),this._state="done"}catch(e){this._error=e.message||e}finally{this._busy=!1}}}_handleConnectSerialSkip(){this._error=void 0,this._state="pick_platform"}async _handleConnectSerialSubmit(){var e;let t;this._busy=!0,this._error=void 0;let c=!1;try{let o,i;try{o=await navigator.serial.requestPort()}catch(e){return console.error(e),void("NotFoundError"===e.name?A():this._error=e.message||String(e))}t=B(o),this._state="connecting_webserial";try{await t.main_fn(),await t.flash_id()}catch(e){return console.error(e),this._state="connect_webserial",void(this._error="Failed to initialize. Try resetting your device or holding the BOOT button while selecting your serial port until it starts preparing the installation.")}this._state="prepare_flash";const r=t.chip.CHIP_NAME;if(!Object.keys(I).includes(r))return this._state="connect_webserial",void(this._error=`Unable to identify the connected device (${t.chip.CHIP_NAME}).`);i=I[r],this._data.board=null!==(e=D[i].defaultBoard)&&void 0!==e?e:void 0;try{const{configuration:e}=await w(this._data);this._configFilename=e}catch(e){return console.error(e),this._state="connect_webserial",void(this._error="Unable to create the configuration")}c=!0;try{await C(this._configFilename)}catch(e){return console.error(e),this._state="connect_webserial",void(this._error="Unable to compile the configuration")}this._state="flashing";try{const e=await L(this._configFilename);await O(t,e,!0,(e=>{this._writeProgress=e}))}catch(e){return console.error(e),this._state="connect_webserial",void(this._error="Error installing the configuration")}c=!1,this._installed=!0,await N(t.transport),this._state="wait_come_online";try{await new Promise(((e,t)=>{const c=R((t=>{t[this._configFilename]&&(c(),clearTimeout(o),e(void 0))})),o=setTimeout((()=>{c(),t("Timeout")}),2e4)}))}catch(e){console.error(e),this._error="Configuration created but unable to detect the device on the network"}this._state="done"}finally{this._busy=!1,t&&(console.log("Closing port"),await t.transport.disconnect()),c&&await E(this._configFilename)}}async _handleClose(){this.parentNode.removeChild(this)}async _handleCopyApiKey(){var e;q(this._apiKey),this._inputApiKeyBanner.style.setProperty("display","flex"),await K(3e3),null===(e=this._inputApiKeyBanner)||void 0===e||e.style.setProperty("display","none")}};me.styles=[P,k`
      :host {
