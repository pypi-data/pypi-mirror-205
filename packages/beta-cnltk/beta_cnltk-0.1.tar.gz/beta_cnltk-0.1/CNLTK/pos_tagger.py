...
vocabs_pkl_filepath = os.path.join(
    os.path.dirname(__file__), 'models', 'vocab_model3.pkl')

pos_h5_filepath = os.path.join(os.path.dirname(
    __file__), 'models', 'POS_TAG_WORD_validated.h5')
...
def POS_TAGGER_MODEL(test_samples):
    ...
    model2 = keras.models.load_model(modelx, custom_objects={
                                     'attention': attention, "CRF": CRF, 'crf_loss': crf_loss, 'crf_viterbi_accuracy': crf_viterbi_accuracy}, compile=True)
    model2.compile(optimizer=tf.optimizers.Adam(lr=0.008), loss=losses.crf_loss, metrics=[
                   tf.keras.metrics.Recall(), tf.keras.metrics.AUC(), tf.keras.metrics.Precision(), 'accuracy'])
    # model2.summary()
...
def predict_POS_model(test_sample):
    ...
    predix = logits_to_tokens(
        predictions, {i: t for t, i in tagindex_model.items()})
...
# test_sentence = 'Na covid nuon si Glenn ug si Ondoy , atong niadto sila ug regional , nagkuha sila tambal paracetamol ugma sa hospital'
# sentz, tagz = predict_POS_model(test_sentence)
# print(sentz)
# print(tagz)
