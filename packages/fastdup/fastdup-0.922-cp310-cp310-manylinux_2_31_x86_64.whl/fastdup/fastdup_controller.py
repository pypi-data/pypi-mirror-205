...
class FastdupController:
    ...
    def _merge_df_with_annot(self, df: pd.DataFrame, left_on: List[str],
        ...
        for col_name in left_on:
            ...
            df2 = pd.merge(df2, df_annot, left_on=col_name, right_on=FD.ANNOT_FD_ID, how='left')
            df_annot.columns = original_col_names
            if len(df2) == 0:
                assert len(df2), f"Failed to merge df with annotations {col_name} {df_annot.columns} {df2.head()} {df_annot.head()}"
        return df2
