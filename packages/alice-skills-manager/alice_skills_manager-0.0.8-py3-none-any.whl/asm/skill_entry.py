...
DEFAULT_CONSTRAINTS = '/etc/alice/constraints.txt'
FIVE_MINUTES = 300
...
class SkillEntry(object):
    ...
    def __init__(self, name, path, url='', sha='', asm=None):
        url = url.rstrip('/')
        ...
        if asm:
            u = url.lower()
            self.meta_info = asm.repo.skills_meta_info.get(u, {})
        else:
            self.meta_info = {}
        if name is not None:
            self.name = name
    ...
    @property
    def is_dirty(self):
        """True if different from the version in the alice-skills repo.

        Considers a skill dirty if
        - the checkout sha doesn't match the alice-skills repo
        - the skill doesn't exist in the alice-skills repo
        ...
        skill_shas = {d[0]: d[3] for d in self.asm.repo.get_skill_data()}
    ...
    @classmethod
    def from_folder(cls, path, asm=None, use_cache=True):
        """Find or create skill entry from folder path.

        Arguments:
            path:       path of skill folder
            asm:        asm instance to use for caching and extended information
                        retrieval.
            use_cache:  Enable/Disable cache usage. defaults to True
        """
        if asm and use_cache:
            skills = {skill.path: skill for skill in asm.local_skills.values()}
            if path in skills:
                return skills[path]
        return cls(None, path, cls.find_git_url(path), asm=asm)
    ...
    def match(self, query, author=None):
        search, search_tokens, search_common = self._extract_tokens(
            query, ['skill', 'fallback', 'alice']
        )

        name, name_tokens, name_common = self._extract_tokens(
            self.name, ['skill', 'fallback', 'alice']
        )
