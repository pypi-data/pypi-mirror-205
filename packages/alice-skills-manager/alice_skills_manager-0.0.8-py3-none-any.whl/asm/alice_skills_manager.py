...
ASM can be used on the command line but is also used by Alice core daemons.
"""
...
class AliceSkillsManager(object):
    SKILL_GROUPS = {'default', 'alice_mark_1', 'picroft', 'kde',
                    'respeaker', 'alice_mark_2', 'alice_mark_2pi'}
    ...
    def __init__(self, platform='default', old_skills_dir=None,
        ...
        self.skills_dir = (skills_dir or
                           BaseDirectory.save_data_path('alice/skills'))

        self.repo = repo or SkillRepo()
        self.versioned = versioned
        self.lock = AsmProcessLock()
    ...
    def _refresh_skill_repo(self):
        """Get the latest alice-skills repo code."""
        try:
            self.repo.update()
    ...
    def _get_remote_skills(self):
        """Build a dictionary of skills in alice-skills repo keyed by id"""
        remote_skills = []
        for name, _, url, sha in self.repo.get_skill_data():
            ...
            remote_skills.append(
                SkillEntry(name, skill_dir, url, sha, asm=self)
            )
    ...
    def _merge_remote_with_local(self, remote_skills):
        ...
        for skill_file in glob(path.join(self.skills_dir, '*', '__init__.py')):
            skill = SkillEntry.from_folder(path.dirname(skill_file), asm=self,
                                           use_cache=False)
    ...
    def find_skill(self, param, author=None, skills=None):
        ...
        if param.startswith('https://') or param.startswith('http://'):
            ...
            return SkillEntry(name, skill_directory, param, asm=self)
        else:
            skill_confs = {
