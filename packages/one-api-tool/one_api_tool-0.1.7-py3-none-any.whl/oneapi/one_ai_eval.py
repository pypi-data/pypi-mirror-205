...
eval_prompt_solve_first_instruction = """Please solve the [Question] independently to obtain the [Correct answer], and then evaluate and comment each [Candidate answer] based on the [Correct answer]. Finally, output all [Candidate answers] scores (0-1) in a summary format of {"number": "score"}."""
eval_prompt_with_target_instruction = """Please evaluate and comment each [Candidate answer] based on the [Correct answer]. Then output all [Candidate answer] scores (0-1) in a summary format of {"number": "score"}."""
eval_prompt_with_target_template = "[Question]: {q}\n[Correct answer]: {target}\n[Candidate answer]:\n{options}\n[System]:\n{instruction}"
eval_prompt_without_target_template = "[Question]: {q}\n[Candidate answer]:\n{options}\n[System]:\n{instruction}"


def eval_one(prompt: str, answers: List[str], target: Union[str, None], api_tool: OneAPITool, model: str) -> Union[List[float], None]:
    answers_option_number = generate_letters(len(answers))
    format_option_data = '\n'.join([f'{answers_option_number[i]}. {answers[i]}' for i in range(len(answers))])
    target = '' if target and len(target) > 1024 else target
    if target: 
        prompt = eval_prompt_with_target_template.format(q=prompt, target=target,options=format_option_data, instruction=eval_prompt_with_target_instruction)
    else: 
        prompt = eval_prompt_without_target_template.format(q=prompt, options=format_option_data, instruction=eval_prompt_solve_first_instruction)
    result = ''
    try:
        result = api_tool.simple_chat(prompt, model=model)
        result_json = extract_all_json(result)
        return [float(result_json.get(answers_option_number[i], 0)) for i in range(len(answers))]
...
def eval_one_group(data_group: pd.DataFrame, api_tool: OneAPITool, model: str) -> pd.DataFrame:
    ...
    if 'target' in data_group.keys():
        ...
        target = ''
    return eval_one(prompt, answers, target, api_tool, model)
...
def eval_one_file(config_file, 
    ...
    assert {'prompt', 'output'} - set(eval_data.keys()) == 0, f'format error, eval data must include column:\"prompt\", \"output\"'
    ...
    if sample_num > 0:
        sample_keys = random.sample(grouped.groups.keys(), len(grouped))
    else:
    ...
    for key in tqdm(sample_keys):
        ...
        result = eval_one_group(group, tool, eval_model_name)
        if result:
    ...
    if 'model' in eval_data.keys():
        ...
        if 'category' in eval_data.keys():
            score_category = eval_results_df.groupby(['model', 'category'])['score'].sum().reset_index().sort_values(by='model')
            print(f'{"-"*20}SCORE BY CATEGORY{"-"*20}\n{score_category.to_markdown(index=False)}')
