...
class Conformant(ModelDimension):
    ...
    def fit(fhir_path: pd.DataFrame, context: dict) -> pd.DataFrame:
        ...
        context_paths = list(pd.DataFrame(context["dim"])['path']) # just make this np array
...
class Complete(ModelDimension):
    ...
    def fit(fhir_path: pd.DataFrame, context: dict) -> pd.DataFrame:
        ...
        fhir_path['Complete'] = fhir_path.query("Conformant == 1").apply(_score_dim, axis=1)
        return fhir_path
...
class Plausible:
    ...
    def subject_score(fhir_path, subject: dict):
        
        # display(fhir_path)
        score = 0
        if type(subject) != dict:
            return 0
        elif subject.get("reference").replace("Patient/","") in fhir_path['patient_id']:
    ...
    def type_score(clinical_status: dict):
        VALID_CODES = [
            "pay",
            "EHCPOL",
            "HSAPOL",
    ...
    def fit(fhir_path: pd.DataFrame, context: dict) -> pd.DataFrame:
        ...
        def _score_dim(dim: pd.Series):
            ...
            if dim["Complete"] in [None, 0]:
                ...
                return Plausible.subject_score(fhir_path, value)
            elif dim_path.endswith(".period"):
                return Plausible.period_score(value)
            elif dim_path == "Coverage.type":
                return Plausible.coverage_type_score(value)

        fhir_path["Plausible"] = fhir_path.query("Conformant == 1").apply(_score_dim, axis=1)
        return fhir_path
