    ...
    <div :class="$style['ops-room-header__wrapper']">
      ...
      <div :class="$style['ops-info-wrapper']">
        <div :class="$style['ops-text-ellipsis']">
          ...
          >
            <RoomHeaderStatus
              :total-users="room?.people"
              :online-users="currentUsersOnline"
            >
              <template #trigger>
...
const filterUserOnline = computed(() => {
  const totalUsersInRoom = removeOwnIdOnline(
    websocketService.peopleInRoom.value.map((el) => el?.id)
  );

  const currentRoomOnline = currentUsersOnline.value.filter((el: number) =>
    totalUsersInRoom.includes(el)
  );
  return `online (${currentRoomOnline?.length}/${totalUsersInRoom?.length})`;
});
...
//header
.ops-room-header {
  @apply flex items-center w-full h-[4rem] z-[10] mr-[1px] border-b border-grey-100 bg-white px-3;
