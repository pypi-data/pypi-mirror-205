...
class Broker:
    ...
    def add_strategy_symbols(self, symbols, exchange):
        ...
        not_found_symbols = [symbol for symbol in symbols
            if symbol not in self.exchange_symbol_to_instrument_id_map[exchange]]

        Constants.logger.error("Symbols [{}] not found in data provided by broker".format(not_found_symbols))
        self.streaming_instruments = list(
            set(self.streaming_instruments + self.strategy_instruments)
        )
        print("Adding {} {} symbols for streaming".format(len(tradable_symbols), exchange))
    ...
    def __add_interval_ticks_to_market_df(self, interval):
        ...
        for symbol in self.ongoing_tick_updates[interval]:
            ...
            if symbol in self.prev_tick_volume[interval]:
                volume = interval_tick['total_volume'] - self.prev_tick_volume[interval][symbol]
            ...
            for tick in ticks:
                ...
                if (symbol in self.ongoing_tick_updates[interval]) and (
                    ...
                    if symbol not in self.ongoing_tick_updates[interval]:
                        Constants.logger.info(
                            "Found new tick for {} last_trade_time {} tick_interval_time {} volume_till_now {}".format(
                                symbol, last_trade_time, tick_interval_time, volume_till_now
                            )
                        )
                        self.__upsert_ongoing_tick(
                            interval,
    ...
    def place_or_modify_order(self, quantplay_order, is_new_order=True):
        ...
        instrument_id = self.exchange_symbol_to_instrument_id_map[exchange][tradingsymbol]
        ...
        try:
            if is_new_order:
                ...
                Constants.logger.info(f"[NEW_ORDER_REQUEST] order_id {order_id} order {quantplay_order}")
