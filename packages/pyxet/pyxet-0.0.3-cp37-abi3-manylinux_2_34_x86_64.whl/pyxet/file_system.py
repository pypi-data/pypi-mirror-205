...
def open(file_url, user=None, token=None, **kwargs):
    """
...
class XetFS(fsspec.spec.AbstractFileSystem):
    ...
    def __init__(self, repo_url, branch=None, user=None, token=None):
        ...
        User and token are needed for private repositories.  They may be provided as 
        explicit arguments here, or they can be passed in with the URL by prefixing 
        `xet://<user>[:token]@xethub.com/`.  For example, 
        `xet://user1:mytokenxyz@xethub.com/data_user/data_repo/main/data/survey.csv` would
        access the file `data/survey.csv` on the branch `main` of the repo `data_user/data_repo` 
        with credentials `user=user1` and `token=mytokenxyz`. 
        ...
        ```
        import pyxet
        fs = pyxet.XetFS('xet://xethub.com/XetHub/Flickr30k/main/results.csv')
        fs = pyxet.XetFS('/XetHub/Flickr30k/main/')
        fs = pyxet.XetFS('https://xethub.com/XetHub/Flickr30k/', branch = 'main')
