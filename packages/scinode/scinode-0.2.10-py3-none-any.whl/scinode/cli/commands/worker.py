...
def start_worker(
    ...
    if ctx.obj.use_celery:
        ...
        print("Start worker {}, sleep: {}".format(worker.name, sleep))
        if foreground:
            worker.run()
        else:
            worker.start()
...
def start(ctx, name, pool, concurrency, foreground, sleep):
    ...
    scinode worker start localhost --concurrency=100 --pool eventlet
    """
...
def add(ctx, file):
    ...
    if file:
        ...
        name = click.prompt("Name of this worker:", default="localhost")
