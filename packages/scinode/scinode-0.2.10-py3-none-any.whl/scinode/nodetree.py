...
class NodeTree:
    ...
    def nodes_to_dict(self, short=False):
        ...
        for node in self.nodes:
            if short:
                ...
                nodes[node.name] = node.to_dict(worker_name=self.worker_name)
        return nodes
    ...
    def from_dict(cls, ntdata):
        ...
        for name, ndata in ntdata["nodes"].items():
            # load the module for node created by decorator
            if ndata["metadata"].get("register_path", False):
                load_module(ndata["metadata"]["register_path"])
            node = nt.nodes.new(
