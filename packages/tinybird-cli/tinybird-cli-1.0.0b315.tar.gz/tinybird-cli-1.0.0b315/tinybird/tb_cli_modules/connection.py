...
@connection_create.command(name="snowflake", short_help='Creates a Snowflake connection in the current workspace', hidden=True)
...
async def connection_create_snowflake(
    ...
    if connection_name and no_validate is False:
        if await client.get_connector(connection_name, 'snowflake') is not None:
            raise CLIConnectionException(FeedbackManager.info_conection_already_exists(name=connection_name))
    else:
        while not connection_name:
            ...
            if no_validate is False:
                if await client.get_connector(connection_name, 'snowflake') is not None:
                    click.echo(FeedbackManager.info_conection_already_exists(name=connection_name))
                    connection_name = None
    assert isinstance(connection_name, str)

    if show_instructions:
        instructions = await client.get_snowflake_integration_query(role)
        if instructions:
