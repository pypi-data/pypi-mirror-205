...
class TinyB(object):
    ...
    async def populate_node(self, pipe_name: str, node_name: str, populate_subset: bool = False, populate_condition: Optional[str] = None, truncate: bool = True, unlink_on_populate_error: bool = False):
        params: Dict[str, Any] = {
    ...
    async def get_connector(
        ...
        return next((c for c in await self.connections(connector=service) if c['name'] == name), None)

    async def get_snowflake_integration_query(self, role: str) -> Optional[Dict[str, Any]]:
        try:
            params = {'role': role}
