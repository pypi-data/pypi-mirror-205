...
class GLRenderer:
    ...
    def _stats(self, rgb, idx):
        if self.verbose:
            self._vprint(f"Statistics for image #{idx + 1} [{self.files.filespecs[idx]}]:")
            self._fit_gamut(rgb)
    ...
    def _fit_gamut(self, rgb):
        ...
        t0 = time.time()
        inf_mask = ~np.isfinite(rgb)  # replace infs with maximum finite value
        finite_max = np.max(rgb[~inf_mask])
        rgb[inf_mask] = finite_max
        max_rgb = np.max(rgb, axis=-1, keepdims=True)  # (H, W, 1)
        t1 = time.time()
        above_thr = np.abs(max_rgb) > 0.5 * np.mean(max_rgb)
        above_eps = np.abs(max_rgb) > 1e-6
        t2 = time.time()
        zeros = np.zeros_like(rgb)  # (H, W, 3)
        t3 = time.time()
        zero = np.isclose(max_rgb, 0.0)
        valid = above_thr & above_eps & ~zero
        ...
        e3 = (t4 - t3) * 1000
        e4 = (t5 - t4) * 1000
        pct_valid = 100 * np.sum(valid) / valid.size
        self._vprint(f"Gamut bounds based on {pct_valid:.2f}% of pixels: {max_dist}")
