...
class SWSReceiver(FLReceiver):
    ...
    async def establish_websocket_connection(self, loop, stop):
        ...
        while not self.stopped:
            try:
                ...
                pRecvEvt.set()
                logger.error(str(ex))
                raise Exception("No valid certificate detected. Please replace the default certificate with your own "
    ...
    def handle_message(self, recv_msg):
        ...
        logger.info("Handler for message type {} is {}".format(message_type,handler))
