...
class AbstractAutoAISubsamplingIteratorBatched:
    ...
    def iterate_through_dataset_and_validate(self, return_data_as_iterator, enable_sampling, sample_size_limit, sampling_type,
                                             number_of_batch_rows, return_subsampling_stats, experiment_metadata, total_size_limit):
    ...
    def iterate_through_dataset(self, iterator, number_of_batch_rows, return_subsampling_stats):
        ...
        whole_data_size = 0
        whole_byte_size = 0
        prev_whole_byte_size = 0

        indexes = []
        last_unique_indexes_count = 0
        ...
        for data in iterator:
            ...
            if whole_data is None:
                ...
                whole_data = pd.concat([whole_data, data.copy()])
            whole_data_size += len(data)
            print('Batch size:', len(data))
            if number_of_batch_rows:
                self.assertTrue(len(data) <= number_of_batch_rows,
                                f'Batch size bigger than number_of_batch_rows ({len(data)} > {number_of_batch_rows})')
            # print(data)
            i = data[self.index_columns[0]].tolist()
            # print('Batch indexes:', len(i), '/', len(set(i)))
            last_unique_indexes_count = len(set(indexes))
            indexes += i
            # print('Distinct indexes increase:', len(set(indexes)) - last_unique_indexes_count)

            prev_whole_byte_size = whole_byte_size
            whole_byte_size += sys.getsizeof(data)
            indexes = i

        return whole_data, whole_data_size, whole_byte_size, prev_whole_byte_size, len(indexes), len(set(indexes))
    ...
    def validate_returned_data(self, data, return_subsampling_stats):
        ...
        return data, len(data), sys.getsizeof(data), 0, len(indexes), len(set(indexes))

    def is_non_iterator_available(self):
        raise NotImplemented()

    @timeout_decorator.timeout(TIMEOUT)
    def _test_read_func(self, return_data_as_iterator, enable_sampling, sample_size_limit, sampling_type, number_of_batch_rows,
                        return_subsampling_stats, experiment_metadata, total_size_limit):
        print(f"\n\nIterator: {return_data_as_iterator}")
        print(f"Sampling: {enable_sampling} {sampling_type}({sample_size_limit/(1024*1024) if sample_size_limit else '-'} MB)")
        ...
        try:
            ...
            data, number_of_rows, byte_size, prev_byte_size, indexes_num, unique_indexes_num = self.iterate_through_dataset_and_validate(return_data_as_iterator, enable_sampling, sample_size_limit, sampling_type,
                                                      number_of_batch_rows, return_subsampling_stats,
                                                      experiment_metadata, total_size_limit)

            print('Rows number: ', number_of_rows, '/', self.data_rows_count[0])
            print('Byte size: ', byte_size/(1024*1024), "MB")
            print('Prev byte size: ', prev_byte_size / (1024 * 1024), "MB")
            print('Indexes number:', indexes_num, '/', unique_indexes_num)
            # print(data[self.index_columns[0]].tolist())

            if sample_size_limit:
                self.assertTrue(byte_size <= 1.1*sample_size_limit, f'Byte size much bigger than sample_size_limit ({byte_size} > {sample_size_limit})')
            else:
                self.assertEqual(number_of_rows, self.data_rows_count[0], f'Not complete data set was downloaded ({number_of_rows} / {self.data_rows_count[0]})')
    ...
    def test_04_read(self):
        for func in [self._test_read_func]:
            for return_data_as_iterator in [True, False] if self.is_non_iterator_available() else [True]:
                for enable_sampling in [True, False] if return_data_as_iterator else [False]:
                    for sampling_type in [None] if enable_sampling else [None]:
                        for total_size_limit in [None, 2 * 1024 * 1024, 25 * 1024 * 1024] if enable_sampling else [None]:
                            for number_of_batch_rows in [None, 1000]:
