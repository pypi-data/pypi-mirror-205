...
def check_python_framework_version(client_reqs):
    ...
    print("Detected Client Python Version: {}".format(py_version))
    ...
    if client_system == "Darwin" and client_processor == "arm":
        try:
            ...
            print("Detected tensorflow-macos Version: {}".format(tensorflowmacos_version))
            ...
            print("{}, this may cause unexpected errors.".format(ex))
            print("The selected software spec requires tensorflow-macos=={}.".format(client_reqs['tensorflow_version']))
    else:
        try:
            tensorflow_version = get_module_version(lib_name='tensorflow')
            print("Detected tensorflow Version: {}".format(tensorflow_version))
            if not tensorflow_version.startswith(client_reqs['tensorflow_version']):
                raise Exception("Incompatible tensorflow version found")
        except Exception as ex:
            print("{}, this may cause unexpected errors.".format(ex))
            print("The selected software spec requires tensorflow=={}.".format(client_reqs['tensorflow_version']) + install_msg)

    try:
        torch_version = get_module_version(lib_name='torch')
        print("Detected torch Version: {}".format(torch_version))
        if not torch_version.startswith(client_reqs['torch_version']):
            raise Exception("Incompatible torch version found")
    except Exception as ex:
        print("{}, this may cause unexpected errors.".format(ex))
        print("The selected software spec requires torch=={}.".format(client_reqs['torch_version']) + install_msg)
    
    try:
        scikitlearn_version = get_module_version(lib_name='scikit-learn')
        print("Detected scikit-learn Version: {}".format(scikitlearn_version))
        ...
        print("{}, this may cause unexpected errors.".format(ex))
        print("The selected software spec requires scikit-learn=={}.".format(client_reqs['scikitlearn_version']) + install_msg)


def choose_software_version(software_spec):
    print("Aggregator Software Spec: {}".format(software_spec))
...
class Party(WMLResource):
    ...
    def __init__(self, client=None, **kwargs):
        ...
        print("Detected platform environment: {}".format(platform_env))
