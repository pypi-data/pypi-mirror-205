...
def p_next_gen_const(p, n, coeff_dict):
    pr = 9  # Precision
    p = rounding(p)
    m = len(p)
...
def p_next_gen_stochastic(p, n, distribution_dict):
    pr = 9  # Precision
    p = rounding(p)
    m = len(p)
    ...
    for x in states:  # For each role model state
        ...
        # Find d(x)
        if distribution_dict[x_str][0] == "uniform":
            uni_min = distribution_dict[x_str][1]
            uni_max = distribution_dict[x_str][2]
            d = np.random.uniform(uni_min, uni_max, 1)[0]
...
def frqs(generations, initial_p_vec, n, conform_dict):
    ...
    key = next(iter(conform_dict))
    if type(conform_dict[key]) == float:
        for i in range(generations):
            p = p_next_gen_const(p, n, conform_dict)
            for j in range(m):
                all_p_over_time[j].append(p[j])
    else:
        for i in range(generations):
            p = p_next_gen_stochastic(p, n, conform_dict)
            for j in range(m):
                all_p_over_time[j].append(p[j])
