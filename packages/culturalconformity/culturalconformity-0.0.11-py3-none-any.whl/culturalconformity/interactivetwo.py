...
def set_bounds_uniform(d_min, d_max, x_vec):
    pr = 9  # Precision
    msg = "For the state x = " + str(x_vec) + ", choose a minimum \nvalue for d(x) that is not below " + str(
        d_min) + "\nor above " + str(d_max) + ", or enter '.' to use " + str(d_min)
    uni_min = input(msg)
    if uni_min == '.':
        uni_min = d_min
    else:
        uni_min = float(uni_min)  # Later, figure out what to do if they entered a letter

    while (round(uni_min, pr) < round(d_min, pr)) or (round(uni_min, pr) > round(d_max, pr)):
        uni_min = input("Invalid answer; try again. " + msg)
        if uni_min == '.':
            uni_min = d_min
        else:
            uni_min = float(uni_min)  # Later, figure out what to do if they entered a letter

    msg = "For the state x = " + str(x_vec) + ", choose a maximum \nvalue for d(x) that is not above " + str(
        d_max) + "\nor below " + str(uni_min) + ", or enter '.' to use " + str(d_max)
    uni_max = input(msg)
    if uni_max == '.':
        uni_max = d_max
    else:
        uni_max = float(uni_max)  # Later, figure out what to do if they entered a letter

    while (round(uni_max, pr) > round(d_max, pr)) or (round(uni_max, pr) < round(uni_min, pr)):
        uni_max = input("Invalid answer; try again. " + msg)
        if uni_max == '.':
            uni_max = d_max
        else:
            uni_max = float(uni_max)  # Later, figure out what to do if they entered a letter

    return [uni_min, uni_max]


def choose_conform_dist(n, m, dist_type):
    ...
    for x_vec in states:  # For each role model state
        ...
        if x_str not in previous_states:
            if round(denom_high, pr) == 0 or round(denom_low, pr) == 0:
                d = 0
                previous_states[x_str] = ["uniform", 0, 0]
            else:
                d_min = -1 * denom_high
                ...
                if dist_type == 'u':  # Uniform
                    uni_bounds = set_bounds_uniform(d_min, d_max, x_vec)
                    uni_min = uni_bounds[0]
                    uni_max = uni_bounds[1]

                previous_states[x_str] = ["uniform", uni_min, uni_max]

                # Make sure to repeat the line previous_states[x_str] = here for each dist
...
def interactivetwo():
    ...
    if intro == '?':
        print("This function will allow you to specify conformity coefficients, initial variant frequencies,\n"
              ...
              + "results will go into the variable called x. See github.com/kaleda/culturalconformity for\n"
              + "how to use the output of each option in the other functions. Let's get started!")

    msg = "Enter 1 for the discrete, constant conformity model. \nEnter 2 for the discrete, stochastic conformity model."
    answer = input(msg)
    while answer not in ['1', '2']:
        answer = input("Sorry, that was not a valid option. " + msg)

    if answer == '1':  # Discrete, constant conformity model
        ...
        msg = "Enter 1 to specify distributions of conformity coefficients. \nEnter 2 to make your list of initial frequencies. \nEnter 9 to quit. "
        answer = float(input("What would you like to do? \n" + msg))
        while not answer.is_integer() or answer < 1 or answer > 9:
            answer = float(input("Sorry, that was not a valid number. \n" + msg))
        ...
        if answer == 1 or answer == 2:
            m = choose_m()
        if answer == 1:  # Specify distributions of conformity coefficients
            n = choose_n()
            msg = "Enter 'u' for a uniform distribution, 'n' for a truncated normal distribution, or..."
            dist_type = input(msg)
            distribution_dict = choose_conform_dist(n, m, dist_type)
