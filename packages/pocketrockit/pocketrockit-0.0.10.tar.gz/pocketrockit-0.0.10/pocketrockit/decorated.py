...
from pocketrockit.engine import Env, Stage, error, run
...
def pattern_list_from(pattern_definition: Sequence[str | Sequence[str]]) -> Sequence[Sequence[int]]:
    """Turns various pattern definition styles into"""
    if isinstance(pattern_definition[0], str) and len(pattern_definition) == 1:
        # only one string given
        return [[e for line in p.split("|") for e in line.split()] for p in pattern_definition]
    if isinstance(pattern_definition[0], (list, tuple)):
        return [[e for line in p.split("|") for e in line.split()] for p in pattern_definition[0]]
...
def midiseq(
    *pattern: str | Sequence[str],
    ...
    pattern_list = pattern_list_from(pattern)
    if len(pattern_list[0]) != 16:
        raise RuntimeError(f"Does not evaluate to 16 notes: {pattern_list}")
    ...
    while True:
        ...
        start_loop = (reset_tick or 0) // sum(len(elements) for elements in pattern_list)
        skip = (reset_tick or 0) % sum(len(elements) for elements in pattern_list)

        for loop, tick, element, scale in (
            (_loop, _tick, _element, _scale)
            for _loop in count(start_loop)
            for _scale in (midi_scale(keys[_loop % len(keys)]),)
            for elements in (pattern_list[_loop % len(pattern_list)],)
...
def tests():
    ...
    scale = list(range(100, 108))
    print(resolve("i", scale))
    print(resolve("ii", scale))
    print(resolve("n1", scale))
    print(resolve("n2", scale))
    print(resolve("t1", scale))
    print(resolve("t2", scale))
    print(resolve("n1+", scale))

    for name, midi_tone in NOTES.items():
        assert resolve(name, scale) == midi_tone

    pattern_list = pattern_list_from("x x x x")
    pattern_list = pattern_list_from(["x x x x"])
    pattern_list = pattern_list_from("")
