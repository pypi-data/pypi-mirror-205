...
dl = [Doc(text=i, embedding=np.random.random((dims,)), embedding2=np.random.random((dims,))) for i in range(100)]
# create a Document Index
index = HnswDocumentIndex[Doc](work_dir='test_index11111')
index._hnsw_indices
# index your data
if not Path(index._work_dir).exists():
    index.index(dl)
