...
class atxServer(MongoClientTradeGateway):
    def __init__(self, config_path, endtime):
        MongoClientTradeGateway.__init__(self, config_path, endtime)

        self.load_tg_setting(config_path)
    ...
    def load_tg_setting(self, config_path):
        try:
            config_filename = os.path.join(config_path, 'atx_cicc_server_config.json')
    ...
    def req_buy_order_insert(self, obj):
        ...
        trade_param = f"basket_id={sid}"
        #client_name = "三方网关两融测试账户2".encode('utf-8')
        ...
        self.logger.info(f"[req_buy_order_insert] insert_atx_order_info (res){res} (msg){db_msg}")
                ...
                if self.trade_index == len(rtn_trade_table)-1 or len(rtn_trade_table) == 0:
                    ...
                    while rtn_trade_table[self.trade_index+1] is not rtn_trade_table.last_record:
                        ...
                        if record.OrdStatus in [2,4]: #only need filled/canceled
                            ...
                            self.logger.info(f"[monitor_trade_update] traded_not_finished (msg){record.Text}")
    ...
    def on_trade_msg(self, record):
        ...
        if _id in self.oid_to_local_ids[oid]:
            self.logger.warning(f"[on_trade_msg] duplicated_trade_msg (record){record}")
            return
        else:
        ...
        #time_struct = time.mktime(trade_time.timetuple())
    ...
    def update_position(self, traded_vol, order, sid , oid, side, trade_amt, exchange):
        ...
        if position == None and traded_vol > 0:
            ...
            ticker = order['ticker']
            cost = 0
            total_vol = td_trade_vol + actual_yd_pos
            if not total_vol == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--config_filename', dest= 'config_filepath', default= _config_filename)
