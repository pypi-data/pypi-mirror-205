...
def xla_call_module(args, version, module, Sout, Tout, function_list, dim_args_spec=[], platforms=[], name=None):
  ...
  Args:
    ...
    function_list: A list of functions decorated with @Defun.
  ...
  if tld.is_eager:
    ...
    try:
      _result = _dispatcher_for_xla_call_module(
          (args, version, module, Sout, Tout, function_list, dim_args_spec,
          platforms, name,), None)
      if _result is not NotImplemented:
        return _result
  ...
  try:
    _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "XlaCallModule", args=args, version=version, module=module, Sout=Sout,
                         Tout=Tout, function_list=function_list,
                         dim_args_spec=dim_args_spec, platforms=platforms,
                         name=name)
  except (TypeError, ValueError):
    _result = _dispatch.dispatch(
...
def xla_call_module_eager_fallback(args, version, module, Sout, Tout, function_list, dim_args_spec, platforms, name, ctx):
  version = _execute.make_int(version, "version")
  module = _execute.make_str(module, "module")
  if not isinstance(Sout, (list, tuple)):
    raise TypeError(
