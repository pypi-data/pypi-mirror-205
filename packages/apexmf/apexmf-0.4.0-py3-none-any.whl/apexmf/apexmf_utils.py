""" APEXMF support functions: 02/09/2021 created by Seonggyu Park
...
def cvt_array_fr_mmuse(wd, nrows, ncols, filenam):
    ...
    a = np.reshape(df.values, (nrows, ncols))
    f = StringIO()
    np.savetxt("out.out", a, fmt='%.12e')
    f.close()


def cvt_array_fr_bas(wd, nrows, ncols, filenam, outfile=None):
    os.chdir(wd)
    with open(os.path.join(wd, filenam), "r") as f:
        lines = f.read().splitlines()
    df = []
    for i in lines:
        for x in i.split():
            df.append(float(x))
    a = np.reshape(df, (nrows, ncols))
    # f = StringIO()
    if outfile is None:
        outfile = "out.out"
    np.savetxt(outfile, a, fmt='%.7e', delimiter='\t')
    # f.close()
    print('done')
...
if __name__ == '__main__':
    ...
    wd = 'D:/Projects/Watersheds/White/Analysis/MODFLOWs/mf_211029'
    # cvt_array_fr_bas(wd, 70, 131,"initialhead_raw.txt", "inhdarray.txt")
    delete_duplicate_river_grids(wd, 'wt_1500.riv')
