""" APEX-MODFLOW scenario support utilities: 02/10/2021 created by Seonggyu Park
...
def get_weather_folder_lists(wd):
    os.chdir(wd)
    weather_fds = [name for name in os.listdir(".") if os.path.isdir(name)]
    full_paths = [os.path.abspath(name) for name in os.listdir(".") if os.path.isdir(name)]
    print(weather_fds)
    return weather_fds, full_paths
...
def execute_scenarios(
    ...
    for i, mp in zip(model_nams, model_paths):
        for j, wp in zip(weather_nams, weather_paths):
            ...
            cwd = new_model_dir
            os.chdir(cwd)
            os.system("start cmd /k apexmf.exe")
...
def execute_grid_pp_models(
            model_wd,
            model_nam, 
            weathers_wd,
            scn_model_wd=None, model_lists=None, weather_lists=None,
            copy_files_fr_model=None,
            copy_files_fr_weather=None,
            reuse_models=None,
            ):
    ...
    if scn_model_wd is None:
        scn_model_wd = '..'


    for j, wp in zip(weather_nams, weather_paths):
        new_model_dir = os.path.join(scn_model_wd, "{}_{}".format(model_nam, j))
        # try:
        #     shutil.rmtree(new_model_dir, onerror=_remove_readonly)#, onerror=del_rw)
        # except Exception as e:
        #     raise Exception("unable to remove existing worker dir:" + \
        ...
        if os.path.exists(new_model_dir) and reuse_models is None:
            try:
                ...
                raise Exception("unable to remove existing worker dir:" + \
                                "{0}\n{1}".format(new_model_dir,str(e)))
            print("  Creating new model: {}_{} ... processing".format(model_nam, j))
            try:
                shutil.copytree(model_wd, new_model_dir)
            except Exception as e:
                raise Exception("unable to copy files from worker dir: " + \
                                "{0} to new worker dir: {1}\n{2}".format(model_wd, new_model_dir,str(e)))
            print("  Creating new model: {}_{} ... passed".format(model_nam, j)) 
        elif os.path.exists(new_model_dir) and reuse_models is True:
            try:
                os.chdir(model_wd)
                # shutil.copyfile('file.cio', os.path.join(new_model_dir, 'file.cio'))
                ...
                raise Exception("unable to copy *.pst from main worker: " + \
                                "{0} to new worker dir: {1}\n{2}".format(model_wd, new_model_dir,str(e)))
            print("  Using existing model: {}_{} ... passed".format(model_nam, j)) 
        else:
            try:
                shutil.copytree(model_wd, new_model_dir)
                print("  Copying model: {}_{} ... passed".format(model_nam, j)) 
                weather_files = os.listdir(wp)
                for wf in weather_files:
                    shutil.copyfile(os.path.join(wp, wf), os.path.join(new_model_dir, wf))
                print("  Copying weather inputs: {}_{} ... passed".format(model_nam, j)) 
            except Exception as e:
                raise Exception("unable to copy files from worker dir: " + \
                                "{0} to new worker dir: {1}\n{2}".format(model_wd, new_model_dir,str(e)))
            print("  Creating new model: {}_{} ... passed".format(model_nam, j)) 
        if copy_files_fr_model is not None and reuse_models is True:
            try:
                ...
                raise Exception("unable to copy {0} from model dir: " + \
                                "{1} to new model dir: {2}\n{3}".format(f, model_wd, new_model_dir,str(e)))      
        if copy_files_fr_weather is not None and reuse_models is True:
            try:
                os.chdir(wp)
                for f in copy_files_fr_weather:
...
if __name__ == '__main__':
    model_wd = "D:\\Projects\\manuscript_CRB_PCP\\Data\\calibrated_models\\animas"
    model_nam = "animas"
    weathers_wd = "D:\\Projects\\manuscript_CRB_PCP\\Data\\pcp_products\\animas"
    gridpp_wd = "D:\\Projects\\manuscript_CRB_PCP\\Analysis\\gridpp_models"
    # get_weather_folder_lists(wd)

    execute_grid_pp_models(model_wd, model_nam, weathers_wd, scn_model_wd=gridpp_wd, reuse_models=True)
