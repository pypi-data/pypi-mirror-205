...
class KafkaStreamingClientExtensions(object):
    ...
    _interop_func = InteropUtils.get_function("kafkastreamingclientextensions_gettopicconsumer")
    _interop_func.restype = c_void_p
    _interop_func.argtypes = [c_void_p, c_void_p, c_void_p, ctypes.c_int, ctypes.c_int]
    @staticmethod
    def GetTopicConsumer(client: c_void_p, topic: str, consumerGroup: str = None, commitMode: CommitMode = CommitMode.Automatic, autoOffset: AutoOffsetReset = AutoOffsetReset.Latest) -> c_void_p:
        """
        Parameters
        ----------
        ...
        result = InteropUtils.invoke("kafkastreamingclientextensions_gettopicconsumer", client, topic_ptr, consumerGroup_ptr, commitMode.value, autoOffset.value)
