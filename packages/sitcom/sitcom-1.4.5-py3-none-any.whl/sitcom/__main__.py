...
class Ui_MainWindow(object):     
    ...
    def Mov(self):
      ...
      def animate(i):
        if self.comboBox_scale.currentText()=='Linear':
          vmin,vmax=interval.get_limits(self.ima1[i])
          self.i1=ax.imshow(cv2.medianBlur(self.ima[i],5),extent=[self.x[0],self.x[-1],self.y[0],self.y[-1]],cmap=self.colorm,vmin=vmin,vmax=vmax)
          self.fram.append(cv2.medianBlur(self.ima[i],5))          
        elif self.comboBox_scale.currentText()=='Logarithmic':
      ...
      self.ani = animation.FuncAnimation(self.canvas.figure, animate,frames=len(self.ima1),interval=30)
      self.canvas.draw()
