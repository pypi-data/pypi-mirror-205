...
def do_upgrade(config_filename, attempt_fixes=False):
    ...
    if ci_cd in ("github-actions", "gitlab-ci"):
        print(
            ...
            f"   anymlops render -c {config_filename}\n"
        )
...
class UpgradeStep(ABC):
    ...
    def requires_anymlops_version_field(self):
    ...
    def upgrade_step(self, config, start_version, config_filename, *args, **kwargs):
        ...
        config_filename may be useful to understand the file path for anymlops-config.yaml, for example
        to output another file in the same location.

        The standard body here will take care of setting anymlops_version and also updating the image tags.
        ...
        if self.requires_anymlops_version_field():
            print(f"Setting anymlops_version to {self.version}")
            config["anymlops_version"] = self.version
    ...
    def _version_specific_upgrade(
        ...
        if (
            ...
            if not kwargs.get("attempt_fixes", False):
                raise ValueError(
                    f"{customauth_warning}\n\nRun `anymlops upgrade --attempt-fixes` to switch to basic Keycloak authentication instead."
                )
            else:
        ...
        realm_import_filename = config_filename.parent / "anymlops-users-import.json"

        realm = {"id": "anymlops", "realm": "anymlops"}
        realm["users"] = [
            {
                "username": k,
                "enabled": True,
        ...
        # project was never needed in Azure - it remained as PLACEHOLDER in earlier anymlops inits!
        if "azure" in config:
            if "project" in config["azure"]:
                del config["azure"]["project"]

        # "oauth_callback_url" and "scope" not required in anymlops-config.yaml
