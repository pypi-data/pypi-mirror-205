...
from anymlops.utils import namestr_regex
    ...
    def validate_guest_accelerators(cls, v):
        ...
        for i in v:
            assertion_error_message = """
                ...
                # TODO: replace with anymlops.dev new URL
                https://docs.anymlops.dev/en/stable/source/admin_guide/gpu.html?#add-gpu-node-group
            """
            try:
...
class Main(Base):
    ...
    # If the anymlops_version in the schema is old
    # we must tell the user to first run anymlops upgrade
    @validator("anymlops_version", pre=True, always=True)
    def check_default(cls, v):
        """
        Always called even if anymlops_version is not supplied at all (so defaults to ''). That way we can give a more helpful error message.
        """
        if not cls.is_version_accepted(v):
            if v == "":
                v = "not supplied"
            raise ValueError(
                f"anymlops_version in the config file must be equivalent to {__version__} to be processed by this version of anymlops (your config file version is {v})."
                " Install a different version of anymlops or run anymlops upgrade to ensure your config file is compatible."
            )
        return v
