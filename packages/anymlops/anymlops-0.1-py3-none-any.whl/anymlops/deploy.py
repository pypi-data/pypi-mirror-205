...
from anymlops.provider import terraform
from anymlops.provider.dns.cloudflare import update_record
from anymlops.stages import checks, input_vars, state_imports
...
def provision_08_anymlops_tf_extensions(stage_outputs, config, disable_checks=False):
    directory = "stages/08-anymlops-tf-extensions"

    stage_outputs[directory] = terraform.deploy(
        directory=directory,
        input_vars=input_vars.stage_08_anymlops_tf_extensions(stage_outputs, config),
    )
...
def deploy_configuration(
    ...
    if config.get("prevent_deploy", False):
        # Note if we used the Pydantic model properly, we might get that anymlops_config.prevent_deploy always exists but defaults to False
        raise ValueError(
            textwrap.dedent(
                """
    ...
    logger.info(f'All anymlops endpoints will be under https://{config["domain"]}')
