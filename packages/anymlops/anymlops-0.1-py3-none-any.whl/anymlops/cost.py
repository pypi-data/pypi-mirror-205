...
INFRACOST_NOTE = """
Anymlops rely upon node-pools which is a usage resource but doesn't get captured in the above report. A general node-pool
...
def infracost_report(path, dashboard, file, currency_code, compare):
    ...
    args:
        path: path to the anymlops stages directory.
    """
    console = Console()
    ...
    if _check_infracost() and _check_infracost_api_key():
        ...
        if not os.path.exists(path):
            ...
            if data:
                ...
                logger.error(
                    "No data was generated. Please check your Anymlops configuration and generated stages."
                )
    else:
