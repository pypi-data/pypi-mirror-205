...
from anymlops.deprecate import DEPRECATED_FILE_PATHS
from anymlops.provider.cicd.github import gen_anymlops_linter, gen_anymlops_ops
from anymlops.provider.cicd.gitlab import gen_gitlab_ci
from anymlops.stages import tf_objects


def render_template(output_directory, config_filename, force=False, dry_run=False):
    # get directory for anymlops templates
    import anymlops

    template_directory = pathlib.Path(anymlops.__file__).parent / "template"
    ...
    config["anymlops_config_yaml_path"] = str(config_filename.absolute())
    ...
    directories = [
        ...
        "stages/08-anymlops-tf-extensions",
    ]
    if (
...
def render_contents(config: Dict):
    ...
    contents = {
        ...
        **tf_objects.stage_08_anymlops_tf_extensions(config),
    }
...
def gen_cicd(config):
    ...
    GiHub-Actions - anymlops/providers/cicd/github.py
    GitLab-CI - anymlops/providers/cicd/gitlab.py
    """
    cicd_files = {}
    ...
    if cicd_provider == "github-actions":
        ...
        cicd_files[gha_dir + "anymlops-ops.yaml"] = gen_anymlops_ops(config)
        cicd_files[gha_dir + "anymlops-linter.yaml"] = gen_anymlops_linter(config)
...
def set_env_vars_in_config(config):
    ...
    for idx in private_entries:
        set_anymlops_secret(config, idx)
...
def set_anymlops_secret(config, idx):
    placeholder = get_sub_config(config, idx)
    secret_var = get_anymlops_secret(placeholder)
