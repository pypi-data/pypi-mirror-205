...
from anymlops.stages import input_vars, state_imports
...
def gather_stage_outputs(config):
    ...
    stage_outputs["stages/08-anymlops-tf-extensions"] = _terraform_init_output(
...
def destroy_stages(stage_outputs, config):
    ...
    with kubernetes_provider_context(
        ...
        with keycloak_provider_context(
            ...
            status["stages/08-anymlops-tf-extensions"] = _terraform_destroy(
                directory="stages/08-anymlops-tf-extensions",
                input_vars=input_vars.stage_08_anymlops_tf_extensions(
                    stage_outputs, config
                ),
                ignore_errors=True,
            )
...
def destroy_configuration(config):
    ...
    you can use 'anymlops deploy' to re-install infrastructure using same config file\n"""
    )
