...
class VisFlagger(object):
    ...
    def plot_amp_phase(self, baseline_idx, delay_max=4, stokes='I', window_fct='boxcar', hpass_filter=True, hpass_n_chan=20):
        ...
        ax3.axhline(self.horizon[i], c='black')
        ax4.imshow(np.abs(ps), aspect='auto', extent=extent, norm=mpl.colors.LogNorm(vmin=10, vmax=1e3))
    ...
    def flag_freqs_band(self, freqs_match, freqs_filter, n_sigma=5, do_plot=False, stokes='I'):
    ...
    def flag_delay_baseline_outliers(self, n_sigma=5, window_fct='boxcar', wedge_factor=1.2, delay_max=4,
        ...
        return new_flags, masked_ratio_med_abs, fig
...
def apply_vis_filter(ms_file, config_file, plot_dir=None, dry_run=False):
    ...
    for band, s in c_s.flag_freqs_band.items():
        f_f = s['freqs_filter']
        f_m = s['freqs_match']
        new_flags, masked_score, fig = vis_flagger.flag_freqs_band(f_m, f_f, n_sigma=s['n_sigma'], stokes=s['stokes'], do_plot=do_plot)
    ...
    new_flags, masked_ratio_med_abs, fig = vis_flagger.flag_delay_baseline_outliers(do_plot=do_plot, n_sigma=s.n_sigma, 
                                                                                    window_fct=s.window_fct, wedge_factor=s.wedge_factor,
    ...
    if do_plot:
        ...
        for i in np.where(masked_ratio_med_abs.mask & (masked_ratio_med_abs.mask < 1e10))[0]:
            fig = vis_flagger.plot_amp_phase(i, stokes='I', window_fct='hann')
