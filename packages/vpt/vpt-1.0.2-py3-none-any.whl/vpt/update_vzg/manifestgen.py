...
class ManifestGenerator:
    ...
    version = ''.join([str(version_d['Major']), '.', str(version_d['Minor']),
                       '.', str(version_d['Patch'])])
    ...
    def _set_spatial_bbox(self, textureWidth, textureHeight, transformMatrix):
        self._bBoxInfoDict['MinX'] = \
            np.float64(-transformMatrix[0][2] / transformMatrix[0][0])
        self._bBoxInfoDict['MinY'] = \
            np.float64(-transformMatrix[1][2] / transformMatrix[1][1])
        self._bBoxInfoDict['MaxX'] = \
            (textureWidth - transformMatrix[0][2]) / transformMatrix[0][0]
        self._bBoxInfoDict['MaxY'] = \
            (textureHeight - transformMatrix[1][2]) / transformMatrix[1][1]
    ...
    def create_json_manifest(self):
        ...
        self.generalInfo['VoxelGridSize'] = {
            'Width': self._gridSize[0],
            'Height': self._gridSize[1]
        }
