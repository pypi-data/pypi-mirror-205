...
def split_args(parsed: argparse.Namespace) -> Tuple[Dict, argparse.Namespace]:
    ...
    del parsed.aws_profile_name, parsed.aws_access_key, \
        parsed.aws_secret_key, parsed.gcs_service_account_key
...
def main(parsed: argparse.Namespace):
    try:
        ...
        log.exception(f"vpt has encountered a runtime error ({err}) and will now exit")
        print(
            "\n"
            "vpt has encountered a runtime error and will now exit.\n"
            "\n"
            "Details:\n"
