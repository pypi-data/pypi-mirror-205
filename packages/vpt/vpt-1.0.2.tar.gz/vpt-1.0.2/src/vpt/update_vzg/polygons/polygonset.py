...
class PolygonSet:
    ...
    def points_transform(self,
        ...
        if not all([1 > ((p[0] - points[0][0]) * normScale[0] * 0.5 + 0.5) > 0 or
                    1 > ((p[1] - points[0][1]) * normScale[1] * 0.5 + 0.5) > 0 for p in points[1:]]):
            return False

        if self._poly_star_check(points.copy(), gridSize, expBbox, cellSize,
                                 normScale):
            return True
    ...
    def _poly_star_check(self, points, gridSize, expBbox, cellSize, normScale):
        ...
        if self._define_star_polygon(pointsMicronDelta):
            self._make_star_packed_lods(
                points, expBbox, cellSize, normScale, gridSize)
            return True

        return False

    def _make_star_packed_lods(self, points, expBbox,
                               cellSize, normScale, gridSize):
        self._packedPolygons.append(PackedStarPolygon(
            points, gridSize).get_packed_bytes(self._cellId))
        ...
        self._fan_packed_lod(points, LodLevel.Min, 9,
                             expBbox, cellSize, normScale)

    def _make_fan_packed_lods(self, points, polys, expBbox,
                              cellSize, normScale):
        fanPoly = PackedFanPolygon(points, LodLevel.Max)
        self._packedPolygons.append((3, fanPoly.get_packed_bytes(
            polys, cellSize, self._cellId, expBbox)))

        self._fan_packed_lod(points, LodLevel.Middle, 17,
                             expBbox, cellSize, normScale)

        self._fan_packed_lod(points, LodLevel.Min, 9,
                             expBbox, cellSize, normScale)

    def _fan_packed_lod(self,
                        points,
                        lodLevel,
                        filterCount: int,
                        expBbox,
        ...
        if not result:
            ...
            self._packedPolygons.append(
                (polyType, fanPoly.get_packed_bytes(polys, cellSize,
                                                    self._cellId, expBbox)))

    @staticmethod
    def _points_pack_fan_reduction(points, normScale) -> np.array:
        p0 = points[0]
        packFactor12 = 1 << 12
    ...
    @staticmethod
    def _points_pack_star_reduction(points, gridSize) -> np.array:
        ...
        for pIdx in range(0, len(points)):
            ...
            xDelta = np.uint32((0.5 * delta[0] * gridSize[0] + 0.5) *
                               packFactor12)
            yDelta = np.uint32((0.5 * delta[1] * gridSize[1] + 0.5) *
                               packFactor12)
    ...
    def _thinning(self, points: np.ndarray):
        ...
        for coords in iter_coords:
            ...
            thinned_bool_filter_arr.append(
                not (x_curr_number == x_prev_number
                     and y_curr_number == y_prev_number))
        ...
        thinned_bool_filter_arr[0] = (not (
                x_curr_number == x_prev_number
                and y_curr_number == y_prev_number))
    ...
    def _vertex_level_reduce(self, points, pointsThreshold) -> np.ndarray:
        ...
        while vertices_count > pointsThreshold:
            first_not_reduced_ind: int = self._find_next_not_reduced_vertex(
                true_vertices_l, 0)
            second_not_reduced_ind: int = self._find_next_not_reduced_vertex(
                true_vertices_l, first_not_reduced_ind + 1)
            dist_first_v, dist_second_v \
                = first_not_reduced_ind, second_not_reduced_ind
            min_dist_sqr = 1000

            while second_not_reduced_ind < primary_vertices_count:
                dist = l2_dist_sqr(
                    coords[first_not_reduced_ind][0] -
                    coords[second_not_reduced_ind][0],
                    coords[first_not_reduced_ind][1] -
                    coords[second_not_reduced_ind][1],
                )
                ...
                first_not_reduced_ind = second_not_reduced_ind
                second_not_reduced_ind = self._find_next_not_reduced_vertex(
                    true_vertices_l, first_not_reduced_ind + 1)
            ...
            coords[dist_second_v][0] = (coords[dist_first_v][0] +
                                        coords[dist_second_v][0]) / 2
            coords[dist_second_v][1] = (coords[dist_first_v][1] +
                                        coords[dist_second_v][1]) / 2
            vertices_count -= 1
    ...
    def _define_star_polygon(points) -> bool:
        ...
        center = np.asarray((packFactor12 / 2, packFactor12 / 2),
                            dtype=np.float32)
    ...
    @staticmethod
    def _filter_points_by_indices(points: np.ndarray, indices: list)\
            -> np.ndarray:
