...
async def switchs_settings_param(data: dict):
    switchs_dict = {
...
async def add_equipment_sub_directories(data: dict):
    ...
    if BackupShell.check_ssh_connection(username=data['backup_username'], host=data['backup_host'], port=data['backup_port']):
        BackupShell.backup()
        return True
    return False
