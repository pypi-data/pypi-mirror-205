			   ...
			   vector<result_t> &results4) 
{
  ...
  try{
		...
		{
			throw std::exception("Error in false_discovery_rate_compute module!");
		}


		if(rets1.empty()) 
		{
			throw std::exception("Result_1 is empty!");
		}
		else{
		...
		if(rets2.empty()) 
		{
			throw std::exception("Result_2 is empty!");
		}
		else{
		...
		if(rets3.empty()) 
		{
			throw std::exception("Result_3 is empty!");
		}
		else{
			size_t i, j;
		...
		if(rets4.empty()) 
		{
			throw std::exception("Result_4 is empty!");
		}
		else{
			size_t i, j; 
...
class FdrCalculator {
    ...
    std::vector<result_t> results1, results2, results3, results4;
		...
		FdrCalculator(const std::vector<std::string>& args) : argv(args) {
			...
			for (int i = 0; i < argv.size() - 1; i++) {
				Tree tree;
				Tree polytree;
				if (!tree.load(treeFiles[i])){return;} else{
...
PYBIND11_MODULE(_fdrcpp, m) {
    ...
    py::class_<FdrCalculator>(m, "FdrCalculator")
		...
		m.def("create_calculator", [](const py::list& args) {
        std::vector<std::string> argv;
        for (auto& arg : args) {
            argv.push_back(arg.cast<std::string>());
        }
        return new FdrCalculator(argv);
    });
}
