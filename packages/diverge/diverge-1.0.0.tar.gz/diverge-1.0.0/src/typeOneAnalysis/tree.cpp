...
void node_t::generate_profile(int * profile, double * branch, int total_leaves) {
	list<node_t *>::const_iterator j;
	node_t * middle = nullptr;
	int i = 0;
	for(j = _children.begin(); j != _children.end(); j++) {
		(*j)->generate_profile(profile, branch, total_leaves); 
#ifdef DEBUG_DIVERGE
		cout << "ID = " << _id << " label = " << _label << "    child = " << (*j)->_id << "\n";
#endif
		if(i == 1) 
		  middle = *j;
		i++;
	...
	if(n_children() == 0) {// no children, it is a leaf
	  ...
	  return; //FIX ME: error, should print some error mesg here
	}
#ifdef DEBUG_DIVERGE
	cout << "ID = " << _id << " label = " << _label << " length = " << _length << "\n";
#endif
	return;
}
...
bool
Tree::load(const string filename) {
  FILE *file = fopen(filename.c_str(), "r");
  if(!file) return false;

  string str;
  if(!get_entire_line(file, str)) {
    fclose(file);
    return false;
  }
