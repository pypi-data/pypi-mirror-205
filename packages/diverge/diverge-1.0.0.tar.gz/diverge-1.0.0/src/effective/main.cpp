	...
	int& EffectiveNumberType1, int& EffectiveNumberType2)
{
	...
	try{
		...
		if (rets1.empty()){throw std::exception("Results_1 is empty!");}
		else{
		...
		if (rets2.empty()){throw std::exception("Results_2 is empty!");}
		else{
...
class EffCalculator {
    ...
    std::vector<result_t> results1,results2;
		...
		EffCalculator(const std::vector<std::string>& args) : argv(args) {
