						 ...
						 std::vector<result_t> &results2)
{
	...
	summary.clear();rets.clear();
	int i;
	try
	{
		if (!rvs_compute(trees, sequences, summary, rets))
		{
			throw std::exception("RVS method failed.Please recheck input parameters");
		}
		if (summary.empty())
		{
			throw std::exception("Summary is emtpy.");
		}
		else
		{
		...
		if (rets.empty())
		{
			...
			for (i = 0; i < ngroups; i++)
			{
				r_names.push_back(names[i] + "_Xk");
				r_names.push_back(names[i] + "_Rk");
			}
...
class RvsCalculator {
    ...
    std::vector<result_t> results;
		...
		RvsCalculator(const std::vector<std::string>& args) : argv(args) {
