...
process(const vector<Tree> &trees,
	...
	try{	
		if(!asymmetric_test_one_compute(trees, sequences,rets)) {
			// printf("Testing Asymmetric Type I method failed.  Please recheck input parameters.\n");
			throw std::exception("Error in asymmetric_test_one_compute module!");
		}
...
class AsymCalculator {
    ...
    std::vector<result_t> results;
		...
		AsymCalculator(const std::vector<std::string>& args) : argv(args) {
...
PYBIND11_MODULE(_asymcpp, m) {
    ...
    py::class_<AsymCalculator>(m, "AsymCalculator")
        ...
        return new AsymCalculator(argv);
    });
}
