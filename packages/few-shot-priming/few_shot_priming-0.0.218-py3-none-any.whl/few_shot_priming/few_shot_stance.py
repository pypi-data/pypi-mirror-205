...
from transformers import OPTConfig, OPTModel, BertForSequenceClassification, AutoModelWithLMHead
...
def load_splits(oversample=True):
    ...
    print(dataset["training"]["claims.stance"].value_counts())
    return dataset
...
def run_experiment_no_fune_tuning(params=None, offline=False, validate=True, splits=None):
    ...
    for i, record in test_dataset.iterrows():
        ...
        template_no_stance = f"While debating the topic {topic} the argument {sentence} was raised which {stance} the topic."
