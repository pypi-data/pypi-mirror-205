...
class ContextAttribute(Util.NoValEnum):
    ...
    PROTOCOL_NAME = CertoraArgument(
        ...
        help_msg="Add the protocol's name for easy filtering in the dashboard. "
                 "Default: taken from the name field in the package.json file, if it exists",
        argparse_args={
            'action': UniqueStore
        }
    )
    ...
    PROTOCOL_AUTHOR = CertoraArgument(
        ...
        help_msg="Add the protocol's author for easy filtering in the dashboard. "
                 "Default: taken from the author field in the package.json file, if it exists",
        argparse_args={
            'action': UniqueStore
        }
    )
    ...
    SOLC_ARGS = CertoraArgument(
        ...
        arg_status=ArgStatus.NEW,
        help_msg="send flags directly to the Solidity compiler",
        argparse_args={
            'action': UniqueStore,
        }
    )
    ...
    PROVER_VERSION = CertoraArgument(
        group=ArgGroups.ENV,
        arg_status=ArgStatus.NEW,
        help_msg="instruct the prover to use a version that is not the default",
        argparse_args={
            'action': UniqueStore
        }
    )
