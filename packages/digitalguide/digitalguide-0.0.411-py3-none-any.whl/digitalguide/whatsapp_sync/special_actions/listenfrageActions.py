...
from digitalguide.whatsapp.generateActions import Action

async def whatsapp_loop_list(client, update: WhatsAppUpdate, context,  key, value, doppelte_antwort):
...
async def whatsapp_loop_list_fertig(client, update: WhatsAppUpdate, context, key, answer_id_list, fertig_antwort):
    if set(answer_id_list).issubset(set(context[key])):
        Action(fertig_antwort)(client, update, context)

async def whatsapp_eval_list(client, update: WhatsAppUpdate, context, answer_id_name_list, poi, response_text):
