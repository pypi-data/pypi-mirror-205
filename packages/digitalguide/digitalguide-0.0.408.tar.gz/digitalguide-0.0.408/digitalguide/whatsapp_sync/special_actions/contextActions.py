...
async def whatsapp_default_name(client, update: WhatsAppUpdate, context):
    context["name"] = update.entry[0].changes[0].value.contacts[0].profile_name


async def whatsapp_save_text_to_context(client, update: WhatsAppUpdate, context, key):
    context[key] = update.get_message_text()


async def whatsapp_save_value_to_context(client, update: WhatsAppUpdate, context, key, value):
    context[key] = value
