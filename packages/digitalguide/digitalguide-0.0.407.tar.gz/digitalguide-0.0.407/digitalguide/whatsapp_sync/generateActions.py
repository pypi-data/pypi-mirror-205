...
async def send_interactive_list(self, recipient_id, body_text, button_text, sections, header_text=None, footer_text=None):
    """
...
async def send_sticker(client, sticker : str, recipient_id: str, link=True):
    if link:
        data = {
            "messaging_product": "whatsapp",
            "to": recipient_id,
...
class Action():
    ...
    async def __call__(self, client, update: WhatsAppUpdate, context):
        ...
        for item in self.actions:
            ...
            r_unsend_messages.set(update.get_from(), result_json.messages[0].id)
