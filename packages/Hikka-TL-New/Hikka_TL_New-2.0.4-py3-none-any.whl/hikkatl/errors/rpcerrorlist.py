...
class MsgVoiceTooLongError(BadRequestError):
    def __init__(self, request):
        self.request = request
        super(Exception, self).__init__('Voice message is too big to recognize' + self._fmt_request(self.request))

    def __reduce__(self):
        return type(self), (self.request,)


class MsgVoiceMissingError(BadRequestError):
    def __init__(self, request):
        self.request = request
        super(Exception, self).__init__('There is no voice in message to transcribe' + self._fmt_request(self.request))
