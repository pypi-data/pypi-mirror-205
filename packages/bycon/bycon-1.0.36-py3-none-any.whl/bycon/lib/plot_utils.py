...
def _plot_order_histograms(plv, results, byc):
    ...
    if plv.get("plot_cluster_histograms", True) is True and len(results) > 2:
        reorder, linkage = cluster_frequencies(plv, results, byc)
...
def _plot_order_samples(plv, results, byc):
    ...
    if plv.get("plot_cluster_samples", True) is True and len(results) > 2:
        reorder, linkage = cluster_samples(plv, results, byc)
        results[:] = [results[i] for i in reorder]
...
def _plot_add_samplestrips(plv, results, byc):
    ...
    if len(results) > 0:
        ...
        results = _plot_order_samples(plv, results, byc)
