...
LastEditTime: 2023-04-26 21:20:24
...
def combine_cli_cmd(jmx_dir, threads, loops, jmx_files=None, others_args=''):
    ...
    for jmx in jmx_files:
        ...
        csv_file_name = f"{csvlog_dir}/{tmp}.csv"
        web_report = f"{report_dir}/{tmp}"
        cmd = f"jmeter -Jthreads={threads} -Jloops={loops} {others_args} -n -t {jmx_dir}/{jmx} -l {csv_file_name} -e -o {web_report}"
        print(cmd)
        cmd_list.append(cmd)
    for cmd in cmd_list:
        exec_cmd(cmd)
