...
class Changelog:
    ...
    def _find_last_tag(self, project_name, tags):
        ...
        # ultima tag
        tag_commit = tags[0]
        tag_version = str(tag_commit).replace(self._get_config().version.format(
            project_name='',
            major=VersionUtils.extract_version(actual_version, Version.MAJOR),
            minor=VersionUtils.extract_version(actual_version, Version.MINOR),
            patch=VersionUtils.extract_version(actual_version, Version.PATCH)), '')
