...
def test_create_with_team():
    """Check creating a PPA for a particular team."""
    lp = LpServiceMock()
    lp.launchpad.add_person('test_team_name')
    ppa_group = PpaGroup(service=lp, name='test_team_name')
    ppa = ppa_group.create('ppa_test_name')
    assert ppa is not None
    assert ppa.address == 'ppa:test_team_name/ppa_test_name'
