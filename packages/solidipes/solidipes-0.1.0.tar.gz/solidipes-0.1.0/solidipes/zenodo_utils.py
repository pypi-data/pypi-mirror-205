...
def get_host_and_id(identifier):
    ...
    schemes = [
        # must check "sandbox.zenodo" first, otherwize "zenodo" will match
        {
            "pattern": r"sandbox.zenodo.org/record/(\d+)",
...
def update_deposition_metadata(deposition_url, metadata, access_token):
    import json
...
def upload_archive(bucket_url, archive_path, access_token, progressbar=None):
    import os
    ...
    check_response(response, 200, "uploading archive")
...
def download_files(record, destination, progressbar=False):
    ...
    for i in range(n_files):
        ...
        # Download file
        print(f"({i+1}/{n_files}) Downloading {key}...")
        try:
