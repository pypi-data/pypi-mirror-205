    ...
    if isinstance(tile_grid_size, tuple) or isinstance(tile_grid_size, list):
        if len(tile_grid_size) != 2:
            raise AutoMLVisionValidationException(
                "tile_grid_size is of size {}, should be of size 2. Cannot use tiling. For more details, refer {}"
                .format(len(tile_grid_size), TilingParameters.TILING_DOC_LINK), has_pii=False)
    else:
        # Resolving TypeEror when tile_grid_size is neither tuple nor list
        raise AutoMLVisionValidationException(
            "tile_grid_size value is {}, but it should be of size 2. Cannot use tiling. For more details, refer {}"
            .format(tile_grid_size, TilingParameters.TILING_DOC_LINK), has_pii=False)
