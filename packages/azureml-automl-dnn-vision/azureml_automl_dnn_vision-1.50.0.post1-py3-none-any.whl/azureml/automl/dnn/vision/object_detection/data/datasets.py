...
from azureml.automl.dnn.vision.common.constants import SettingsLiterals, TrainingCommonSettings
...
class CommonObjectDetectionDataset:
    ...
    def _downsample_for_instance_segmentation(image, annotations):
        ...
        if downsampling_factor > 1.0:
            ...
            logger.warning(
                "Downsampling {}x{} image with {} objects by {:.2f} to {}x{}.".format(
                    image.width, image.height, num_objects_with_masks, downsampling_factor,
    ...
    def _init_dataset(self, images_df, stream_image_files, ignore_data_errors=True):
        ...
        for index, label in enumerate(images_df[self._label_column_name]):
            ...
            try:
                ...
                logger.warning("Error in annotations. Skipping image at index {} (approximate).".format(index))
                if ignore_data_errors:
                    logging_utilities.log_traceback(ex, logger)
                    continue
                else:
            ...
            if (len(label) > 0) and (len(cur_img_object_infos) == 0):
                logger.warning("No valid annotations. Skipping image at index {} (approximate).".format(index))
                continue
        ...
        logger.info("{} background images.".format(num_background_images))
