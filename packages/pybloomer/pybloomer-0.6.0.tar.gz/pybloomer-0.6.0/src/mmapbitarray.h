...
MBArray *
mbarray_Create_Mmap(BTYPE num_bits, const char *file, const char *header, int header_len, int oflag, int perms);
...
static FORCE_INLINE size_t _vector_offset(MBArray *array, BTYPE bit) {
    return (size_t)(array->preamblesize + bit / (sizeof(DTYPE) << 3));
}


static FORCE_INLINE size_t _vector_byte(BTYPE bit) {
    return 1 << (bit % (sizeof(DTYPE) << 3));
}


static FORCE_INLINE int mbarray_Set(MBArray *array, BTYPE bit) {
    if (bit > array->bits) {
        errno = EINVAL;
        return 1;
    }
...
static FORCE_INLINE int mbarray_Clear(MBArray *array, BTYPE bit) {
    if (bit > array->bits) {
        errno = EINVAL;
        return 1;
    }
...
static FORCE_INLINE int mbarray_Test(MBArray *array, BTYPE bit) {
    if (bit > array->bits) {
        errno = EINVAL;
        return -1;
    }
