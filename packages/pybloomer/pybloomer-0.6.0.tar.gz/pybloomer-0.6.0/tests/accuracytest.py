...
class TestAccuracyMixin(object):
    ...
    def test_false_pos_degredation(self):
        ...
        for error_rate in (0.1, 0.01, 0.001, 0.0001):
            ...
            items_in_filter = set(self._gen_random_items(bf.capacity))
            ...
            print("    %6s %9s %s" % ("n", "false_pos", "estimated error_rate"))
            for i in range(chunk_count):
                ...
                false_pos = len(
                    list(
                        filter(
                            bf.__contains__,
                            self._gen_random_items(pos_test_count, items_in_filter),
                        )
                    )
                )
    ...
    def test_accuracy(self):
        print(
            "\n%14s\t%14s\t%17s"
            % ("pos_test_count", "false_pos_rate", "error_rate_target")
        )
        # we'll check 10% to 0.01%
        for error_rate in (0.1, 0.01, 0.001, 0.0001):
            bf = self._bf(error_rate)
            ...
            false_neg = len(items_in_filter) - len(
                list(filter(bf.__contains__, items_in_filter))
            )
            ...
            false_pos = 0
            for test in self._gen_random_items(pos_test_count, items_in_filter):
                if test in bf:
                    false_pos += 1
            ...
            print(
                "%14d\t%14f\t%17f" % (pos_test_count, false_pos_rate, error_rate_target)
            )
            self.assertTrue(
                ...
                % (
                    false_pos,
                    pos_test_count,
                    false_pos / float(pos_test_count),
                    error_rate_target,
                ),
            )
            self.assertEqual(
                false_neg_rate,
                0.0,
                "false negative rate is nonzero: %0.6f" % (false_neg_rate,),
            )
            del bf
...
class StringAccuracyMallocTestCase(unittest.TestCase, TestAccuracyMixin):
    ...
    def _random_item(self):
        return "".join(random.choice(self.CHARS) for _ in range(self.STR_LEN))

    def _bf(self, error_rate):
        return pybloomer.BloomFilter(self.FILTER_SIZE, error_rate)
...
class StringAccuracyMmapTestCase(unittest.TestCase, TestAccuracyMixin):
    ...
    def setUp(self):
        self.temp_file = tempfile.NamedTemporaryFile(suffix=".bloom", delete=False)
    ...
    def _bf(self, error_rate):
        return pybloomer.BloomFilter(
            self.FILTER_SIZE, error_rate, self.temp_file.name
        )
