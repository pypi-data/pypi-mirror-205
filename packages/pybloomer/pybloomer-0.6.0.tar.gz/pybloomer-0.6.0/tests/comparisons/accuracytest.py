...
def test_errors(error_rate, filter_size, correct_overlap, num_test_words):
    ...
    try:
        bf = pybloomer.BloomFilter(filter_size, error_rate, bloom_file.name)
    except TypeError:
        bf = pybloomer.BloomFilter(filter_size, error_rate)

    with open(WORDS_FILE) as source_file:
        with open(TEST_WORDS) as test_file:
            run_test(
                bf, source_file, test_file, correct_overlap, num_test_words, error_rate
            )
...
def run_test(bf, source_file, test_file, correct_overlap, num_test_words, error_rate):
    ...
    positive_matches = sum(1 for word in test_file if word.rstrip() in bf)

    actual_error_rate = float(positive_matches - correct_overlap) / correct_overlap

    print(
        "Specified: %f; Measured: %f; num_hashes: %d, num_bits: %d"
        % (
            error_rate,
            actual_error_rate,
