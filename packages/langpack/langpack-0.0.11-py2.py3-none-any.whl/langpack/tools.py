...
def deploy():
    ...
    if args.instance == "lambda":
        ...
        if args.app_url:
            ...
            docker_command = f"sudo docker run --pull always --network host -v /home/ubuntu:/workspace {env_str} chuanli11/langpack-ubuntu:22.04 download_app_from_s3 {args.app_url}"
        ...
        docker_command = f"nohup sudo docker run --pull always --network host -v {app_path}:/app {env_str} -w /app chuanli11/langpack-ubuntu:22.04 python3 app.py > /dev/null 2>&1 &"
        print("-----------------------------------------------")
        print(docker_command)
        run_docker_command(remote_ip, username, lambda_cloud_key, docker_command)
