...
class ProcessConfig(ProcessConfigBase):
    ...
    def _get_cor(self):
        ...
        if isinstance(cor, str):  # auto-CoR
            cor = estimate_cor(
                ...
                cor_options_str=self.nabu_config["reconstruction"]["cor_options"],
                logger=self.logger,
            )
    ...
    def sino_shape(self, binning=False, subsampling=False, after_ha=False):
        """
        ...
        after_ha: bool
            Whether to return the sinogram shape before building the extended-FoV sinogram (default) or after.
        """
        n_a, _, n_x = self.radios_shape(binning=binning, subsampling=subsampling)
        if self.do_halftomo and after_ha:
            # int(round(cor/binning)) is not equivalent to int(round(cor))//binning
            # the former should be used because HA sinograms are built after data is binned
            cor = self.rotation_axis_position(binning=binning)
            n_x = get_extended_sinogram_width(n_x, cor)
            n_a = (n_a + 1) // 2
        return (n_a, n_x)

    def sinos_shape(self, binning=False, subsampling=False, after_ha=False):
        n_z, _ = self.radio_shape(binning=binning)
        return (n_z,) + self.sino_shape(binning=binning, subsampling=subsampling, after_ha=after_ha)
    ...
    def _build_processing_steps(self):
        ...
        if nabu_config["reconstruction"]["method"] is not None:
            ...
            rec_options["pixel_size_cm"] = dataset_info.pixel_size * 1e-4  # pix size is in microns in dataset_info
