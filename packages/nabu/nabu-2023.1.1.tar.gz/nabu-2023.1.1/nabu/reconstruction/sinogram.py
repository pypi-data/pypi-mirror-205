...
class SinoBuilder:
    ...
    def _get_sino_halftomo(self, radios, i, output=None):
    ...
    def get_sino(self, radios, i, output=None):
        ...
        """
        if self.halftomo:
            return self._get_sino_halftomo(radios, i, output=None)
        else:
            return self._get_sino_simple(radios, i)
    ...
    def _get_sinos_halftomo(self, radios, output=None):
        ...
        for i in range(n_z):
            output[i] = self._get_sino_halftomo(radios, i)
        return output
...
def get_extended_sinogram_width(sino_width, rotation_axis_position):
    ...
    """
    if rotation_axis_position < (sino_width - 1) / 2.0:
        # CoR is on left side - flip to fallback on standard case
        rotation_axis_position = int(sino_width - 1 - rotation_axis_position)
    # TODO it would be more accurate to use int(round(2 * CoR))
    # or perhaps 2*int(round(CoR))) to have a multiple of 2
    # For now it is 2*int(CoR), so we can lose up to two pixels on the edges
    return 2 * int(rotation_axis_position)
