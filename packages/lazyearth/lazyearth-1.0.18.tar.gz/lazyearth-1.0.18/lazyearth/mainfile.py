...
class objearth():
    ...
    def get_model(self,weights_path=None):
        ...
        return model
    def superresolution(self,rgb):
        h5_dir = os.path.join(os.path.dirname(__file__), 'models')
        model_weights_path = os.path.join(h5_dir,'weights.h5')
        model = self.get_model(model_weights_path)                #Prepare model
        x = self.load_data(rgb)
