...
from typing import Callable, Coroutine, Generic, Mapping, Type
...
class FilenameProviderProto(Protocol[T_contr]):
    @abstractmethod
    async def filename(self, file_factory: FileFactoryBase, message: T_contr) -> str:
        pass


class FilenameProviderDefault(FilenameProviderProto):
    def __init__(self, format_string: str) -> None:
        self._format_string = format_string

    async def filename(
        self, file_factory: FileFactoryBase, message: TelegramMessage
    ) -> str:
        return await file_factory.filename(message)


class FilenameProviderFormat(FilenameProviderProto):
    def __init__(self, format_string: str) -> None:
        self._format_string = format_string
    ...
    async def filename(
        ...
        return await super().filename(message)
