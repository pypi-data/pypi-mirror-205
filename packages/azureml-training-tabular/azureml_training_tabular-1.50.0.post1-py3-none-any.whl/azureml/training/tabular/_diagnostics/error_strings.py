...
class ErrorStrings:
    ...
    FORECAST_DISTRIBUTED_WRONG_INDICATOR_TYPE = (
        "Wrong data type in the indicator type dictionary was encountered."
    )
    ...
    FORECAST_SERIES_NOT_IN_TRAIN = (
        "No prediction available for series id {tsid} because "
    ...
    LARGE_DATA_ALGORITHMS_UNSUPPORTED = (
    ...
    ROLLING_FORECAST_MISSING_TARGET_COLUMN = (
        "The provided test data is missing target column which is required when "
        "'forecast_mode' is set to 'rolling'. Please either set 'forecast_mode' "
        "to 'recursive' or provide 'target_column_name'."
    )
    ROLLING_FORECAST_MISSING_TARGET_VALUES = (
        'The provided test data is missing target values for some times within '
    ...
    TCN_EXTRA_COLUMN_IN_TEST_SET = (
        "The data set contains extra column {column}. "
    ...
    TIMESERIES_EXTENSION_DATE_MISALIGNED = (
        "Context data for series id {tsid} is expected to start at "
    ...
    UNSUPPORTED_VALUE_IN_LABEL = (
        "Label column contains unsupported value or values are from "
