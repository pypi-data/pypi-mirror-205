    ...
    def login_widget(self, form_name: str='Login', location: str='main') -> tuple:
        ...
        if location not in ['main', 'sidebar']:
            ...
            if user:
                user = "for " + user  
            if location == 'main':
                login_form = st.form(f'Authentication Required', clear_on_submit=True)
            elif location == 'sidebar':
                login_form = st.sidebar.form('Authentication Required', clear_on_submit=True)
            login_form.subheader(f'Welcome {user}')
