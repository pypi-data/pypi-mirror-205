...
def ApproxCountDistinctPartialAgg(col: str, precision: int) -> List[Column]:
    sc = SparkContext._active_spark_context
    udf_name = f"tecton_approx_count_distinct_partial"
    sc._jvm.com.tecton.udfs.spark3.ApproxCountDistinctPartialRegister().register(udf_name, precision)
    columns = expr(f"{udf_name}({col})")
    return [columns.indices.cast(ArrayType(LongType())), columns.registers.cast(ArrayType(LongType()))]


def _make_approx_count_distinct_partial_aggregation(precision: int) -> Callable:
    def _approx_count_distinct_partial_aggregation(cols: List[str]) -> List[Column]:
        return ApproxCountDistinctPartialAgg(cols[0], precision)
...
AGGREGATION_PLANS = {
    ...
    # TODO(TEC-14015): Support continuous mode for approx count distinct.
