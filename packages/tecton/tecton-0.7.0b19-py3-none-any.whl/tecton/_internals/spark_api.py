...
def get_historical_features_for_feature_service(
    ...
    if isinstance(spine, pd.DataFrame):
        spine_schema = _get_spine_schema_from_feature_set_config(feature_set_config)
...
def get_historical_features_for_feature_definition(
    ...
    if spine is not None:
        ...
        if isinstance(spine, pd.DataFrame):
            fd_schema = _get_spine_schema_from_fd_wrapper(feature_definition)
            if timestamp_key != None:
...
# details about how to build spine_schema for different FeatureView, please see `spine_schema` method defined in
# FeatureViewSpec and its children classes.
def _get_spine_schema_from_fd_wrapper(fd_wrapper: FeatureDefinitionWrapper) -> schema.Schema:
    (jk_schema, rc_schema) = fd_wrapper.spine_schema
    if rc_schema is not None:
        rc_schema = schema_spark_utils.schema_from_spark(
            spark_schema_wrapper.SparkSchemaWrapper.from_proto(rc_schema).unwrap()
        )
        return jk_schema + rc_schema
    else:
        return jk_schema


# Returns the spine schema of this FeatureSetConfig. The column name in the returned schema uses the override join
# key name if there is one.
def _get_spine_schema_from_feature_set_config(feature_set_config: feature_set_config.FeatureSetConfig) -> schema.Schema:
    spine_schema = schema.Schema(schema_pb2.Schema())
    for dac in feature_set_config.definitions_and_configs:
        (jk_schema, rc_schema) = dac.spine_schema
        if rc_schema is not None:
            rc_schema = schema_spark_utils.schema_from_spark(
                spark_schema_wrapper.SparkSchemaWrapper.from_proto(rc_schema).unwrap()
            )
            spine_schema += rc_schema + jk_schema
        else:
            spine_schema += jk_schema
    return spine_schema
