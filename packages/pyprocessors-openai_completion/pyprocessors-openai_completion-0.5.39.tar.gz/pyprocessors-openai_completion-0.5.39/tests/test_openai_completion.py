...
JINJA_PROMPTS = {
    'preserve_entities': """Generates several variants of the following context while preserving the given named entities.
    Context: {{ doc.text }}
    Output language: {{ doc.metadata['language'] }}
    {%- set entities=[] -%}
    {%- for a in doc.annotations -%}
      {%- do entities.append(a.text) -%}
    {%- endfor %}
    Named entities: {{ entities|join(', ') }}""",
    'substitute_entities': """Generates several variants of the following context while substituting the given named entities by semantically similar named entities, for each variant list the new named entities.
    Context: {{ doc.text }}
    Output language: {{ doc.metadata['language'] }}
    {%- set entities=[] -%}
    {%- for a in doc.annotations -%}
...
def test_jinja_doc(typed_prompt):
    ...
    parameters = OpenAICompletionParameters(
        ...
        max_tokens=3000, completion_altText=type,
        prompt=prompt
    )
