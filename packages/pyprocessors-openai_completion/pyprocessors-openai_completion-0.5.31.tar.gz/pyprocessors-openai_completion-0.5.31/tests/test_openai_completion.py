...
def test_jinja_doc():
    parameters = OpenAICompletionParameters(
        ...
        max_tokens=3000, completion_altText="variants",
        prompt="""Generates several variants of the following context while preserving the named entities
Context: {{ doc.text }}
...
Named entities: {{ entities|join(', ') }}"""
    ...
    with source.open("r") as fin:
        ...
        sum_file = testdir / f"data/jinjadocs_{OpenAIModel.gpt_3_5_turbo.value}.json"
