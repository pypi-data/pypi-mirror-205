...
while IFS=$'\t' read -r seq_name cluster_number; do
    ...
    if [ "$seq_name" != "SequenceName" ]; then
        ...
        echo "$seq_name" >> "${tmp_dir}/test_labels/${cluster_number}.txt"
    fi
...
mkdir ${tmp_dir}/test_trees_subtree
...
do
        ...
        while IFS=$'\t' read -r seq_name cluster_number; do
            ...
            if [ "$seq_name" != "SequenceName" ]; then
                # Append the sequence name to the corresponding cluster file, creating a new line for each sequence name
                echo "$seq_name" >> "${tmp_dir}/test_labels_subtree_tmp/${cluster_number}.txt"
            fi
...
do
  i=`basename ${label}`
  i=${i%.*}
  cat ${label} ${tmp_dir}/test_labels/${i}.txt > ${tmp_dir}/test_labels_add_repr
  nw_prune -vf $backbone_tree ${label} > ${tmp_dir}/test_trees_add_repr/${i}.nwk
  s="${s} ${tmp_dir}/test_labels_add_repr"
done
...
mv ${tmp_dir}/test_trees_subtree ${outdir}/test_trees_subtree
