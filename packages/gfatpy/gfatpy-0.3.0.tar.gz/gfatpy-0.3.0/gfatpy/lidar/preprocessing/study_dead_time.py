...
def estimate_dead_time(date_str, channel, don, time_range=None,
    ...
    try:
        t_start = time.time()
        print("Estimate Dead Time for %s. channel=%s. illum=%i" % (date_str, channel, don))
        # get raw data
        ...
        for i in range(n_t):
            # preprocess analog
            an_i = lidar.preprocessing_analog_signal(an[i, :], bz_an, bg_an[i], dc, dc_bg)
            an_pp[i, :] = an_i

            # Loop over taus
            for j in range(n_tau):
                # preprocess photoncounting
                pc_ij = lidar.preprocessing_photoncounting_signal(pc[i, :], tau[j], bz_pc, bg_pc[i])
                pc_pp[i, :, j] = pc_ij

                # gluing
                if gluing_type == "scc":  # D'Amico
                    gl_ij, c_an, c_ph, idxs, glued = lidar.gluing(an_i, pc_ij, ranges,
                                                                  adc_range, adc_bits, n_res=n_res, pc_threshold=20, correlation_threshold=0.9,
                                                                  range_threshold=(1000, 5000), min_points=15, slope_threshold=2,
                                                                  stability_threshold=1, step=5, use_photon_as_reference=True)
                    if glued:
