...
def plot_eta_start_calib(
    ...
    # PHOTONCOUNTING
    signal_to_rcs(d[f"signal_T_P45_{channel_str}p"], d.range).plot(
        ax=ax1, y="range", lw=2, c="deepskyblue", label=r"RCS$^T_{+45}$. PC"
    )
    ...
    d[f"eta_star_profile_{channel_str}a"].plot(ax=ax3, y="range", c="g", label=r"AN")
    d[f"eta_star_profile_{channel_str}p"].plot(ax=ax3, y="range", c="m", label=r"PC")
    ax3.grid()
    ...
    if output_dir is not None:
        if output_dir.exists():
            plt.savefig(output_dir / f"calibration_{channel_str}.png")
        else:
