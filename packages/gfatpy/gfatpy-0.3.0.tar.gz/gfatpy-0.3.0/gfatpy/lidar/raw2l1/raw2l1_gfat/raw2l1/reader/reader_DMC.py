...
def init_data(data_dim, logger):
    ...
    logger.info(
        "DATE_ROTATIONAL_RAMAN is %s and %s"
        % ("03/05/2017 00:00:00", DATE_ROTATIONAL_RAMAN)
    )
...
def read_header(file_id, data, data_dim, index, logger, date_only=False):
    ...
    if data_dim["n_chan"] > 7:
        if dt.datetime.strptime(datetime_start, DATE_FMT) > dt.datetime.strptime(
            DATE_ROTATIONAL_RAMAN, DATE_FMT
        ):
            data["wavelength"][8] = 353.9
            data["wavelength"][9] = 530.2
            logger.info(
                "Raman wavelength changed by %s and %s"
                % (data["wavelength"][8], data["wavelength"][9])
            )
        else:
