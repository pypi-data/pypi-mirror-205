...
from bigeye_sdk.model.dbt_schema import SimpleDbtSchema, SimpleDbtColumn, relationships_test_names
...
from bigeye_sdk.model.protobuf_message_facade import SimpleConstantThreshold, SimpleMetricParameter, SimpleAutoThreshold
...
def dbt_tests_to_metrics(
    ...
    dbt_tables: Dict[str, List[SimpleDbtColumn]] = {t.name.lower(): t.columns for t in dbt_schema.models}
    tables = {t.name.lower(): t.id for t in client.get_tables(warehouse_id=[source.id], schema=[schema_name]).tables if
              t.name.lower() in dbt_tables.keys()}

    for t, col in dbt_tables.items():
        for c in col:
            for test in c.tests:
                ...
                try:
                    ...
                    log.error(f"No template found in source {warehouse_name} with any of the names: {relationships_test_names}")
                    log.info("Moving to next test.")
                    continue

                mc = test.to_datawatch_object(to_config=True)
                mc.warehouse_id = source.id
                mc.dataset_id = tables[t]
