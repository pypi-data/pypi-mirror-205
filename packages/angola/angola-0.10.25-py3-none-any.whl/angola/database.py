...
class _ItemMixin(dict):
    ...
    def update(self, data: dict) -> "Self":
        """
        UPDATE: Update the active _CollectionItem
...
class _CollectionItem(_ItemMixin):
    """
    _CollectionItem
    ...
    def context(self):
        """
        *ContextManager for _CollectionItem
    ...
    def link(self, to_item:"_CollectionItem", data={}, edge_collection_name=None):
        """
        ::Graph::
...
class Database(object):
    ...
    def get_item(self, path:str) -> _CollectionItem:
        """
    ...
    def link_edges(self, from_item:"_CollectionItem", to_item:"_CollectionItem", data:dict={}, edge_collection_name:str=None):
        """
        GRAPH
        Link edges of 2 collections from:to

        Params:
            - from_item:_CollectionItem
            - to_item:_CollectionItem
            - data:dict
    ...
    def unlink_edges(self, from_item:"_CollectionItem", to_item:"_CollectionItem"):
        pass

    def _load_item(self, data:dict) -> "_CollectionItem":
        if "_id" in data:
    ...
    def traverse(self, from_item:"_CollectionItem", collection:"Collection", relations:list("Collection")=None, direction="outbound", filters={}):
        """
        == GRAPH == 
        To traverse 

        Params:
            - from_item:_CollectionItem - The item to traverse from
...
class Collection(object):
    ...
    def _commit(self, item:_CollectionItem, replace_document=False):
        """
    ...
    def item(self, data:dict) -> _CollectionItem:
        """
        Load data as item
        ...
        """
        item = None
        if not isinstance(data, _CollectionItem) and "_key" not in data:
            item = _CollectionItem.new(data, db=self.db, collection=self.collection, commiter=self._commit, immut_keys=self._immut_keys, custom_ops=self._custom_ops)               
        else:
            item = _CollectionItem(data, db=self.db, collection=self.collection, commiter=self._commit, immut_keys=self._immut_keys, custom_ops=self._custom_ops)
    ...
    def get(self, _key) -> _CollectionItem:
        """ 
    ...
    def create(self, data:dict={}) -> _CollectionItem:
        """
        ...
        Example:
            ...
            item.commit()
        """
        return _CollectionItem.new(data, db=self.db, collection=self.collection, commiter=self._commit, custom_ops=self._custom_ops)

    def insert(self, data:dict, _key=None) -> _CollectionItem:
        """
        ...
        if _key or "_key" in data:
            ...
            data["_key"] = _key
        item = data
        if not isinstance(data, _CollectionItem):
            item = _CollectionItem.new(data, db=self.db, collection=self.collection, custom_ops=self._custom_ops)
        self.collection.insert(item.to_dict(), silent=True)
        return self.get(item._key) 

    def update(self, _key:str, data:dict, replace_document=False) -> _CollectionItem:
        """
    ...
    def upsert(self, data:dict) -> _CollectionItem:
        """
...
class SubCollection(object):
    ...
    def __init__(self, item: _CollectionItem, name: str, constraints:list=None, custom_ops:dict={}):
        self._item = item
        self._name = name
