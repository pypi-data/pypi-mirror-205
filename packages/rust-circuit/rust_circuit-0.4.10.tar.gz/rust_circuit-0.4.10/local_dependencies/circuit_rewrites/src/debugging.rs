use anyhow::Result;
use circuit_base::{circuit_utils::toposort_circuit, evaluate, Array, CircuitNode, CircuitRc};
use circuit_utils::replace_nodes;
...
pub fn assert_is_close(a: CircuitRc, b: CircuitRc) {
    let a_ten = evaluate(a).unwrap();
    let b_ten = evaluate(b).unwrap();
...
pub fn randn_ablate_great_grandchildren(circ: CircuitRc) -> Result<CircuitRc> {
