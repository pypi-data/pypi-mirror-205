...
use circuit_utils::replace_nodes_py;
use pyo3::prelude::*;
use rr_util::{
    name::Name, py_types::PyOpAtAxes, rearrange_spec::RearrangeSpec, shape::right_align_shapes,
    util::HashBytes,
};
use rustc_hash::FxHashMap as HashMap;

use crate::concat_rewrite::split_to_concat;
...
// module_remove_some_unused_inputs
...
pub fn fuse_concat_modules(
    ...
    let pure_rank = spec.circuit.rank();
    let modules_one_batch = modules
        .iter()
        .map(|x| {
            let dif = x.rank() - spec.circuit.info().rank();
            match dif {
                0 => Ok(Module::new(
    ...
    let splitted_modules: Vec<CircuitRc> = split_to_concat(
        ...
        modules_one_batch
            .iter()
            .map(|x| x.info().shape[0].unwrap_err())
            .collect::<Result<Vec<_>>>()?,
    )
    .children()
    .collect();
