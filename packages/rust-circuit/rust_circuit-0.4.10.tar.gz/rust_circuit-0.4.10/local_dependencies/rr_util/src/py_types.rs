...
use crate::{
    shape::{shape_into_known, shape_is_known, Shape},
...
pub struct PyUtils {
    ...
    pub optimizing_modules_warning: PyObject,
...
pub static PY_UTILS: GILLazyPy<PyUtils> = GILLazyPy::new_py(|py| {
    ...
    PyUtils {
        ...
        optimizing_modules_warning: get("OptimizingModulesWarning"),
...
macro_rules! make_py_func {
    {
        ...
        paste::paste!{
            $(#[$m])*
            $vi fn [<$name _py>]($py : Python<'_>, $($arg_name : $arg_ty,)*) -> $ret_ty {
                $($tt)*
            }
...
pub fn scalar_to_tensor(v: f64, shape: Shape, device_dtype: TorchDeviceDtype) -> Result<Tensor> {
    if !shape_is_known(&shape) {
        bail!("scalar_to_tensor of shape with unknown sizes {shape:?}")
    }
    PY_UTILS
        .scalar_to_tensor
        .call(
            py,
            (v, PyShape(shape_into_known(shape)), device_dtype),
            None,
        )
...
pub fn i64_to_tensor(v: i64, shape: Shape, device_dtype: TorchDeviceDtype) -> Result<Tensor> {
    if !shape_is_known(&shape) {
        bail!("i64_to_tensor of shape with unknown sizes {shape:?}")
    }
    PY_UTILS
        .scalar_to_tensor
        .call(
...
#[apply(make_py_func)]
#[py_ident(py)]
pub fn un_flat_concat(tensor: &Tensor, split_shapes: Vec<Vec<usize>>) -> PyResult<Vec<Tensor>> {
    PY_UTILS
        .un_flat_concat
        .call(
            py,
            (
                tensor,
                split_shapes.into_iter().map(PyShape).collect::<Vec<_>>(),
            ),
            None,
        )
...
impl Tensor {
    ...
    pub fn hash_base16(&self) -> Option<String> {
        self.hash.as_ref().map(base16::encode_lower)
    }
    ...
    pub fn max_usize(&self) -> Result<usize> {
        Python::with_gil(|py| {
            Ok(self
                .tensor
                .call_method1(py, "max", ())?
                .call_method1(py, "item", ())?
                .extract(py)?)
        })
    }
}
...
pub struct PyShape(pub Vec<usize>);
...
#[derive(FromPyObject, Clone, Debug)]
pub struct PySymShape(pub Shape);

impl IntoPy<PyObject> for PySymShape {
    fn into_py(self, py: Python<'_>) -> PyObject {
        PyTuple::new(py, self.0).into_py(py)
    }
}
