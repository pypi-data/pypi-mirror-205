...
use rr_util::{
    ...
    fn_struct,
    py_types::{i64_to_tensor, random_torch_i64, PySymShape},
    python_error_exception,
    shape::{shape_into_known, Shape},
    sv,
    tensor_util::{
        Slice, TensorAxisIndex, TensorIndex, TorchDevice, TorchDeviceDtype, TorchDeviceDtypeOp,
        TorchDtype,
    },
    tu8v,
    util::HashBytes,
...
impl SampleSpecTrait for RandomSampleSpec {
    fn sample_var(&self, var: &DiscreteVar) -> Result<CircuitRc> {
        let name = var.info().name.map(|n| format!("{n} sampled").into());
        let values = var.values().clone();
        let values_ndim = values.ndim();
        ...
        if self.simplify
            ...
            && var.values().info().device_dtype.device.is_some()
            && let Ok(index) = evaluate(index.clone())
            && index.ndim() == 1
        {
                let index = TorchDeviceDtypeOp{device: var.values().info().device_dtype.device, dtype: None}.cast_tensor(index);
                return Ok(Index::try_new(
                    values,
                    TensorIndex::new_single(TensorAxisIndex::Tensor(index.try_into().unwrap()), 0, values_ndim),
                    name
                )?.rc())
        };
        let batch_x = true;
        let batch_index = false;
        Ok(GeneralFunction::gen_index(
            values,
            index,
            -(values_ndim as i64),
            batch_x,
...
impl RunDiscreteVarAllSpec {
    fn try_new(groups: Vec<CircuitRc>, subsets: Option<Vec<Slice>>) -> Result<Self> {
        ...
        Ok(Self {
            groups: groups.clone(),
            subsets: subsets.unwrap_or_else(|| {
                groups
                    .iter()
                    .map(|_| Slice {
                        start: None,
...
impl SampleSpecTrait for RunDiscreteVarAllSpec {
    fn sample_var(&self, var: &DiscreteVar) -> CircResult {
        ...
        if !axes_to_repeat.is_empty() {
            let counts = self
                .groups
                .iter()
                .filter(|&g| g != var.probs_and_group())
                .map(|g| g.info().shape[0].unwrap_err())
    ...
    fn get_empirical_weights(&self) -> CircuitRc {
        ...
        let all_weights_sum = Einsum::nrc(
            vec![(
                all_weights.clone(),
                (0..(all_weights.rank() as u8)).collect(),
            )],
            tu8v![],
            None,
        );
    ...
    pub fn sample(&mut self, circuit: CircuitRc) -> Result<CircuitRc> {
        ...
        let out = if is_orig_shape {
            out.expand_at_axes(
                ...
                shape_into_known(self.sample_spec.get_sample_shape()),
                None,
            )?
            .rc()
        } else {
...
impl Sampler {
    ...
    fn get_expectation(&self, circuit: CircuitRc) -> CircResult {
        let circuit_rank = circuit.rank() as u8;
        let spec_nb_axes = self.spec_nb_axes() as u8;
...
pub fn factored_cumulant_expectation_rewrite(cumulant: &Cumulant) -> CircuitRc {
    ...
    let mut sub: Vec<(CircuitRc, f64)> = vec![(centered_moment, 1.)];

    for p_indexs in partitions(children.len()) {
        let p = p_indexs
            .iter()
            .map(|is| is.iter().map(|i| (*i, children[*i].clone())).collect_vec())
            .collect_vec();
