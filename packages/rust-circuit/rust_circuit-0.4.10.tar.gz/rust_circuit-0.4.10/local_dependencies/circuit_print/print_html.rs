...
use pyo3::prelude::*;
use rand::Rng;
use rr_util::{fn_struct, name::Name, py_types::MaybeNotSet, simple_default, util::HashBytes};
...
pub fn PrintOptionsBase_print(x: Option<PyObject>, ys: &[CircuitRc]) -> Result<()> {
    Python::with_gil(|py| {
        x.map_or_else(
            || Ok(Default::default()),
            |x| x.extract::<PrintOptionsBase>(py),
        )?
        .print(ys.to_vec())
    })
}
...
#[allow(non_snake_case)]
pub fn PrintHtmlOptions_print(x: Option<PyObject>, ys: &[CircuitRc]) -> Result<()> {
    Python::with_gil(|py| {
        x.map_or_else(
            || Ok(Default::default()),
            |x| x.extract::<PrintHtmlOptions>(py),
        )?
        .print(ys.to_vec())
    })
}
    ...
    pub fn fill_overall_data(
        ...
        fn recurse(
            ...
            for (child_idx, c) in circ.children().enumerate() {
                ...
                if let Some(m) = circ.as_module() && let Some(i) = child_idx.checked_sub(1) {
...
impl PrintHtmlOptions {
    ...
    pub fn size_colorer() -> CircuitToColor {
        CircuitToColor::new_dyn(Box::new(|c| {
            Ok(circuit_to_size_color_code(c).map(color_from_colorcode))
        }))
    }
    ...
    pub fn computability_colorer() -> CircuitToColor {
        CircuitToColor::new_dyn(Box::new(|c| {
            Ok(circuit_to_computability_color_code(c).map(color_from_colorcode))
        }))
    }
}
...
fn color_from_colorcode(code: usize) -> String {
    ...
    COLORCODE_TO_RGB[code].to_owned()
}
