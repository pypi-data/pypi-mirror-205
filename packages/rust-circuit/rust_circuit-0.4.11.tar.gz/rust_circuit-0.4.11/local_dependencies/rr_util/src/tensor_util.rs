...
impl FromPyObject<'_> for PyEllipsis {
    fn extract(ob: &'_ PyAny) -> PyResult<Self> {
        if ob.is_ellipsis() {
            Ok(PyEllipsis)
        } else {
            Err(PyTypeError::new_err("expected Ellipsis"))
        }
    }
}

// only used for FromPyObject for Index constructor
#[derive(FromPyObject)]
pub enum TensorAxisIndexEllipsis {
    Index(TensorAxisIndex),
    Ellipsis(PyEllipsis),
}
...
impl TensorIndex {
    ...
    pub fn from_string_rank(
        ...
        Ok(TensorIndex(
            sections
                ...
                .map(|x| -> Result<Vec<_>> {
                    if x == &"..." {
                        if seen_ellipsis || rank.is_none() {
                            bail!(er.clone());
                        }
                        seen_ellipsis = true;
                        return Ok(vec![
                            TensorAxisIndex::IDENT;
                            rank.unwrap() - sections.len() + 1
                        ]);
                    }
                    if let Ok(int) = parse_numeric(x) {
                        return Ok(vec![TensorAxisIndex::Single(int)]);
                    }
                    ...
                    if let [start, stop] = x.split(':').map(str::trim).collect::<Vec<_>>()[..] {
                        return Ok(vec![TensorAxisIndex::Slice(Slice {
    ...
    pub fn from_bijection_string(
        ...
        Self::from_string_rank(string, None, tensor_cache)
    }
}
