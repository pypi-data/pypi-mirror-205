...
impl PrintOptions {
    ...
    pub fn repr_extra_info(&self, circ: CircuitRc) -> Result<String> {
        Ok(match &**circ {
            ...
            Circuit::GeneralFunction(gf) => gf.spec.serialize()?,
            Circuit::Symbol(sy) => {
                if sy.uuid.is_nil() {
                    "".to_owned()
                } else {
...
static DEBUG_PRINT_OPTIONS: GILLazy<RwLock<PrintOptions>> =
    GILLazy::new(|| RwLock::new(init_print_options()));

#[pyfunction]
pub fn set_debug_print_options(options: PrintOptions) {
    *DEBUG_PRINT_OPTIONS.write().unwrap() = options;
}
...
pub fn debug_repr(circ: CircuitRc) -> Result<String> {
    Ok(format!(
        ...
        DEBUG_PRINT_OPTIONS.read().unwrap().repr(circ)?
    ))
}
