...
pub fn rewrite_cum_to_circuit_of_cum(
    ...
    let r = match &**node {
        Circuit::Rearrange(n) => {
            ...
            .concat()
            .into();
            let new_output_ints = [before_axis, spec.clone().output_ints, after_axis]
                .concat()
                .into();
            let new_int_sizes = spec
                ...
                .clone()
                .into_iter()
                .chain((0..(nb_axis_after + nb_axis_after)).map(|_| Size::NONE))
                .collect();

            let new_spec = RearrangeSpec::new(new_input_ints, new_output_ints, new_int_sizes)?;
            Rearrange::nrc(
                on_sub_cumulant(replace_node(cumulant, n.node(), node_position))?,
                new_spec,
            ...
            for mine_perm in (0..(p_mine.len())).permutations(p_other.len()) {
                ...
                debug_assert!(
                    !(just_fully_touching_partitions
                        ...
                        && !outside_perm.is_empty())
                );

                let fused_per: Vec<Vec<(usize, CircuitRc)>> = [
                    zip(mine_perm.clone(), p_other.clone())
                        .map(|(idx_mine, b_other)| [p_mine[idx_mine].clone(), b_other].concat())
