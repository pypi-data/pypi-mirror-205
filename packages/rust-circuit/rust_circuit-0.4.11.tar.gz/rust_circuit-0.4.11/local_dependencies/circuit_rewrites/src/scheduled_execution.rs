            ...
            match s {
                Instruction::Compute(key, circ) => {
                    ...
                    let tensors: Vec<Tensor> = child_keys
                        ...
                        .collect();
                    if key.len() == 1 {
                        let result = eval_tensors(circ, &tensors)?;
                        anyhow::ensure!(
                            result.shape() == circ.shape(),
                            "Wrong result shape {:?} (expected {:?}) while evaluating {circ:?}",
                            result.shape(),
                            circ.shape(),
                        );
                        live.insert(key[0].unwrap(), result);
                    } else {
                        let rs = eval_tensors_multi_output(circ, &tensors)?;
                        anyhow::ensure!(
                            rs.len() == key.len(),
                            "Wrong number of results {} (expected {}) while evaluating {circ:?}",
                            rs.len(),
                            key.len(),
                            circ = circ
                        );
        ...
        let run = |_py: Python| -> Result<()> {
            match s {
                Instruction::Compute(key, circ) => {
                    ...
                    let tensors_and_scales: Vec<(Tensor, f64)> = child_keys
                        .iter()
                        .map(|child_key| live[child_key].clone())
                        .collect();
                    let tensors: Vec<Tensor> =
                        tensors_and_scales.iter().map(|x| x.0.clone()).collect();
                    if key.len() == 1 {
                        let result = match &***circ {
                            Circuit::Einsum(_) => {
                                let new_scale = tensors_and_scales.iter().map(|x| x.1).product();
                                clamp(&(eval_tensors(circ, &tensors)?, new_scale))
                            }
                        ...
                        for (k, r, os) in
                            izip!(key, rs, &**gf.multi_output_shapes.as_ref().unwrap())
                        {
                            anyhow::ensure!(
                                r.shape() == os,
                                "Wrong result shape {:?} (expected {:?}) while evaluating {circ:?}",
