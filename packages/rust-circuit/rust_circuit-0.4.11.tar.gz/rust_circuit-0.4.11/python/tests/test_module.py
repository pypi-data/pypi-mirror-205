...
def test_push_down_modules(circ, d):
    ...
    note(circ.repr(po))
    circ1 = pu.push_down_modules(circ, traversal=rc.new_traversal(), skip_module=skips)
    note(circ1.repr(po))
    assert_close(circ.evaluate(), circ1.evaluate())


@hypothesis.given(
    get_c_st(probs_default=CP.kw(all=1, Module=20, Symbol=20, Cumulant=0), from_other=False, max_growth_steps=5),
    st.data(),
)
...
def test_upd_nested_id(circ, d):
    note(circ.repr(po))
    circ = rc.conform_all_modules(circ)
    try:
...
def dedup_pairwise(l, f):
    out = []
    for x in l:
        if not any(f(x, y) for y in out):
            out.append(x)
    return out


@hypothesis.given(
    get_c_st(from_other=False, max_growth_steps=10, probs_per_depth=[CP.kw(all=0, branches=1, Cumulant=0)]).filter(
        lambda c: c.num_children > 0
    ),
    st.data(),
)
@mark_not_interesting_if(rc.RearrangeSpecNotConformableError)
@mark_not_interesting_if(rc.ConstructModuleTriedToBatchUnbatchableInputError)
def test_expander_prop(circ, d):
    children = list(rc.all_children(circ))
    children.sort()
    cs = d.draw(st.lists(st.sampled_from(children), unique=True, min_size=1, max_size=2))
    cs = dedup_pairwise(cs, lambda x, y: (x in rc.all_children(y) or (y in rc.all_children(x))))
    b = d.draw(st.integers(1, 5))
    # print("\ncirc:")
    # circ.print()
    # print("circs:")
    # for c in cs:
    #     c.print()
    new_cs = [d.draw(get_c_st(shape=(b,) + c.shape, from_other=False, max_growth_steps=4)) for c in cs]
    note(circ.repr(po))
    expanded = rc.Expander(*[(c, lambda _, new_c=new_c: new_c) for (c, new_c) in zip(cs, new_cs)])(circ)
    note(expanded.repr(po))
    d = {c: new_c for (c, new_c) in zip(cs, new_cs)}

    def upd(x, i):
        return x.update(set(cs), lambda a: rc.Index(d[a], (i,) + ((slice(None, None),) * a.ndim)))

    concatted = rc.Concat(*[upd(circ, i).unsqueeze(0) for i in range(b)], axis=0)
    note(concatted.repr(po))
    assert_close(expanded.evaluate(), concatted.evaluate())
...
def test_expand_node():
    ...
    with pytest.raises(ExpandFixedIndexError):
        circ = Index(Symbol.new_with_none_uuid((2, 3), "0"), I[0:1, 0])
        expanded = expand_node(circ, [Array.randn(5, 3)])
        print(expanded)
    ...
    circ = Add(Symbol.new_with_none_uuid((2, 3, 1), "0"), Symbol.new_with_none_uuid((2, 1, 5), "1"))
    with pytest.raises(ShapeNotBroadcastableError):
        expand_node(
    ...
    s = """
    'rep_fst' [?, ?] Symbol 0bac3288-00e7-41bf-bd59-ac04de67ba47
    'rep_snd' [?, ?] Symbol d5f29a2c-5927-4237-a7ea-7fe7ec2c3be6
    'a' Module
      'add' Add
        'arg0' [] Symbol 7d2c48d4-6110-4759-be98-de15bfb20af2
        'arg1' [] Symbol 86b517ef-b83c-4ea0-9568-f89bb2106db9
      'fst' [?, ?] Symbol c41b44e7-4f04-4124-bf02-34e0133c7c19 ! 'arg0'
      'snd' [?, ?] Symbol 823c2e81-99a2-46b3-bec1-329109e06e20 ! 'arg1'
    """
    ...
    s = """
    'rep_fst' [?] Symbol 0bac3288-00e7-41bf-bd59-ac04de67ba47
    'rep_snd' [8, ?] Symbol d5f29a2c-5927-4237-a7ea-7fe7ec2c3be6
    'a' Module
      'add' Add
        'arg0' [] Symbol 7d2c48d4-6110-4759-be98-de15bfb20af2
        'arg1' [] Symbol 86b517ef-b83c-4ea0-9568-f89bb2106db9
    ...
    s = """
    'rep_fst' [7, 9, ?] Symbol 0bac3288-00e7-41bf-bd59-ac04de67ba47
    'rep_snd' [8, ?] Symbol d5f29a2c-5927-4237-a7ea-7fe7ec2c3be6
    'a' Module
      'add' Add
        'arg0' [?] Symbol 7d2c48d4-6110-4759-be98-de15bfb20af2
    ...
    for sym_on_top in [False, True]:
        s = f"""
        'rep_fst' [12, {"?" if sym_on_top else 8}, 9, 7] Symbol 0bac3288-00e7-41bf-bd59-ac04de67ba47
        'rep_snd' [13, 12, 9, 10, {8 if sym_on_top else "?"}, 7] Symbol d5f29a2c-5927-4237-a7ea-7fe7ec2c3be6
        'a' Module
          'add' Add
            'arg0' [?, ?] Symbol 7d2c48d4-6110-4759-be98-de15bfb20af2
            'arg1' [?] Symbol 86b517ef-b83c-4ea0-9568-f89bb2106db9
...
def get_complex_module_test(
    ...
    'inner' Einsum az,bz,cl->bac
      'inner.sym_arg0' [?, ?] Symbol a409cdc3-d962-4df1-8d1b-c5c8d8b50639
      'inner.concat' Concat 0
        'inner.sym_arg1' [3, ?] Symbol 02927290-fc4f-45b7-9822-267918e897d6
        'outer.sym_arg0' [7, ?] Symbol 6dcfaf1e-5e35-418a-ad88-3d79ae32096c
      'outer.sym_arg1' [?, ?] Symbol ecf273f5-82c5-4fbf-8097-d9b1a250e035
    ...
    assert inner.shape == (10, rc.Size.NONE, rc.Size.NONE)
    assert inner_call.shape == (inner_batch + (10, 7, rc.Size.NONE))
