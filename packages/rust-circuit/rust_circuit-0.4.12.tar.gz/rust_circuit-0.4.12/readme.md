...
Or build with `RUSTFLAGS="-C force-frame-pointers=yes"`, then you can profile with e.g. `perf record --call-graph fp -F 500 $(which python) /path/to/script.py`. View profile with `perf report`, or `perf script -F +pid > out.perf` and upload out.perf to https://www.speedscope.app/ or https://profiler.firefox.com/
