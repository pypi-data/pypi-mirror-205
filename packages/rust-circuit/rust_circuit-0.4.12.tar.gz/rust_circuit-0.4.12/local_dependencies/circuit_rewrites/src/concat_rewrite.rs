...
use pyo3::prelude::*;
use rr_util::{
    shape::{shape_try_as_known, shape_try_into_known},
...
pub fn pull_concat_once(circuit: CircuitRc) -> Option<CircuitRc> {
    ...
    let g = &|x: Index| {
        push_down_index_raw(&x, false, &mut |_, ix| Ok(f(ix)), None, false).unwrap_or(x.rc())
    };
    let name_maybe = circuit
        .children()
        .filter(|x| x.is_concat())
...
fn einsum_pull_concat(einsum: &Einsum, on_ix: &dyn Fn(Index) -> CircuitRc) -> Option<CircuitRc> {
    ...
    for (operand, ints) in einsum.args() {
        if let Circuit::Concat(concat) = &**operand
            ...
            did_anything = true;
            let sections_here = shape_try_into_known(concat.get_sizes_at_axis().into())?;
            ...
            .map(|summand_settings| {
                let einsum_args = einsum
                    ...
                    .map(|(node, ints)| {
                        ...
                        (on_ix(Index::new(node, index, None)), ints.clone())
                    })
                    .collect();
...
fn add_pull_concat(add: &Add, on_ix: &dyn Fn(Index) -> CircuitRc) -> Option<CircuitRc> {
    for (child, rank_difference) in add.nodes_and_rank_differences() {
        if let Circuit::Concat(concat) = &**child {
            if !concat.info().shape[concat.axis].is(1) && let Some(sections_here) = shape_try_as_known(&concat.get_sizes_at_axis()) {
...
fn generalfunction_pull_concat(
    ...
    let batch_rank =
        generalfunction.rank() - generalfunction.num_non_batchable_output_dims as usize;
    for operand in generalfunction.children() {
        if let Circuit::Concat(concat) = &**operand && concat.axis < batch_rank {
            ...
            let new_operands = concat
                .children()
                .map(|cnode| {
                    let span_here = cnode.info().shape[concat.axis].t()?;
...
pub fn concat_drop_size_zero(concat: &Concat) -> Option<Concat> {
    ...
    let new_nodes = concat
        .children()
        .filter(|x| {
            if x.info().shape[concat.axis].is(0) {
                did_anything = true;
                false
            } else {
                true
...
pub fn index_concat_drop_unreached(index: &Index) -> Option<CircuitRc> {
    ...
    if concat
        ...
        || concat.num_children() == 0
    {
        return None;
    }
    let axis_index = &index_canon[concat.axis];
    let operand_starts: Vec<usize> = cumsum(shape_try_as_known(
        &concat
            .children()
            .map(|x| x.shape()[concat.axis])
            .collect::<Vec<_>>(),
    )?);
...
pub fn split_to_concat(circuit: CircuitRc, axis: usize, sections: Vec<usize>) -> Concat {
    split_to_concat_cb(circuit, axis, &sections, &|x| x.rc())
}
...
pub fn concat_elim_split(circuit: &Concat) -> Option<Index> {
    ...
    let sections: Vec<USlice> = sections
        .iter()
        .map(|x| Some(x.to_uslice(child.info().shape[axis_lower].t()?)))
        .collect::<Option<_>>()?;
    let mut place = 0;
    for s in &sections {
        if s.start != place {
