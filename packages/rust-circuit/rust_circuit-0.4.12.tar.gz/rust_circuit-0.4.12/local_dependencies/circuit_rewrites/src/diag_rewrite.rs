                ...
                match &**node {
                    Circuit::Add(child) => {
                        ...
                        let new_add = Add::nrc(
                            explicit_add
                                .children()
                                .map(|node| Einsum::new_trace(node, ints.clone(), None).rc())
                                .collect(),
                            None,
                        );
                        did_anything = true;
                    ...
                    Circuit::Rearrange(child) => {
                        ...
                        let ints_for_intersection = (0..child.rank())
                            .map(|i| {
                    ...
                    Circuit::Concat(child) => {
                        ...
                        if deduped_here.len() != new_raw_ints.len() {
                            ...
                            let new_concat = Concat::nrc(
                                child
                                    .children()
                                    .map(|x| Einsum::new_trace(x, new_raw_ints.clone(), None).rc())
                                    .collect(),
                                new_concat_axis,
                                None,
                            );
