...
use circuit_base::{
    deep_map_op_context, deep_map_preorder_unwrap, deep_map_unwrap, get_compatible_dtype,
    on_circuit_names, prelude::*, visit_circuit_unwrap, Add, Index,
};
use circuit_utils::deep_replace_parents_fix;
    ...
    F: Fn(&Circuit, &mut OptimizationContext) -> Option<CircuitRc>,
{
        ...
        F: Fn(&Circuit, &mut OptimizationContext) -> Option<CircuitRc>,
    {
        ...
        let mut result: CircuitRc = circ
            .map_children_op(&mut |x: CircuitRc| fully_simplify(x, context, f))
            .map_or_else(
                || circ.clone(),
                |x| {
                    did_anything = true;
        ...
        let result = if did_anything { Some(result) } else { None };
        context
            .cache
            .simplified
    ...
    fully_simplify(circ.clone(), opt_context, &f).unwrap_or(circ)
}

#[pyfunction]
pub fn deep_push_down_index_raw(circ: CircuitRc, min_size: Option<usize>) -> CircuitRc {
    deep_replace_parents_fix(circ, |c, parents| {
        if min_size.is_some() && c.info().numel() < BigUint::from(min_size.unwrap()) {
            return None;
        }
        if parents.is_empty() || !parents.iter().all(|n| n.is_index()) {
            return None;
        }
        ...
        if parents.len() == 1 {
            ...
            if c.is_index() {
                ...
                return push_down_index_op(ix).map(|x| vec![x]);
            };
        }
        ...
        let (base_ix, new_ixs_t): (Vec<TensorAxisIndex>, Vec<Vec<TensorAxisIndex>>) = (0..c.rank())
            .map(|i| {
                let ixs = idxs.iter().map(|ix| ix.0[i].clone()).collect();
                if let Some(l) = c.shape()[i].t() {
                    TensorAxisIndex::indices_union_and_rebased(&ixs, l, ddt)
                } else {
                    (TensorAxisIndex::IDENT, ixs)
                }
            })
            .unzip();
        ...
        // can go None => cpu due to slices => tensor index, which is problematic w/ modules
        if base.info().device_dtype.device != c.info().device_dtype.device {
            return None;
        }
...
pub fn deep_heuristic_nest_adds(circ: CircuitRc) -> CircuitRc {
    ...
    let mut add_unions: Vec<(CircuitRc, usize)> = vec![];
    let mut add_ids: IndexSet<HashBytes> = IndexSet::default();
    ...
    visit_circuit_unwrap(circ.clone(), |c: CircuitRc| {
        if c.is_add() {
            if let Some(gr) = c
                ...
                .map(|x| {
                    let (ix, b) = add_ids.insert_full(x.info().hash);
                    if b {
                        uf.grow(1);
                    }
                    ix
            ...
            {
                add_unions.push((c.clone(), *gr))
            }
        }
    });

    let add_groups = add_unions.iter().into_group_map_by(|x| uf.find_(x.1));

    let mut mapping: HashMap<Add, Add> = HashMap::default();
    for add_group in add_groups.values() {
        if add_group.len() == 1 {
            break;
        }
        let mut seen_adds: Vec<(Add, CircBloomFilter)> = add_group
            .iter()
            .map(|(x, _)| {
                let y = x.as_add_unwrap().clone();
                let bl = child_bloom_filter(&y);
                (y, bl)
            })
            .collect();
                ...
                if circ.1.intersection_count(&circ2.1) >= 2 && circ.0 != circ2.0 {
                    let intersection = Add::new(
                        circ.0
                            .children_sl()
                            .iter()
                            .filter(|x| circ2.0.children_sl().contains(x))
                            .cloned()
                            .collect(),
                        None,
                    );
                    if intersection.num_children() >= 2
                        && intersection != circ.0
                        && intersection != circ2.0
                    {
                        intersections
                            .insert(intersection.clone(), child_bloom_filter(&intersection));
                    }
                }
            }
        }
        // println!(
        //     "add group {} elements + {} intersections",
        //     seen_adds.len(),
        //     intersections.len()
        // );
        ...
        for _ in 0..2 {
            ...
            for cand_sup in &seen_adds {
                if cand_sup.0.num_children() >= 3 {
                    ...
                    if timeout_timer.elapsed().as_millis() > 20 {
                        // println!("nest adds group timed out, continuing with current nesting");
                        break;
                    }
                }
            }
    ...
    deep_map_op_context(
        ...
        &|x: CircuitRc, context: &mut OptimizationContext| match &**x {
            Circuit::Einsum(ein) => {
                ...
                if result.is_err() {
                    Some(timed!(batch_einsum(ein, context).unwrap_or(x)))
                } else {
