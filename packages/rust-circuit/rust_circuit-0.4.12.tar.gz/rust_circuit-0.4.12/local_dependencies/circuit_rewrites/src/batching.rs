...
pub fn batch_to_concat(
    ...
    let l = circuit.info().shape[axis].unwrap_err()?;
    if batch_size == 0 {
    ...
    let circuit = deep_map_unwrap(circuit, |x| {
        match &**x {
            Circuit::Index(index) => Index::slice_edges_to_none(index).rc(),
            // Circuit::Scatter(scatter) => Scatter::nrc(
            //     scatter.node(),
            //     scatter.index.edges_to_none(&scatter.info().shape),
            ...
            module_spec
                ...
                .map(|inp_spec| {
                        ...
                        .find(|(_c, spec)| spec.symbol.uuid == inp_spec.symbol.uuid)
                    {
                        ...
                        result[leaves_axis[c]] = Size::known(axis_size);
                        return result;
                    }
        ...
        module_spec
            ...
            .map(|inp_spec| {
                    ...
                    .find(|z| z.1.symbol.uuid == inp_spec.symbol.uuid)
                {
                    return Index::nrc(
                        c.clone(),
                        TensorIndex::new_single(index.clone(), leaves_axis[c], c.rank()),
                        None,
                    );
                }
                input_specs
...
pub fn batch_einsum(einsum: &Einsum, context: &mut OptimizationContext) -> Result<CircuitRc> {
    ...
    let output_size = einsum.info().numel_usize();
    if output_size > max_single_mem {
        bail!("batch_einsum: einsum output size={:?} exceeds max_single_mem={:?}, so can't batch it\n{einsum:?}", oom_fmt(output_size), oom_fmt(max_single_mem))
    }
    ...
    for (i, intermediate) in intermediates_above_mem.iter().enumerate() {
        for v in intermediate.info().named_axes.values() {
    ...
    let batch_int_len = einsum.shape_map().unwrap()[&(batch_int as u8)].unwrap_err()?;
