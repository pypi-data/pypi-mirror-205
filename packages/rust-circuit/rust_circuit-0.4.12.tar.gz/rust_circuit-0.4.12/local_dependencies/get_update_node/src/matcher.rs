...
make_single_many!(TypeTags, CircuitType, HashSet);
make_single_many!(Names, Name, HashSet);
make_single_many!(Circuits, CircuitRc, HashSet);
...
impl MatcherData {
    ...
    fn item_hash(&self, hasher: &mut blake3::Hasher) {
        match self {
            ...
            Self::Name(x) => {
                for s in x.iter().map(|x| NameOrdById(*x)).sorted() {
                    hasher.update(&s.0.to_le_bytes());
                }
            }
            Self::Type(x) => {
                for t in x.iter().sorted() {
    ...
    pub fn validate_matched(&self, matched: &BTreeSet<CircuitRc>) -> Result<()> {
        fn run_on_set<T: Eq + hash::Hash + Ord + Debug + Clone, D: Display>(
            ...
            if !matcher_set.is_subset(found_set) {
                bail!(
                    ...
                    convert(matcher_set),
                    convert(found_set),
                    convert(&matcher_set.difference(found_set).cloned().collect())
                )
            } else {
                Ok(())
            }
        ...
        match &self.data {
            MatcherData::Name(names) => {
                let found_names = matched.iter().filter_map(|x| x.info().name).collect();
                run_on_set(names, &found_names, "names", "name", |x| format!("{:?}", x))
            }
            MatcherData::Type(types) => {
                let found_types = matched.iter().map(|x| x.type_tag()).collect();
...
impl Matcher {
    ...
    pub fn types(types: Vec<CircuitType>) -> Self {
        MatcherData::Type(types.into_iter().collect::<HashSet<_>>()).into()
    }

    #[staticmethod]
    #[pyo3(signature=(*circuits))]
    pub fn circuits(circuits: Vec<CircuitRc>) -> Self {
        MatcherData::EqM(circuits.into_iter().collect::<HashSet<_>>()).into()
    }
