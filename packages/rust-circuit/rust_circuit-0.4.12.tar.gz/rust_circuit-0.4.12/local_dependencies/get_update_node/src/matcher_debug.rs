...
use anyhow::Result;
use circuit_base::{CircuitNode, CircuitNodeUnion, CircuitRc};
use circuit_print::print::PrintOptions;
...
use crate::{
    iterative_matcher::{per_child, Chains, IterativeMatcher},
    matcher::{Matcher, MatcherData},
    IterateMatchResults, IterativeMatcherData, IterativeMatcherRc,
};

static MAX_SELF_SINGLE_LINE_WIDTH: Lazy<usize> = Lazy::new(|| {
...
impl Display for IterativeMatcher {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self.data() {
            ...
            IterativeMatcherData::Chains(chains) => repr_chains(chains, false),
...
fn repr_chains(chains: &Chains, started: bool) -> String {
    if let Some(x) = chains.as_single() {
        return format!("{}", &x.0);
    };
    let go_item = |x: &IterativeMatcher, y: &Chains| {
        maybe_indented_list(
            [format!("{}", &x), repr_chains(y, true)].to_vec(),
            " -> ",
            true,
            true,
        )
    };
    if !chains.found && chains.chains.len() == 1 {
        let x = &chains.chains.first_key_value().unwrap();
        let o = go_item(&x.0, &x.1);
        return if started { o } else { format!("[{}]", o) };
    }
    format!(
        "Any({}{})",
        if chains.found { "Term, " } else { "" },
        maybe_indented_list(
            chains
                .chains
                .iter()
                .sorted_by_key(|x| x.0.to_string())
                .map(|x| format!("[{}]", go_item(x.0, x.1)))
                .collect(),
            ", ",
            false,
            true
...
fn repr_single_many<T, F>(x: &HashSet<T>, f: F) -> String
where
    F: Fn(&T) -> String,
    T: std::cmp::Ord,
{
    ...
    format!(
        "{{{}}}",
        maybe_indented_list(x.iter().sorted().map(f).collect(), ", ", false, true)
    )
}
