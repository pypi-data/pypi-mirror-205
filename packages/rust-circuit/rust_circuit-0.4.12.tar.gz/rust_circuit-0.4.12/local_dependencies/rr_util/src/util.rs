...
use crate::{
    compact_data::TinyVecU8, eq_by_big_hash::EqByBigHash, make_lazy, name::Name, simple_from,
};
pub type AxisInt = u8;
...
pub fn dict_to_list<T: Clone>(dict: &HashMap<usize, T>, max: Option<usize>, default: T) -> Vec<T> {
    let max = max.unwrap_or(*dict.keys().max().unwrap_or(&0));
    let mut result = vec![default; max + 1];
...
pub fn cumsum<T: std::ops::Add<Output = T> + Default + Copy>(col: &[T]) -> Vec<T> {
    col.iter()
...
pub fn split_first_take<T: Default>(x: &mut [T]) -> Option<(T, impl Iterator<Item = T> + '_)> {
    x.split_first_mut()
        .map(|(l, r)| (mem::take(l), r.iter_mut().map(mem::take)))
}
...
#[derive(Clone, Debug)]
pub struct CowArcHash<T>(Arc<CowArcHashInner<T>>);

#[derive(Debug)]
struct CowArcHashInner<T> {
    hash: SyncUnsafeCell<HashBytes>,
    valid_hash: AtomicBool,
    x: T,
}

impl<T: Clone> Clone for CowArcHashInner<T> {
    fn clone(&self) -> Self {
        let x = Self {
            valid_hash: self.valid_hash.load(Ordering::Acquire).into(),
            hash: SyncUnsafeCell::new([0u8; 32]),
            x: self.x.clone(),
        };
        if self.valid_hash.load(Ordering::Acquire) {
            unsafe { *x.hash.get() = *self.hash.get() };
        }
        x
    }
}

impl<T: EqByBigHash> EqByBigHash for CowArcHash<T> {
    fn hash(&self) -> [u8; 32] {
        unsafe {
            if !self.0.valid_hash.load(Ordering::Acquire) {
                *self.0.hash.get() = self.0.x.hash();
                self.0.valid_hash.store(true, Ordering::Release);
            }
            *self.0.hash.get()
        }
    }
...
impl<T> CowArcHash<T> {
    ...
    where
        T: Clone,
    {
        // should be valid since user must drop the `&mut T` before calling hash
        let m = Arc::<CowArcHashInner<T>>::make_mut(&mut self.0);
        m.valid_hash = false.into();
        &mut m.x
    }

    pub fn new(x: T) -> CowArcHash<T> {
        CowArcHash(Arc::new(CowArcHashInner {
            hash: SyncUnsafeCell::new([0u8; 32]),
            valid_hash: false.into(),
            x: x,
        }))
    }
}

impl<T> Deref for CowArcHash<T> {
    type Target = T;
...
impl<T> std::ops::Deref for ArcCow<T> {
    type Target = T;
    fn deref(&self) -> &Self::Target {
        match self {
            Self::Arc(x) => x,
...
impl<T: Clone> std::ops::DerefMut for ArcCow<T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        match self {
            Self::Arc(x) => Arc::<T>::make_mut(x),
            Self::Owned(x) => x,
        }
    }
}
