...
#[cfg(test)]
use crate::shape::shape_from_known;
use crate::{
    ...
    shape::{Shape, Size, SIZE_MATCH},
    sv,
    tensor_util::{
        check_canon_axes, check_canon_idxs, PyParseError, TensorAxisIndex, TensorIndex, TorchDevice,
    },
    tu8v,
    util::{
...
pub fn check_canon_axes_as(rank: usize, axes: &[i64]) -> Result<RInnerInts> {
...
impl fmt::Display for RearrangeSpec {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        f.write_str(&self.to_string_impl(true, true))
    }
}
...
impl FromStr for RearrangeSpec {
    ...
    fn from_str(string: &str) -> Result<Self, Self::Err> {
        ...
        let mut f = |s: &str| -> Result<RInts> {
            let handle_size = |num: Size, sizes: &mut Shape| {
                sizes.push(num);
                new_int(sizes.len())
            };
            let parse_num = |s: &str| Ok(Size::from_str(s)?);
            let mut handle_string_nosize = |s: String, sizes: &mut Shape| {
            ...
            RE_TOP_LEVEL
                ...
                .map(|tl| {
                    if let Some(c) = tl.get(1) {
                        ...
                        Ok(tu8v![handle_size(num, &mut sizes)?])
                        ...
                        c.as_str()
                            ...
                            .map(|s| {
                                parse_num(s)
                                    .and_then(|z| handle_size(z, &mut sizes))
        ...
        Self::new(f(input)?, f(output)?, sizes)
            .with_context(|| format!("failed to construct rearrange spec after parsing {string}"))
    }
}
...
impl RearrangeSpec {
    #[new]
    pub fn new(input_ints: RInts, output_ints: RInts, int_sizes: Shape) -> Result<Self> {
    ...
    pub fn unflatten_axis(ndim: usize, axis: i64, shape: Shape) -> Result<Self> {
        ...
        let out_ints: RInts = (0..axis as u8)
            ...
            .map(|i| tu8v![i])
            .collect();
        let op_shape: Shape = iter::repeat(Size::NONE)
            .take(ndim - 1)
            .chain(shape)
            .collect();
    ...
    pub fn prepend_batch_shape(new_shape: Shape, old_rank: usize) -> Result<Self> {
        ...
        let out = Self::new(
            ...
            new_shape
                .into_iter()
                .chain(std::iter::repeat(Size::NONE).take(old_rank))
                .collect(),
        )
    ...
    pub fn to_einops_string(&self) -> String {
        self.to_string_impl(false, false)
    }
                            ...
                            if ints_in_other.contains(i) || self.int_sizes[*i as usize].is_none() {
                                    ...
                                    && self.int_sizes[*i as usize].is_some()
                                {
                                    format!(
                                        ...
                                        int_to_str(self.int_sizes[*i as usize].unwrap())
                                    )
                                } else {
                                    ALPHABET[*i as usize].to_owned()
                                }
    ...
    pub fn add_batch_dims(&self, batch_rank: usize) -> Self {
        ...
        Self::new(
            ...
            self.int_sizes
                .iter()
                .cloned()
                .chain(vec![Size::NONE; batch_rank].into_iter())
                .collect(),
        )
        .unwrap()
    }
    ...
    pub fn shapes(&self) -> Result<(Shape, Shape)> {
        Ok((
            self.input_ints
                .iter()
                .map(|x| x.iter().map(|y| self.int_sizes[*y as usize]).product())
                .collect(),
            self.output_ints
                .iter()
                .map(|x| x.iter().map(|y| self.int_sizes[*y as usize]).product())
    ...
    /// ```
    /// # use rr_util::{sv,tu8v, rearrange_spec::RearrangeSpec, shape::Size};
    ...
    ///     RearrangeSpec::new(sv![tu8v![0], tu8v![]], sv![tu8v![0]], sv![Size(3)]).unwrap();
    ...
    pub fn canonicalize(&self, special_case_ones: bool) -> Self {
        ...
        let mut new_int_sizes: Shape = sv![];
                    ...
                    for &i in single_dim.iter() {
                        if let Some(&new_i) = map.get(&i) {
                            ...
                            if let Some(old_size) = int_sizes[i as usize] {
                                ...
                                new_int_sizes[new_int_sizes_idx] =
                                    Some(new_int_sizes[new_int_sizes_idx].unwrap() * old_size)
                                        .into();
                            }
    ...
    fn fill_empty_ints_impl(&self, allow_rust_invalid: bool) -> Result<RearrangeSpec> {
        ...
        let result = RearrangeSpec {
            ...
            int_sizes: self
                ...
                .iter()
                .cloned()
                .chain(vec![Size::known(1); (next_int - first_int_to_add) as usize])
                .collect(),
        };
...
impl RearrangeSpec {
    ...
    pub fn new_canon(input_ints: RInts, output_ints: RInts, int_sizes: Shape) -> Self {
    ...
    pub fn expand_to(&self, shape: &Shape) -> Result<RearrangeSpec> {
        ...
        for (dim, (inner_ints, &expand_size)) in izip!(&self.input_ints, shape).enumerate() {
            if inner_ints
                .iter()
                .any(|x| !self.int_sizes[*x as usize].is_known())
            {
            ...
            let current_size = inner_ints
                ...
                .product::<usize>();
            if expand_size.is(current_size) {
                continue;
            }
            if inner_ints.len() == 1 {
                // change to wild card if single non-matching int
                int_sizes[inner_ints[0] as usize] = Size::NONE;
                continue;
            }
            if let Some(expand_size) = expand_size.t() {
                ...
                let non_dividing_idx = divides_size
                    ...
                    .unwrap();
                int_sizes[inner_ints[non_dividing_idx] as usize] = Size::NONE;
            }
        }
        ...
        for (ints, &l) in zip(&self.input_ints, shape) {
            if let Some(l) = l.t() {
                let none_indices: Vec<usize> = ints
                    .iter()
                    .enumerate()
                    .filter(|(_i, x)| !self.int_sizes[**x as usize].is_known())
                    .map(|(i, _x)| i)
                    .collect();
                let non_wildcard_size: usize = ints
                    ...
                    .product();
                let has_wildcard = !none_indices.is_empty();
                ...
                if has_wildcard {
                    if none_indices.len() > 1 {
                        ...
                        int_sizes[ints[none_indices[0]] as usize] =
                            Size::known(l / non_wildcard_size);
                    }
                }
            }
        }
    ...
    pub fn expand(
        ...
        RearrangeSpec::new(
            ...
            dict_to_list(&int_sizes_usize, None, 0)
                .into_iter()
                .map(Size::some)
                .collect(),
        )
    ...
    ///     sv![tu8v![0], tu8v![1]],
    ///     sv![tu8v![0, 1]],
    ///     sv![Size::known(2), Size::known(3)],
    /// )
    /// .unwrap();
    /// let outer = RearrangeSpec::new(
    ///     sv![tu8v![0]],
    ///     sv![tu8v![0], tu8v![1]],
    ///     sv![Size::known(6), Size::known(4)],
    /// )
    /// .unwrap();
    /// let fused = RearrangeSpec::new(
    ///     sv![tu8v![0], tu8v![1]],
    ///     sv![tu8v![0, 1], tu8v![2]],
    ///     sv![Size::known(2), Size::known(3), Size::known(4)],
    /// )
    /// .unwrap();
    ...
    pub fn fuse(inner: &Self, outer: &Self) -> Result<Self> {
        ...
        Ok(Self::new(
            input_ints,
            output_ints,
            outer_sizes.iter().map(|&x| Size::some(x)).collect(),
        )
        .unwrap()
        .canonicalize(true))
    }
    ...
    fn expand_at_axes_impl(
        ...
        Ok(Self::new(
            ...
            std::iter::repeat(Size::NONE)
                .take(orig_ndim)
                .map(Ok)
                .chain(sorted_counts.iter().map(|x| Ok(Size::some(*x))))
                .collect::<Result<_>>()?,
        )
        .unwrap())
    }
    ...
    pub fn index_before(
        ...
        let spec = RearrangeSpec {
            int_sizes: self
                ...
                .iter()
                .enumerate()
                .map(|(i, l)| Some(int_indices[&(i as u8)].axis_len(l.unwrap())).into())
                .collect(),
            input_ints: self
                .input_ints
                .iter()
...
#[test]
fn test_expand_to() {
    let s = shape_from_known;

    let spec = RearrangeSpec::from_str("a->a").unwrap();
    assert_eq!(spec.expand_to(&s(&[8])).unwrap(), spec);
    assert_eq!(spec.expand_to(&s(&[2])).unwrap(), spec);

    let spec_sized = RearrangeSpec::from_str("a:5->a").unwrap();
    assert_eq!(spec_sized.expand_to(&s(&[5])).unwrap(), spec_sized);
    assert_eq!(spec_sized.expand_to(&s(&[2])).unwrap(), spec);
    assert_eq!(spec_sized.expand_to(&s(&[6])).unwrap(), spec);

    let spec = RearrangeSpec::from_str("(a:5) b (k:2 y:7)-> (k a b) y").unwrap();
    assert_eq!(spec.expand_to(&s(&[5, 3, 14])).unwrap(), spec);
    assert_eq!(
        spec.expand_to(&s(&[2, 3, 14])).unwrap(),
        RearrangeSpec::from_str("a b (k:2 y:7)-> (k a b) y").unwrap()
    );
    assert_eq!(
        spec.expand_to(&s(&[7, 3, 14])).unwrap(),
        RearrangeSpec::from_str("a b (k:2 y:7)-> (k a b) y").unwrap()
    );
    assert_eq!(
        spec.expand_to(&s(&[7, 3, 21])).unwrap(),
        RearrangeSpec::from_str("a b (k y:7)-> (k a b) y").unwrap()
    );
    assert_eq!(
        spec.expand_to(&s(&[7, 3, 18])).unwrap(),
        RearrangeSpec::from_str("a b (k:2 y)-> (k a b) y").unwrap()
    );
    assert!(spec.expand_to(&s(&[5, 3, 5])).is_err());
    assert!(spec.expand_to(&s(&[2, 5, 3, 14])).is_err());
    assert!(spec.expand_to(&s(&[5, 3, 28])).is_err());
}
...
pub enum RearrangeSpecError {
    ...
    #[error(
        "expanding to expand_size={expand_size} (!= current_size={current_size}) {} {}\n{s}\n{c}\n({e_name})",
        "was ambiguous for",
        format!(
