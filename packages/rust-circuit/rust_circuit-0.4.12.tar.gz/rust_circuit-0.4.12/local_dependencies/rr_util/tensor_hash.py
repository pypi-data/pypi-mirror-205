...
def get_cuda_kernels(device_id):
...
def hash_tensor_contents_gpu(tensor):
    ...
    torch.cuda.default_stream(torch.device).wait_stream(
        torch.cuda.current_stream(tensor.device)
    )  # in case user has changed the stream?
    ...
    _gpuhash, _gpuhash_finalize, ctx = get_cuda_kernels(tensor.device.index)
    ctx.push()
    _gpuhash(
        tensor,
...
if __name__ == "__main__":
    ...
    tensor1 = torch.zeros(4_000_000_000, dtype=torch.float32, device="cuda:1")
    import time

    for n in range(3):
        t = time.time()
        tensor1[10 + n] = 1.0
        tensor1[0:10] = torch.zeros(10, dtype=torch.float32, device="cuda:0")
        h = hash_tensor_contents_gpu(tensor1)
