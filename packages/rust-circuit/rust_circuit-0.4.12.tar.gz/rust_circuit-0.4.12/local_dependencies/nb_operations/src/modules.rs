...
use expand_node::{expand_node, replace_expand_map, MapReplaceExpander, ReplaceMapRc};
...
use rr_util::{
    ...
    rearrange_spec::{RearrangeSpec, RearrangeSpecError},
    shape::Size,
    sv, tu8v, unwrap,
        ...
        .map(|(((items, cum_batch_shape), m), overall_mod_batch_shape)| {
            (*items)
                ...
                .map(|(((node, orig_node), arg_spec), orig_batch_shape)| {
                    ...
                    let any_batching = extra_dims > 0 || !orig_batch_shape.is_empty();
                    let needs_pad_orig = missing_ndims > 0 && extra_dims > 0;
                    let new_node = if (any_batching && !cum_batch_shape.is_empty())
                        || needs_pad_orig
                    {
                        ...
                        let mut sizes = sv![Size::NONE; maker.running];
                        for i in pad_for_orig.clone() {
                            sizes[i] = overall_mod_batch_shape[i - pad_for_orig.start];
                        }
                        for i in cum.clone() {
                            sizes[i] = cum_batch_shape[i - cum.start];
                        }
                        ...
                        Rearrange::try_new(node, spec, rep_name)?.rc()
                    } else {
                        node
                    };
...
impl NestedModuleItems {
    fn new(
        ...
        let flat = fuse_modules_impl(&modules, &new_nodes_flat)
            .with_context(|| format!("fusing modules {modules:#?}"))?;
...
impl ModulePusher {
    fn push_down_modules_impl(
        ...
        let out = self
            ...
            .map(|x| x.0)
            .context("pushing down modules")?;
        if let CircOrGet::Circ(ref c) = out {
            assert_eq!(c.shape(), circuit.shape());
        }
        Ok(out)
    }
    ...
    fn push_down_modules_rec(
        ...
        let (found_get, all_finished, new_get) = get.match_iterate_continue(circuit.clone())?;
        ...
        let (found_skip_module, _, new_skip_module) =
            skip_module.match_iterate_continue(circuit.clone())?;
            ...
            .and_then(|x| (!found_skip_module).then_some(x))
        {
            ...
            let (out, (termed_prior_iter, mut prior_inner_circuits)) = rec(
                ...
                &items.push(m.clone(), new_nodes_flat, new_nodes_base)?,
                extra_replacements,
            )?;
            if mode == PushDownMode::PushOverride {
                ...
                let out = if mode == PushDownMode::Circ {
                    let out = out.unwrap();
                    let m_shape = m.shape();
                    if m_shape.len() > out.ndim() {
                        Some(out.repeat_to(
                            m_shape.clone(),
                            m.info().name.map(|x| format!("{} rep_batch", x).into()),
                        )?)
                    } else {
                        Some(out)
                    }
    ...
    fn build_module_flat_no_call(
        ...
        let name = self
            ...
            .context("namer failed in push down module")?;
        Ok(Module::new(nodes, ModuleSpec { circuit, arg_specs }, name))
    }
...
impl ModuleConstructUpdateBindings {
    ...
    fn bind_module(
        ...
        m: Module,
        count_mods: usize,
        outer_updated_syms: IndexSet<CircuitRc>,
        ...
        let (new_spec_circuit, base_circuit, prior_modules) = if count_mods > 1 && !self.is_flatten
        {
            self.bind_module(
                ...
                outer_updated_syms
                    .into_iter()
                    .chain(arg_specs.iter().map(|x| x.symbol.crc()))
                    .collect(),
                &applied_modules[..applied_modules.len() - 1],
                overall_count,
            )?
        } else {
            (m.spec.circuit.clone(), m.spec.circuit, Vec::new())
        };
        ...
        if !m_out.aligned_batch_shape().is_empty() {
...
impl SymbolExtractor {
    ...
    fn extract_symbols_rec(
        ...
        let (_, finished, traversal_per_child) = traversal.match_iterate_continue(circ.clone())?;
        ...
        let (new_circ, traversal_per_child) = if let Some(m) = circ.as_module() {
            ...
            let traversal_per_child = iter::once(spec_traversal)
                ...
                .collect();
            (circ.clone(), traversal_per_child)
        } else {
            (circ.clone(), traversal_per_child)
        };
        ...
        let per_child_syms = if !freshly_bound_syms.is_empty() {
            ...
            iter::once(SymbolSetRc::new(removed))
                ...
                .take(new_circ.num_children())
                .collect()
        } else {
            vec![symbols.clone(); new_circ.num_children()]
        };

        assert_eq!(per_child_syms.len(), new_circ.num_children());
        assert_eq!(traversal_per_child.len(), new_circ.num_children());
        let new_circ = new_circ.map_children_enumerate(|i, c| {
            let (new, map) =
                self_.extract_symbols_rec(c, &per_child_syms[i], traversal_per_child[i].clone())?;
            for (_, (new_circ, arg_spec)) in map {
                add_sym_circ(&mut out_syms, arg_spec, new_circ)?;
            }
            Ok(new)
        })?;
...
#[pyfunction]
pub fn pull_out_of_modules(
    circuit: CircuitRc,
    matcher: IterativeMatcherRc,
    update_new_binds: MaybeUpdate,
    matcher_new_binds: Option<IterativeMatcherRc>, /* different matcher for new binds? defaults to matcher */
    module_construct_callback: Option<PyObject>,
    circuit_to_arg_spec: Option<PyObject>,
) -> Result<CircuitRc> {
    let (a, b) = pull_out_of_modules_get(
        circuit,
        matcher,
        update_new_binds,
        matcher_new_binds,
    ...
    if !b.is_empty() {
        bail!("pull_out_of_modules: still pulling circuits before returning (call pull_out_of_modules_extract to get results, or use a matcher that deposits them earlier) {:?}", b)
    }
    Ok(a)
}

#[pyfunction]
pub fn pull_out_of_modules_get(
    circuit: CircuitRc,
    matcher: IterativeMatcherRc,
    update_new_binds: MaybeUpdate,
    matcher_new_binds: Option<IterativeMatcherRc>, /* different matcher for new binds? defaults to matcher */
    module_construct_callback: Option<PyObject>,
    circuit_to_arg_spec: Option<PyObject>,
) -> Result<(CircuitRc, Vec<(CircuitRc, ModuleArgSpec)>)> {
    #[apply(cached_lambda)]
    #[key((circ.info().hash, matcher.clone(), matcher_new_binds.clone()),
          (HashBytes, IterativeMatcherRc, IterativeMatcherRc))]
    #[use_try]
    fn recurse(
        circ: CircuitRc,
        matcher: IterativeMatcherRc,
        matcher_new_binds: IterativeMatcherRc,
    ) -> Result<(CircuitRc, Vec<(CircuitRc, ModuleArgSpec)>)> {
        let (found, finished, new_matcher) = matcher.match_iterate_continue(circ.clone())?;
        let (_, _, new_matcher2) = matcher_new_binds
            .clone()
            .match_iterate_continue(circ.clone())?;
        if found {
            let spec = if let Some(cb) = &circuit_to_arg_spec {
                pycall!(cb, (circ.clone(),))
            } else {
                ModuleArgSpec::new(
                    Symbol::new(
                        circ.shape().clone(),
                        Uuid::from_slice(&circ.info().hash[..16]).unwrap(),
                        circ.info().name,
                    ),
                    true,
                    true,
            ...
            Ok((spec.symbol.clone().into(), vec![(circ, spec)]))
        } else if finished {
            Ok((circ, vec![]))
        } else if let Some(m) = circ.clone().as_module() {
            let (spec_circ_2, rs) = recurse(
                m.spec.circuit.clone(),
                new_matcher[0].clone(),
                new_matcher2[0].clone(),
            )?;

            let vars: IndexSet<CircuitRc> =
                m.spec.arg_specs.iter().map(|s| s.symbol.crc()).collect();

            let (rs_with_fvs, rs_no_fvs): (Vec<_>, _) = rs
                .iter()
                .cloned()
                .partition(|(x, _)| is_intersecting_free_syms(x, &vars));

            let m_new = if !rs_with_fvs.is_empty() {
                let rs_fvs_new = itertools::process_results(
                    rs_with_fvs.iter().map(|(c, y)| -> Result<_> {
                        let x = replace_expand_map(
                            c.clone(),
                            m.spec
                                .arg_specs
                                .iter()
                                .zip(m.args())
                                .map(|(s, x)| (s.symbol.crc(), x.clone()))
                                .collect(),
                        )?;
                        Ok((
                            (
                                update_new_binds.call(x.clone())?.unwrap_or(x),
                                y.symbol.info().name,
                            ),
                            y,
                        ))
                ...
                let spec_circ_3 = circuit_utils::replace_nodes(
                    spec_circ_2.clone(),
                    &rs_fvs_new
                        .iter()
                        .flat_map(|(_, v)| {
                            v[1..]
                                .iter()
                                .map(|w| (v[0].symbol.info().hash, w.symbol.crc()))
                        })
                        .collect(),
                );
                ...
                Module::try_new(
                    m.args()
                        .chain(rs_fvs_new.iter().map(|((x, _), _)| x))
                        .cloned()
                        .collect(),
                    ModuleSpec {
                        circuit: spec_circ_3,
                        arg_specs: m
                            ...
                            .arg_specs
                            .iter()
                            .chain(rs_fvs_new.iter().map(|(_, b)| b[0]))
                            .cloned()
                            .collect(),
                    },
                    m.info().name,
            ...
            let (_, _, new_matcher2_again) =
                matcher_new_binds.match_iterate_continue(m_new.clone())?;

            let mut out = rs_no_fvs;
            let circ_out = m_new.map_children_enumerate(|i, c| {
                Ok(if i == 0 {
                    if rs_with_fvs.is_empty() {
                        spec_circ_2.clone()
                    } else {
                        c
                    }
            ...
            let circ_out = if !rs_with_fvs.is_empty() && let Some(cb) = &module_construct_callback {
                pycall!(cb, (circ_out, circ,), anyhow)?
            } else {
                circ_out
            };
            ...
            let mut out = Vec::new();
            let circ_out = circ.map_children_enumerate(|i, c| {
                let (x, y) = recurse(c, new_matcher[i].clone(), new_matcher2[i].clone())?;
                out.extend(y);
                Ok(x)
            })?;
    ...
    recurse(
        circuit,
        matcher.clone(),
        matcher_new_binds.unwrap_or(matcher),
    )
}
