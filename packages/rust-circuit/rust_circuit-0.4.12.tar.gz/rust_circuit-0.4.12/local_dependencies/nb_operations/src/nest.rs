...
macro_rules! nest_spec_for {
    ($name:ident [$($deriv:ident ,)*] {
        $(
            $field:ident : $field_ty:ty,
        )*
    }) => {
    paste::paste!{
    #[derive(Clone, Debug, $($deriv)*)]
...
impl NestInfo for NestEinsumsInfo {
    ...
    fn to_full(self, orig_full: Option<&Self::Full>) -> Self::Full {
        let out_axes = orig_full.and_then(|x| x.out_axes.clone());
        Self::Full {
            name: self.name,
            out_axes,
...
impl<I: NestFullInfo> NestIdxsItem<I> {
    fn all_indices(&self) -> Vec<usize> {
        match self {
            Self::Single(x) => vec![*x],
            Self::Many(x) => x.iter().flat_map(|x| x.idxs.all_indices()).collect(),
        }
    }
...
impl<I: NestInfo> NestSpec<I> {
    ...
    pub fn get_full(
        ...
        match self {
            Self::Info(NestSpecInfo { info, .. }) => info.clone().to_full(orig_info),
            Self::Sub(_) => orig_info.cloned().unwrap_or_default(),
        }
    }
    ...
    pub fn convert_to_named_idxs_impl(
        ...
        let find_rest = |ints: &[usize]| Self::find_rest(ints.iter().cloned().collect(), mapping);
    ...
    Circuit: AsOp<Full::CircuitType>,
{
    ...
    let (updated_current_matcher_to_pair, found): (Vec<_>, Vec<_>) = current_matcher_to_pair
        .into_iter()
        .map(|m| {
            let (found, _, new) = m.match_iterate_continue(circ.clone())?;
            Ok((new, found))
        })
        .collect::<Result<Vec<_>>>()?
        .into_iter()
    ...
    let rec_on_children = x
        ...
        .map(|((c, new_matcher), updated_matchers_to_pair)| {
            flatten_impl(c, new_matcher, updated_matchers_to_pair)
        })
        .collect::<Result<Vec<_>>>()?;
    let x = x
