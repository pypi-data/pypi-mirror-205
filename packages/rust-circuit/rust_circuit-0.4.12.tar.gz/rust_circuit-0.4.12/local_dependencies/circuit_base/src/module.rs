...
use rr_util::{
    ...
    shape::{right_align_shapes, shape_is_known, Shape, Size},
    util::{arc_unwrap_or_clone, counts_g_1, is_unique, HashBytes},
    IndexSet,
};
...
use crate::{
    ...
    circuit_info::{get_free_symbols, get_raw_free_symbols, CachedCircuitInfo},
    ...
    circuit_utils::{
        ...
        deep_map_preorder_unwrap, visit_circuit_unwrap,
    },
    constant::Symbol,
    named_axes::{deep_strip_axis_names, get_axis_leaves, propagate_named_axes},
    new_rc_unwrap,
    prelude::*,
    self_funcs::replace_expand_map,
    PyCircuitBase, Rearrange,
};
...
impl ModuleSpec {
    ...
    pub fn batch_shapes<'a>(&self, nodes: &'a [CircuitRc]) -> Vec<&'a [Size]> {
    ...
    pub fn out_shape(&self, nodes: &[CircuitRc]) -> Result<Shape> {
        let bs = self.aligned_batch_shape(nodes)?;
        return Ok([&bs[..], self.circuit.shape()].concat().to_smallvec());
    }

    pub fn check_expand_valid_shapes(
        &self,
        nodes: &[CircuitRc],
        check_inherited_restrictions: bool, /* currently only checked on module construction, not on resize */
    ) -> Result<()> {
        ...
        let spec_restrictions =
            check_inherited_restrictions.then(|| get_symbol_restrictions(&self.circuit));
        for (arg_spec, node) in izip!(self.arg_specs.iter(), nodes) {
            let (a, b, c) = spec_restrictions
                .as_ref()
                .and_then(|m| m.get(&arg_spec.symbol).cloned())
                .unwrap_or((true, true, false));
            let batchable = a && arg_spec.batchable;
            let expandable = b && arg_spec.expandable;
            let ban_non_symbolic_size_expand = c || arg_spec.ban_non_symbolic_size_expand;

            if node.rank() < arg_spec.symbol.info().rank() {
                bail!(ConstructError::ModuleRankReduced {
                    node_rank: node.rank(),
            ...
            if !batchable && node.rank() > arg_spec.symbol.info().rank() {
                bail!(ConstructError::ModuleTriedToBatchUnbatchableInput {
                    node_rank: node.rank(),
                    symbol_rank: arg_spec.symbol.rank(),
                    arg_spec: arg_spec.clone(),
                    spec_circuit: self.circuit.clone()
            ...
            if !expandable
                && node.shape()[node.rank() - arg_spec.symbol.rank()..]
                    != arg_spec.symbol.shape()[..]
            {
                bail!(ConstructError::ModuleTriedToExpandUnexpandableInput {
                    node_shape: node.shape().clone(),
            ...
            if ban_non_symbolic_size_expand {
                for (dim, (&new_size, &old_size)) in node.shape()
                    ...
                    .enumerate()
                {
                    if new_size != old_size && old_size.is_known() {
                        bail!(
                            ConstructError::ModuleTriedToExpandOnNonSymbolicSizeAndBanned {
                                old_size,
                                new_size,
                                dim,
                                node_shape: node.shape().clone(),
    ...
    pub fn expand_shape(&self, shapes: &Vec<Shape>) -> Result<(CircuitRc, Vec<HashBytes>, String)> {
        ...
        let symbols = shapes
            ...
            .collect();
        let result = self.expand_raw(&symbols).with_context(|| {
            format!(
                "module spec {:?} expansion failed for argument shapes {shapes:?}",
                self.circuit.info().name
            )
        })?;
    ...
    pub fn check_no_ident_matching_issues(&self) -> Result<()> {
        let my_idents: HashMap<_, _> = self
            ...
            .collect();
        let sym_single_map = self.circuit.is_symbol().then(|| {
            [self.circuit.clone()]
                .into_iter()
                .collect::<IndexSet<CircuitRc>>()
        });
        let syms = sym_single_map
            .as_ref()
            .unwrap_or_else(|| &get_raw_free_symbols(&self.circuit));

        for sym_here in syms {
            let sym_here_ = sym_here.as_symbol_unwrap();
            if let Some(&my_sym) = my_idents.get(&sym_here_.ident()) {
                if my_sym != sym_here_ {
                    bail!(
                        SubstitutionError::FoundNEQFreeSymbolWithSameIdentification {
                            sym: my_sym.clone(),
                            matching_sym: sym_here_.clone()
                        }
                    )
                }
            }
    ...
    pub fn is_batchable_to_inputs(&self, dim: usize) -> Option<Vec<usize>> {
        let (_, _, leavs_axis, leaves_non_axis) =
            get_axis_leaves(self.circuit.clone(), dim).ok()?;
        dbg!((&dim, &leavs_axis, &leaves_non_axis));
        None
    }
        ...
        fn recurse(circ: CircuitRc, symbols: &SymbolSetRc) -> Result<Option<CircuitRc>> {
            ...
            let new = if let Some(m) = circ.as_module() {
                let mut removed: BTreeSet<_> = symbols
                    .iter()
                    .filter(|x| get_raw_free_symbols(&circ).contains(&x.crc()))
                    .cloned()
                    .collect();
...
pub fn py_get_free_symbols(circuit: CircuitRc) -> Vec<CircuitRc> {
...
impl ModuleSpec {
    ...
    pub fn new_free_symbols(circuit: CircuitRc, check_unique_arg_names: bool) -> Result<Self> {
        let arg_specs = get_free_symbols(&circuit)
            .iter()
            .map(|x| ModuleArgSpec::new(x.as_symbol_unwrap().clone(), true, true, true))
            .collect();
        let out =
...
impl Module {
    ...
    pub fn try_new(nodes: Vec<CircuitRc>, spec: ModuleSpec, name: Option<Name>) -> Result<Self> {
        ...
        let mut substitute_cached_info = ModuleSpec {
            // substituing all to avoid exponential worst case (and also to avoid exponential due to expand_node rerun)
        ...
        .info()
        .clone();
        // need to check because we substituted all
        spec.check_expand_valid_shapes(&nodes, true).with_context(|| format!("module construction name={name:?} failed due to nested module with stronger restrictions"))?;
        ...
        // - If the spec.circuit is a single free symbol, it isn't counted
        ...
        substitute_cached_info.free_symbols = if free_syms.is_empty() {
            ...
            let b = OnceBox::new();
            let _ = b.set(Box::new(Arc::new(free_syms)));
            Some(b)
        };
    ...
    fn child_axis_map_inputs_uncached(&self) -> Result<Vec<Vec<Option<usize>>>> {
        ...
        let spec_circuit_resized = substitute_all_modules(spec_resized.circuit);
        ...
        let mut seen: Vec<bool> = vec![false; self.num_children() - 1];
        let mut axes_dead: Vec<bool> = vec![false; self.rank()];
        let mut result: Vec<Vec<Option<usize>>> =
            self.args().map(|c| vec![None; c.rank()]).collect();
        ...
        visit_circuit_unwrap(out_named, |x| {
            if let Some(sym) = x.as_symbol() {
                if let Some(i) = self.spec.arg_specs.iter().position(|x| {
                    ...
                    let mut vec = vec![None; sym.rank()];
                    for (k, v) in &sym.info().named_axes {
                        vec[*k as usize] = Some(v.parse::<usize>().unwrap());
                    }
                    if !seen[i] {
                        result[i] = vec;
                        seen[i] = true;
                        ...
                        for (k, v) in vec.iter().enumerate() {
                            if result[i][k] != *v {
                                result[i][k] = None;
                                if let Some(ax) = (*v).or(result[i][k]) {
                                    axes_dead[ax] = true;
                                }
                            }
                        }
                ...
                let mut children_named_axes: HashSet<Name> = HashSet::default();
                for m in x.children() {
                    children_named_axes.extend(m.info().named_axes.values())
                }
                for v in x.info().named_axes.values() {
                    if !children_named_axes.contains(v) {
                        axes_dead[v.parse::<usize>().unwrap()] = true;
                    }
                }
            }
        });
...
impl CircuitNode for Module {
    ...
    fn child_axis_map(&self) -> Vec<Vec<Option<usize>>> {
        ...
        once(vec![None; self.spec.circuit.rank()])
            .chain(map_inputs)
            .collect()
    }
...
impl Module {
    ...
    fn new_py(
        ...
        Ok(Module::new_kwargs(&kwargs.unwrap_or_default(), spec, name)?.into_init())
    }
    ...
    pub fn child_axis_map_inputs(&self) -> Result<Vec<Vec<Option<usize>>>> {
        let key = (
        ...
        // could also exclude batch dim from shape & re-add them here
...
pub fn any_modules(circuit: CircuitRc) -> bool {
    let mut b = false;
    visit_circuit_unwrap(circuit, |x| {
        if x.is_module() {
            b = true;
...
pub fn has_free_sym(circuit: &CircuitRc, sym: &Symbol) -> bool {
    if let Some(other) = circuit.as_symbol() {
        ...
        get_raw_free_symbols(circuit).contains(&sym.crc())
    }
}

pub fn is_intersecting_free_syms(circuit: &CircuitRc, syms: &IndexSet<CircuitRc>) -> bool {
    if circuit.is_symbol() {
        syms.contains(circuit)
    } else {
        !get_raw_free_symbols(&circuit).is_disjoint(syms)
    }
}
...
fn get_symbol_restrictions(circ: &Circuit) -> HashMap<Symbol, (bool, bool, bool)> {
    if circ.num_children() == 0 {
        return HashMap::default();
    }
    if let Some(r) = SYMBOL_RESTRICTIONS.with(|c| c.borrow().get(&circ.info().hash).cloned()) {
        return r;
    }

    let merge = |out: &mut HashMap<_, _>, s: Symbol, bs| {
        let (a, b, c) = bs;
        if let Some(&(d, e, f)) = out.get(&s) {
            out.insert(s, (a && d, b && e, c || f));
        } else {
            out.insert(s, bs);
        }
    };
    ...
    for c in circ.children_sl() {
        for (s, bs) in get_symbol_restrictions(c) {
            merge(&mut out, s, bs);
        }
    }
    if let Some(m) = circ.as_module() {
        let spec_restrictions = get_symbol_restrictions(&m.spec.circuit);
        for (c, arg_spec) in zip(m.args(), &m.spec.arg_specs) {
            let (x, y, z) = spec_restrictions
                .get(&arg_spec.symbol)
                .unwrap_or(&(true, true, false));
            let batchable = arg_spec.batchable && *x;
            let expandable = arg_spec.expandable && *y;
            let ban_non_symbolic_size_expand = arg_spec.ban_non_symbolic_size_expand || *z;
            if !batchable || !expandable || ban_non_symbolic_size_expand {
                for sym in (*get_free_symbols(c)).iter() {
                    merge(
                        &mut out,
                        sym.as_symbol_unwrap().clone(),
                        (batchable, expandable, ban_non_symbolic_size_expand),
                    )
                }
            }
        }
    ...
    SYMBOL_RESTRICTIONS.with(|c| c.borrow_mut().insert(circ.info().hash, out.clone()));
...
thread_local! {
    ...
    static SYMBOL_RESTRICTIONS: RefCell<HashMap<HashBytes, HashMap<Symbol, (bool, bool, bool)>>> = RefCell::new(Default::default());
}
