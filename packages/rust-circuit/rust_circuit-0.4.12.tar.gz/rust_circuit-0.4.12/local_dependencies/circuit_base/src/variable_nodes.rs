...
use rr_util::{
    name::Name,
    shape::{shape_eq_if_known, Shape, Size},
    sv,
};
use uuid::Uuid;

use crate::{
    auto_name::{child_name_with_maybe_paren, OperatorPriority},
    ...
    CircuitRc, ConstructError, PyCircuitBase, Scalar,
};
...
impl Tag {
    ...
    pub fn new(node: CircuitRc, uuid: Uuid, name: Option<Name>) -> (Self) {
        Self {
            uuid,
            info: CachedCircuitInfo::incomplete(name, node.shape().clone(), vec![node]).unwrap(),
        }
        .initial_init_info()
        .unwrap()
    }
...
impl CircuitNode for Tag {
    ...
    fn child_axis_map(&self) -> Vec<Vec<Option<usize>>> {
        vec![(0..self.node().rank()).map(Some).collect()]
    }
...
impl DiscreteVar {
    ...
    pub fn try_new(
        ...
        if !values.info().shape[0].eq_if_known(probs_and_group.info().shape[0]) {
    ...
    pub fn new_for_py(
        ...
        let size = values.shape()[0];
        if !size.is_known() {
            bail!(ConstructError::DiscreteVarSamplesDimUnknownSize {});
        }
        let probs_and_group = probs_and_group
            .unwrap_or_else(|| Self::uniform_probs_and_group(size.unwrap(), None).rc()); // TODO: name
...
impl DiscreteVar {
    ...
    pub fn uniform_probs_and_group(size: usize, name: Option<Name>) -> Tag {
        ...
        Tag::new_with_random_uuid(
            Scalar::nrc(1.0 / (size as f64), sv![Size::known(size)], None),
            name,
        )
    }
    #[getter]
...
impl StoredCumulantVar {
    ...
    pub fn try_new(
        ...
        for (k, v) in cumulants.iter() {
            let shape_here: Shape = shape
                ...
                .copied()
                .collect();
            if !shape_eq_if_known(&shape_here, v.shape()) {
        ...
        let out = Self {
            cumulant_ixs: cumulants.keys().copied().collect(),
            uuid,
            info: CachedCircuitInfo::incomplete(
                name,
                cumulants[&1].info().shape.clone(),
                cumulants.values().cloned().collect(),
            )?,
        };
    ...
    pub fn new_mv(
        ...
        Self::try_new(higher_cumulants, uuid.unwrap_or_else(Uuid::new_v4), name)
    }
}
...
impl CircuitNode for StoredCumulantVar {
    ...
    fn child_axis_map(&self) -> Vec<Vec<Option<usize>>> {
        self.children().map(|x| vec![None; x.rank()]).collect()
    }
