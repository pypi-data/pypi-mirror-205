...
use circuit_base::{
    named_axes::set_named_axes, Add, Array, CircuitNode, CircuitRc, CircuitType, Concat,
    ConstructError, Cumulant, DiscreteVar, Einsum, GeneralFunction, Index, Module, ModuleArgSpec,
    ModuleSpec, Rearrange, Scalar, Scatter, StoredCumulantVar, Symbol, Tag,
};
...
use rr_util::{
    ...
    shape::{shape_eq, Shape, Size, SIZE_MATCH},
    tensor_db::ensure_all_tensors_local,
    tensor_util::{parse_numeric, ParseError, PyParseError, TensorIndex, TorchDeviceDtypeOp},
    timed,
...
impl Parser {
    ...
    fn parse_circuits_impl(
        ...
        static RE_STR: Lazy<(Regex, String)> = Lazy::new(|| {
            ...
            let shape_axis_match: String =
                format!(r"(?:{}\s*:\s*)?{}\s*", AXIS_NAME_MATCH, *SIZE_MATCH);
        ...
        for line in lines {
            ...
            if is_ref {
                ...
                let (shape, named_axes) = if let Some(shape_cap) = re_captures.get(shape_cap) {
                    ...
                    let parse_axis_size = |x| -> Result<(Size, Option<&str>)> {
                        let (striped_x, named_axis) = extract_named_axis(x)?;

                        Ok((
                            Size::from_str(striped_x).context("failed to parse size for shape")?,
                            named_axis,
                        ))
                    };
                    ...
                    let axes_info = axis_strs
                        .into_iter()
                        .map(parse_axis_size)
                        .collect::<Result<Vec<(Size, Option<&str>)>, _>>()?;
                ...
                let variant = flip_op_result(re_captures.get(variant_cap).map(|s| {
                    s.as_str().parse().map_err(|_| {
                        anyhow::anyhow!(ParseCircuitError::InvalidVariant {
                            v: s.as_str().to_owned()
                        })
                    })
                }))?;
                ...
                if is_new_node {
                    ...
                    macro_rules! fail_check {
                        ($n:ident, $eq:expr) => {{
                            let failed = $n.clone().map(|x| $eq(&x, &old_node.$n)).unwrap_or(false);
                            if failed {
                                any_fail = true;
                                err_strs += &format!(
                                    "{} mismatch. new={:?} != old={:?}\n",
                        ...
                        ($n:ident) => {
                            fail_check!($n, |a, b| a != b)
                        };
                    }
                    fail_check!(name, |a: &Name, b| &Some(*a) != b);
                    fail_check!(shape, |a: &Shape, b: &Option<Shape>| {
                        b.is_none() || !shape_eq(a, b.as_ref().unwrap())
                    });
                    fail_check!(variant);
                    fail_check!(extra);
    ...
    fn deep_convert_partial_circ(
        ...
        if let Some(ref_circ) = ident
            .as_str()
            .and_then(|x| self.reference_circuits.get(&Name::new(x)))
        {
        ...
        let result = self
            ...
            .rename(partial_circuits[ident].name);
    ...
    fn deep_convert_partial_circ_children(
        ...
        let result = match ps.variant {
            T::Array => {
                ...
                if self.tensors_as_random || ps.extra == "rand" {
                    ...
                    Array::randn_named(
                        ...
                        ps.name,
                        self.tensors_as_random_device_dtype,
                    )?
                    .rc()
                } else {
            ...
            T::GeneralFunction => {
                GeneralFunction::new_from_parse(children, ps.extra.clone(), ps.name)
                    .with_context(|| "while parsing GeneralFunction with extra {ps.extra}")?
                    .rc()
            }
            T::Index => {
                expected_k_children(1)?;
                Index::try_new(
                    children[0].clone(),
                    TensorIndex::from_string_rank(
                        &ps.extra,
                        Some(children[0].rank()),
                        tensor_cache,
                    )?,
                    ps.name,
                )?
...
fn extract_named_axis(axis: &str) -> Result<(&str, Option<&str>)> {
