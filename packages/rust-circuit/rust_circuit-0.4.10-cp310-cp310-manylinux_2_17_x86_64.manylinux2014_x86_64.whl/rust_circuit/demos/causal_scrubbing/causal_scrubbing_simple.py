...
assert (  # To make sure this notebook doesn't get broken
    ...
    == """out GeneralFunction # cond_sampler=FuncSampler(d=IntDatum(xs=[9.0, 1.0, 0.0], label=1.0), f(d)=True), other_inputs_sampler=UncondSampler(d=IntDatum(xs=[0.0, 6.0, 8.0], label=1.0))
  D' GeneralFunction # cond_sampler=FuncSampler(d=IntDatum(xs=[0.0, 9.0, 8.0], label=1.0), f(d)=True), other_inputs_sampler=UncondSampler(d=IntDatum(xs=[3.0, 2.0, 9.0], label=1.0))
    A' GeneralFunction # cond_sampler=FuncSampler(d=IntDatum(xs=[0.0, 8.0, 5.0], label=1.0), f(d)=False), other_inputs_sampler=UncondSampler(d=IntDatum(xs=[0.0, 4.0, 6.0], label=0.0))
      x0' GeneralFunction # cond_sampler=FuncSampler(d=IntDatum(xs=[0.0, 5.0, 5.0], label=1.0), f(d)=0.0)
    B' GeneralFunction # cond_sampler=FuncSampler(d=IntDatum(xs=[0.0, 2.0, 8.0], label=0.0), f(d)=False), other_inputs_sampler=UncondSampler(d=IntDatum(xs=[0.0, 9.0, 4.0], label=1.0))
      x1' GeneralFunction # cond_sampler=FuncSampler(d=IntDatum(xs=[3.0, 9.0, 8.0], label=1.0), f(d)=9.0)
  y' GeneralFunction # cond_sampler=FuncSampler(d=IntDatum(xs=[6.0, 0.0, 6.0], label=1.0), f(d)=1.0)"""
)
# %% [markdown]
