...
def get_file_path(model_name, root_path=RUN_ROOT_PATH, **kwargs):
    ...
    - model_evaluation 시 경로 지정을 위해 사용
    ...
    evl_path = 'for-evl-json'    # 모델 평가를 위한 폴더
    ...
    if 'keras' in get.model_type(model_name) or 'tf' in get.model_type(model_name) or 'pytorch' in get.model_type(model_name):
        ...
        path_list = [json_path_model, csv_path_visual, json_path_visual, json_path_shap, evl_path]   # DL
        ...
        if metric_lightgbm in metricsLightgbm.CLASSIFICATION:
            path_list = [json_path_model, json_path_visual, json_path_shap, evl_path]       # lightgbm & Classification
        elif metric_lightgbm in metricsLightgbm.REGRESSION:
            path_list = [json_path_model, json_path_shap, evl_path]                         # lightgbm & Regression
        else:
            pass
        ...
        path_list = [json_path_model, json_path_visual, json_path_shap, evl_path]       # ML & Classification

    else:
        path_list = [json_path_model, json_path_shap, evl_path]                         # ML & Regression
    ...
    if len(path_list) == 3:     # ML & Regression
        ...
        dir_dict = {'home_path': home,
                    ...
                    'evl_json': evl_file_name_path,
                    'evl_json_full': home + prefix_path + evl_file_name_path,
