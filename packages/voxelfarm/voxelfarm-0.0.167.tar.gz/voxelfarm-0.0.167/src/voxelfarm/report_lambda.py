...
class report_lambda_framework:
    ...
    def valid_volume_channel_buffer(self, component, channel):
        return False

    def valid_attribute_channel_buffer(self, component, attribute, channel):
        return False

    def valid_volume_buffer(self, component):
        return False

    def valid_attribute_buffer(self, component, attribute):
        return False
...
class report_lambda_host:
    ...
    def convert_to_alphanumerical(self, metadata_component, metadata_column, df, df_column = None):
        if not df.empty:
            if df_column:
                if df_column in df:
                    return df.apply(lambda row : self.get_enum_attribute_value(metadata_component, metadata_column, int(row[df_column])-1), axis = 1)
            else:            
    ...
    def load_pandas_dataframe(self, component):
        ...
        if not self.lambda_framework.is_meta():
