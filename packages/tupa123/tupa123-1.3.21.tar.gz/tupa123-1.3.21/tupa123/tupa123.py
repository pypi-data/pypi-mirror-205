    ...
    def Fit_ADAM(self, matX, matY, matX2=[0], matY2=[0]):
        ...
        try: #Inicializa com pesos ja salvos, se existir esses arquivos-------------------------------------
            ...
            if self.nn2c ==1: #uma linha de data ele sempre le como vetor de colunas, e a transposta disso precisa de um esquema especial
                P12aux = np.array([np.loadtxt(nomepasta + "P12.txt")]) 
                P12 = P12aux.T
        ...
        if (len(matX2)>1):            
            matX2 = self.Normalize(matX2,nomepasta + "vmax_in2.txt",nomepasta + "vmin_in2.txt",nomepasta + "media_in2.txt",nomepasta + "desvio_in2.txt")                
            if (self.normout==1):
                matY2 = self.Normalize(matY2,nomepasta + "vmax_out2.txt",nomepasta + "vmin_out2.txt",nomepasta + "media_out2.txt",nomepasta + "desvio_out2.txt")            
            #deleta os arquivos criados da validação cruzada que não sao uteis
            os.remove(nomepasta + "vmax_in2.txt")
            os.remove(nomepasta + "vmin_in2.txt")
            os.remove(nomepasta + "media_in2.txt")
            os.remove(nomepasta + "desvio_in2.txt")
            os.remove(nomepasta + "vmax_out2.txt")
            os.remove(nomepasta + "vmin_out2.txt")
            os.remove(nomepasta + "media_out2.txt")
            os.remove(nomepasta + "desvio_out2.txt")
