...
def RoiItemClassDef(
    roiName, shape, actionToolTip=None, actionIcon=None, actionText=None
):
    def inner(cls):
        cls.roiName = roiName
        cls.shape = shape
...
def registerRoiItem(klass):
    ...
    if roiName is None:
        raise AttributeError(
            "Failed to register Roi class {0} roiName "
...
class RoiItemBase(qt.QObject):
    ...
    def _remove(self, handles=True, shape=True):
        ...
        if shape:
            self._plot.remove(self._name, kind="item")
    ...
    def _plotSignal(self, event):
        evType = event["event"]
        if evType == "drawingFinished" and event["parameters"]["label"] == self._name:
            self._finish(event)
        elif evType == "drawingProgress" and event["parameters"]["label"] == self._name:
    ...
    def _connect(self):
        ...
        self._plot.sigInteractiveModeChanged.connect(self._interactiveModeChanged)
    ...
    def _disconnect(self):
        ...
        self._plot.sigInteractiveModeChanged.disconnect(self._interactiveModeChanged)
        self._connected = False
    ...
    def _drawHandles(self, handles):
        for i_handle, handle in enumerate(handles):
            item = self._plot.addMarker(
                self._points[handle][0],
    ...
    def start(self):
        ...
        self._plot.setInteractiveMode(
            "draw", shape=self.shape, source=self, label=self._name, color="pink"
        )
        self._connect()
...
class ImageRoiManager(qt.QObject):
    ...
    def _createRoiActions(self):
        ...
        roiActionGroup.triggered.connect(
            self._roiActionTriggered, qt.Qt.QueuedConnection
        )
    ...
    def showRois(self, show):
        ...
        if self._optionActions:
            ...
            action.setText("\U0001F440" if show else "\u2012")
    ...
    def _startRoi(self):
        ...
        item.sigRoiDrawingFinished.connect(
            self._roiDrawingFinished, qt.Qt.QueuedConnection
        )
        item.sigRoiDrawingStarted.connect(
            self._roiDrawingStarted, qt.Qt.QueuedConnection
        )
        item.sigRoiDrawingCanceled.connect(
            self._roiDrawingCanceled, qt.Qt.QueuedConnection
        )
        item.sigRoiMoved.connect(self.sigRoiMoved, qt.Qt.QueuedConnection)
        item.start()
...
@RoiItemClassDef("zoom", None, actionIcon="normal", actionToolTip="Zoom.")
...
@RoiItemClassDef("edit", None, actionIcon="crosshair", actionToolTip="Edit ROI(s)")
...
class RectRoiItem(RoiItemBase):
    ...
    center = property(
        lambda self: (
            self._left + (self._right - self._left) / 2.0,
            self._bottom + (self._top - self._bottom) / 2.0,
        )
    )
    """
    ...
    def _handleMoved(self, name, x, y, index):
        if name == self._center:
            ...
            for i in (v_op, h_op):
                self._setHandleData(self._corners[i], (self._xData[i], self._yData[i]))
    ...
    def _drawEvent(self, event):
        ...
        self._left, self._bottom, self._right, self._top = left, bottom, right, top
...
class PolygonRoiItem(RoiItemBase):
    def _drawFinished(self, event):
        ...
        vertices = ["V{0}_{1}".format(idx, uuid) for idx in range(len(points) - 1)]
