...
def logtime(func):
    """A decorator function that logs the elapsed time.
    """
    ...
    def wrapper(*args, **kwargs):
        ...
        log('----------------------------------------------------------------------')
        log(f'Time spent: {elapsed} seconds')
        return rv
...
def cmd_search(args):
    ...
    The output is a list of JSON objects with following properties:
    * "book_id": ID of the book.
    * "id": ID of the item.
    * "file": matched filename in the item.
    * "context": an object of context snippets of the match.
    """
    kwargs = args.copy()
    kwargs['context'] = {
    ...
    try:
        for item in search.search(**kwargs):
            msg = json.dumps({
