...
/*
連続テト譜エディタ by Mihys
Ver 1.15e(2021/11/07)
・ダークモード(prefers-color-scheme:dark)に対応。
・「Fumen for mobile誘導」新規ウィンドウ表示ボタン追加。
Ver 1.15d2(2021/01/29)
・非SSL(HTTP)でアドオンリストやTinyURL等が利用できないバグ修正。
Ver 1.15d (2021/01/24)
・SSL(HTTPS)に対応。
Ver 1.15c (2020/11/15)
・LISTの表示サイズ効率化。
・データ出力時にVIEWとLISTのリンクを追加。
Ver 1.15b (2020/05/04)
・「Fumen for mobile誘導」ボタン追加。
    ...
    if (addon_ins2 > 0) {
        document.write(
            '<script language="JavaScript" src="' +
                addon_hash.substring(0, addon_ins2) +
                '"></script>'
        )
    }
...
baseurl_https = 'https://fumen.zui.jp/'
baseurl_http = 'http://fumen.zui.jp/'
...
asctbl =
    ' !"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~' // ASCII文字テーブル
framelim = 2000 // 限界フレーム数
fldlines = 24 // フィールド段数
fldblks = fldlines * 10 // フィールドブロック数
f = new Array(fldblks)
encf = new Array(fldblks)
af = new Array(fldblks * (framelim + 1)) // 仮想フィールド
p = new Array(3)
ap = new Array(3 * (framelim + 1)) // ピース種類,角度,座標
...
function versioncheck (startframe) {
    // バージョン判定
    ...
    if (insurl1 >= 0 && insurl2 >= 0 && insurl1 < insurl2)
        verstr = verstr.substring(insurl2 + 1)
    lng = verstr.indexOf('@')
    if (lng < 0) {
        ...
        if (edit != null) {
            edit.innerHTML =
                ...
                location.search.substring(0, cmd + 1) +
                'v' +
                tx.value.substring(1) +
                '">[Edit]</a>'
        }
        ...
        verstr = ''
        for (i = 0; i <= lng; i++) {
            tmp = enctbl.indexOf(tmpstr.charAt(i))
            if (tmp >= 0) verstr = verstr + tmpstr.charAt(i)
        }
...
function decode090 (startframe) {
    ...
    var viewjmp = document.getElementById('viewjmp')
    var dumpjmp = document.getElementById('dumpjmp')
    startframe = startframe != null && startframe > 0 ? startframe : 0
    encstr = tx.value
    enclen = 0
    for (i = 0; i < encstr.length; i++) {
        tmp = enctbl.indexOf(encstr.charAt(i))
        if (tmp >= 0) enc[enclen++] = tmp
    }
    ...
    for (e = startframe; encc < enclen; e++) {
        ...
        for (j = 0; j < 220; ) {
            ...
            tmp += enc[encc++] * 64
            tmp2 = tmp % 220
            tmp = Math.floor(tmp / 220)
            tmp1 = tmp % 17
            tmp = Math.floor(tmp / 17)
            for (i = 0; i <= tmp2; i++)
                af[e * fldblks + j++ + fldblks - 220] += tmp1 - 8
        }
        // タイプ,角度,座標入力
        tmp = enc[encc++]
        tmp += enc[encc++] * 64
        tmp += enc[encc++] * 4096
        ap[e * 3 + 0] = tmp % 8
        tmp = Math.floor(tmp / 8)
        ap[e * 3 + 1] = tmp % 4
        tmp = Math.floor(tmp / 4)
        ap[e * 3 + 2] = (tmp % 220) + fldblks - 220
        tmp = Math.floor(tmp / 220)
        au[e] = tmp % 2
        ...
        // フィールドコピー
        for (i = 0; i < fldblks; i++)
            af[(e + 1) * fldblks + i] = af[e * fldblks + i]
        // ブロック配置
        if (ap[e * 3 + 0] > 0) {
            for (j = 0; j < 4; j++)
                af[
                    (e + 1) * fldblks +
                        ap[e * 3 + 2] +
                        b[ap[e * 3 + 0] * 32 + ap[e * 3 + 1] * 8 + j * 2 + 1] * 10 +
                        b[ap[e * 3 + 0] * 32 + ap[e * 3 + 1] * 8 + j * 2] -
                        11
                ] = ap[e * 3 + 0]
        }
        // フィールドずらし消去
        for (i = 20, k = 20; k >= 0; k--) {
            chk = 0
            for (j = 0; j < 10; j++)
                chk += af[(e + 1) * fldblks + k * 10 + j + fldblks - 220] > 0
            if (chk < 10) {
                for (j = 0; j < 10; j++)
                    af[(e + 1) * fldblks + i * 10 + j + fldblks - 220] =
                        af[(e + 1) * fldblks + k * 10 + j + fldblks - 220]
                i--
            }
        }
        for (; i >= 0; i--)
            for (j = 0; j < 10; j++)
                af[(e + 1) * fldblks + i * 10 + j + fldblks - 220] = 0
        // せり上がり
        if (au[e]) {
            for (i = 0; i < 210; i++)
                af[(e + 1) * fldblks + i + fldblks - 220] =
                    af[(e + 1) * fldblks + i + 10 + fldblks - 220]
            for (i = 210; i < 220; i++) af[(e + 1) * fldblks + i + fldblks - 220] = 0
        }
        // ミラー発動
        if (am[e])
            for (i = 0; i < 21; i++)
                for (j = 0; j < 5; j++) {
                    tmp = af[(e + 1) * fldblks + i * 10 + j + fldblks - 220]
                    af[(e + 1) * fldblks + i * 10 + j + fldblks - 220] =
                        af[(e + 1) * fldblks + i * 10 + 9 - j + fldblks - 220]
                    af[(e + 1) * fldblks + i * 10 + 9 - j + fldblks - 220] = tmp
                }
        // その他
    }
    if (e > startframe) {
        framemax = e - 1
        frame = startframe
        for (i = 0; i < e * fldblks; i++) {
            if (!(af[i] >= 0 && af[i] <= 8)) af[i] = 0
        }
        if (mini != 2) {
            popframe(frame)
            clt.checked = ct
            ...
            if (viewjmp != null) {
                viewjmp.removeAttribute('href')
            }
            if (dumpjmp != null) {
                dumpjmp.removeAttribute('href')
            }
        }
    } else {
        updateflag = 0
    ...
    for (e = startframe; encc < enclen; e++) {
        ...
        if (acflg) {
            ...
            for (i = 0; i < tmplen; i += 4) {
                ...
                tmpstr += asctbl.charAt(tmp % 96)
                tmp = Math.floor(tmp / 96)
                tmpstr += asctbl.charAt(tmp % 96)
                tmp = Math.floor(tmp / 96)
                tmpstr += asctbl.charAt(tmp % 96)
                tmp = Math.floor(tmp / 96)
            ...
            if (au[e]) {
                for (i = 0; i < 210; i++)
                    af[(e + 1) * fldblks + i + fldblks - 220] =
                        af[(e + 1) * fldblks + i + 10 + fldblks - 220]
                for (i = 210; i < 220; i++)
                    af[(e + 1) * fldblks + i + fldblks - 220] = 0
            }
            // ミラー発動
            if (am[e])
                for (i = 0; i < 21; i++)
    ...
    for (e = startframe; encc < enclen; e++) {
        ...
        if (!ad[e]) {
            ...
            for (i = fldlines - 2, k = fldlines - 2; k >= 0; k--) {
                chk = 0
                for (j = 0; j < 10; j++) chk += af[(e + 1) * fldblks + k * 10 + j] > 0
                if (chk < 10) {
                    for (j = 0; j < 10; j++)
                        af[(e + 1) * fldblks + i * 10 + j] =
                            af[(e + 1) * fldblks + k * 10 + j]
                    i--
                }
            }
            for (; i >= 0; i--)
                for (j = 0; j < 10; j++) af[(e + 1) * fldblks + i * 10 + j] = 0
            // せり上がり
            if (au[e]) {
                for (i = 0; i < fldblks - 10; i++)
                    af[(e + 1) * fldblks + i] = af[(e + 1) * fldblks + i + 10]
                for (i = fldblks - 10; i < fldblks; i++) af[(e + 1) * fldblks + i] = 0
            }
            // ミラー発動
            if (am[e])
                for (i = 0; i < fldlines - 1; i++)
                    for (j = 0; j < 5; j++) {
                        tmp = af[(e + 1) * fldblks + i * 10 + j]
                        af[(e + 1) * fldblks + i * 10 + j] =
                            af[(e + 1) * fldblks + i * 10 + 9 - j]
                        af[(e + 1) * fldblks + i * 10 + 9 - j] = tmp
                    }
            // その他
        }
    }
...
function encode (focusset) {
    ...
    for (e = 0; encc < enclim && e <= framemax; e++) {
        ...
        if (tmpstr != cmstrrep) {
            ...
            for (i = 0; i < tmplen; i += 4) {
                ...
                tmp +=
                    (((ins = asctbl.indexOf(tmpstr.charAt(i + 1))) >= 0 ? ins : 0) % 96) *
                    96
                tmp +=
                    (((ins = asctbl.indexOf(tmpstr.charAt(i + 2))) >= 0 ? ins : 0) % 96) *
                    9216
                tmp +=
                    (((ins = asctbl.indexOf(tmpstr.charAt(i + 3))) >= 0 ? ins : 0) % 96) *
                    884736
                enc[encc++] = tmp % 64
                tmp = Math.floor(tmp / 64)
                enc[encc++] = tmp % 64
        ...
        if (!ad[e]) {
            ...
            for (i = fldlines - 2, k = fldlines - 2; k >= 0; k--) {
                chk = 0
                for (j = 0; j < 10; j++) chk += encf[k * 10 + j] > 0
                if (chk < 10) {
            ...
            // せり上がり
            if (au[e]) {
                for (i = 0; i < fldblks - 10; i++) encf[i] = encf[i + 10]
                for (i = fldblks - 10; i < fldblks; i++) encf[i] = 0
            }
            // ミラー発動
            if (am[e])
                for (i = 0; i < fldlines - 1; i++)
    ...
    if (tmp < 0) tmp = location.href.length
    tmp2 = location.href.indexOf('#')
    if (tmp2 < 0) tmp2 = location.href.length
    ...
    viewjmp.href = view.value
    dumpjmp.href = dump.value
    out.className = 'theme5a'
...
function popframe (pfframe) {
    ...
    for (pfi = 0; pfi < 3; pfi++) p[pfi] = ap[pfframe * 3 + pfi]
    pp.checked = p[0] > 0
    for (pfi = 0; pfi < 4; pfi++) {
        pcustom[pfi] = -1
    }
...
function main () {
    ...
    if (cmd >= 0) tx.value = location.search.substring(cmd + 1)
    versioncheck(0)
    refreshcolor()
}
    ...
    if (pgtype ? frame < framemax : frame < framelim - 1 || frame < framemax) {
        ...
        if (frame > framemax) {
            ...
            if (dc.checked) {
                ...
                // 直後にせり上がり
                if (up.checked) {
                    for (i = 0; i < fldblks - 10; i++) f[i] = f[i + 10]
                    for (i = fldblks - 10; i < fldblks; i++) f[i] = 0
                }
                // 直後にミラー発動
                if (mr.checked)
                    for (i = 0; i < fldlines - 1; i++)
...
function checkpiece (cpt, cpr, cpid) {
    // ピース当たり判定
    cp = 0
    if (cpt >= 1 && cpt <= 7) {
        for (cpi = 0; cpi < 4; cpi++) {
            ...
            if (
                ...
                f[
                    cpid +
                        b[cpt * 32 + cpr * 8 + cpi * 2 + 1] * 10 +
                        b[cpt * 32 + cpr * 8 + cpi * 2] -
                        11
                ] > 0
            )
                cp++
...
function drawpiece () {
    ...
    if (dpt >= 1 && dpt <= 7) {
        if (kbfcs && !dc.checked) {
            ...
            if (dpid >= 0 && dpid < fldblks - 10) {
                for (dpi = 0; dpi < 4; dpi++) {
                    ...
                    dptmp2 =
                        Math.floor(dpid / 10) + b[dpt * 32 + dpr * 8 + dpi * 2 + 1] - 1
                    if (
                        dptmp1 >= 0 &&
                        dptmp1 <= 9 &&
                        dptmp2 >= 0 &&
                        ...
                        fld[dptmp2 * 10 + dptmp1].style.backgroundColor = c[ct * 27 + 9]
                }
            }
        }
        dpid = p[2]
        for (dpi = 0; dpi < 4; dpi++) {
            ...
            if (dptmp1 >= 0 && dptmp1 <= 9 && dptmp2 >= 0 && dptmp2 <= fldlines - 2)
                fld[dptmp2 * 10 + dptmp1].style.backgroundColor = c[ct * 27 + dpt + 9]
        }
    }
}
...
function drawnext () {
    ...
    dnj = frame + 1
    for (dnk = 0; dnk < 2; dnk++) {
        for (; dnj <= framemax; dnj++)
            if (ap[dnj * 3] > 0 && (ap[(dnj - 1) * 3] == 0 || !ad[dnj - 1])) {
                dnt = ap[dnj * 3]
                dnid = (ct && (dnt == 2 || dnt == 5 || dnt == 6)) * 10
                dnr = ct * 2
                if (dnt >= 1 && dnt <= 7) {
                    for (dni = 0; dni < 4; dni++) {
                        dntmp1 = (dnid % 10) + b[dnt * 32 + dnr * 8 + dni * 2]
                        dntmp2 =
                            Math.floor(dnid / 10) + b[dnt * 32 + dnr * 8 + dni * 2 + 1] - 1
                        if (dnk == 0) {
                            if (dntmp1 >= 0 && dntmp1 <= 3 && dntmp2 >= 0 && dntmp2 <= 1)
                                nxfld[dntmp2 * 4 + dntmp1].style.backgroundColor =
                                    c[
                                        ct * 27 +
                                            (ap[(frame + 1) * 3] > 0 &&
                                                (ap[frame * 3] == 0 || !ad[frame])) *
                                                9 +
                                            dnt
                                    ]
                        } else if (dnk == 1) {
                            if (dntmp1 >= 0 && dntmp1 <= 3 && dntmp2 >= 0 && dntmp2 <= 1)
                                nx2fld[dntmp2 * 6 + dntmp1 + 1].style.backgroundColor =
                                    c[
                                        ct * 27 +
                                            (ap[(frame + 1) * 3] > 0 &&
                                                (ap[frame * 3] == 0 || !ad[frame])) *
            ...
            if (dnt >= 1 && dnt <= 7) {
                for (dni = 0; dni < 4; dni++) {
                    ...
                    if (dnk == 0) {
                        ...
                        if (dntmp1 >= 0 && dntmp1 <= 3 && dntmp2 >= 0 && dntmp2 <= 1)
                            hlfld[dntmp2 * 6 + dntmp1 + 1].style.backgroundColor =
                                c[ct * 27 + 9 + dnt]
                    }
                }
            }
...
function refreshquiz (rqframe, newpage, gui, radioupdate, before115) {
    // クイズ
    ...
    if (cmstr.substring(0, 3) == '#Q=') {
        ...
        for (i = 3; i < qlen; i++) {
            qca = cmstr.charAt(i)
            if (qca == ';') {
                quizcm = ';' + cmstr.substring(i + 1)
                break
            }
            if (quiztbl.indexOf(qca) >= 0) {
                qstr += qca
            }
        }
        qlen = qstr.length
        ...
        if (newpage && !ad[rqframe - 1]) {
            ...
            if (quizcur >= 0) {
                cmstr =
                    '#Q=[' +
                    quiztbl.charAt(quizhld) +
                    '](' +
                    quiztbl.charAt(quizcur) +
                    ')' +
                    qnxstr +
                    quizcm
                if (gui) {
        ...
        if (gui) {
            for (j = 0; j < 7; j++) {
                for (i = 0; i < 4; i++) {
                    mode[8 + j * 4 + i].disabled =
                        !quiz || j == quizcur || j == (quizhld >= 0 ? quizhld : quiznx[0])
                            ? false
                            : true
                }
            }
            ...
            if (quizhld >= 0) {
                dnt = quizhld + 1
                dnid = ct && (dnt == 2 || dnt == 5 || dnt == 6)
                dnr = ct * 2
                for (i = 0; i < 4; i++) {
                    qf[
                        (b[dnt * 32 + dnr * 8 + i * 2 + 1] + dnid - 1) * 43 +
                            b[dnt * 32 + dnr * 8 + i * 2] +
                            0
                    ] = c[0 + ct * 27 + dnt]
                }
            ...
            qfstr = ''
            qfstr +=
                '<table width=184 height=12 border=0 cellspacing=0 cellpadding=0><td align=center valign=center style="background-color:#000000;">'
            qfstr += '<table align=center border=0 cellspacing=0 cellpadding=0>'
            for (j = 0; j < 2; j++) {
                qfstr += '<tr>'
                for (i = 0; i < 43; i++) {
                    qfstr +=
                        '<td width=4 height=4 style="background-color:' +
                        qf[j * 43 + i] +
                        ';"></td>'
                }
                qfstr += '</tr>'
        ...
        if (gui) {
            ...
            qfstr = ''
            qfstr +=
                '<table width=184 height=12 border=0 cellspacing=0 cellpadding=0><td align=center valign=center class=theme2>'
            qfstr += '</td></table>'
            if (qfld != null) {
                qfld.innerHTML = qfstr
            }
...
function refresh () {
    ...
    for (rfj = 0; rfj < fldblks; rfj += 10) {
            ...
            for (rfi = 0; rfi < 10; rfi++) {
                ...
                if (f[rfji]) {
                    ...
                    for (rfk = 0; rfk < 4; rfk++) {
                        if (
                            p[2] +
                                b[p[0] * 32 + p[1] * 8 + rfk * 2 + 1] * 10 +
                                b[p[0] * 32 + p[1] * 8 + rfk * 2] -
                                11 ==
                            rfji
                        )
                            rfc++
        ...
        for (rfi = 0; rfi < 10; rfi++) {
            // 表示
            rfji = rfj + rfi
            fld[rfji].style.backgroundColor =
                c[
                    ct * 27 +
                        f[rfji] +
                        (rfc == 10 && f[rfji] > 0) * 18 +
                        (rfji < fldblks - 210 && f[rfji] == 0) * 9
                ]
        }
    }
    ...
    drawpiece()
    for (rfi = 0; rfi < 4; rfi++)
        if (pcustom[rfi] >= 0) fld[pcustom[rfi]].style.backgroundColor = '#ffffff'
    for (rfj = 0; rfj < 2; rfj++)
        for (rfi = 0; rfi < 4; rfi++)
            hlfld[rfj * 6 + rfi + 1].style.backgroundColor = c[ct * 27]
    for (rfj = 0; rfj < 2; rfj++)
        for (rfi = 0; rfi < 4; rfi++)
            nxfld[rfj * 4 + rfi].style.backgroundColor = c[ct * 27]
    for (rfj = 0; rfj < 2; rfj++)
        for (rfi = 0; rfi < 4; rfi++)
            nx2fld[rfj * 6 + rfi + 1].style.backgroundColor = c[ct * 27]
    for (rfj = 0; rfj < 7; rfj++) {
        for (rfi = 0; rfi < 4; rfi++) {
            pcl[rfj * 4 + rfi].style.backgroundColor =
                c[
                    ct * 27 +
                        10 +
                        rfj +
                        (rfj == p[0] - 1 && rfi == p[1] ? -9 : rfi == ct * 2 ? 9 : 0)
                ]
        }
    }
    if (quiz) {
...
function refreshcolor () {
    ...
    refresh()
    for (rfi = 1; rfi < 9; rfi++)
        fcl[rfi - 1].style.backgroundColor = c[ct * 27 + rfi]
    for (i = 0; i < 56; i++) {
        spf[i] = ''
    }
    for (j = 0; j < 7; j++) {
    ...
    for (i = 0; i < 56; i++) {
        if (spfld[i] != null) {
            spfld[i].style.backgroundColor = spf[i]
        }
    }
}
...
function shiftfield (r) {
    ...
    if (r == 0) {
        ...
        for (i = 0; i < 4; i++) {
            if (pcustom[i] >= 0) {
                pcustom[i]++
                if (pcustom[i] > fldblks - 11 || pcustom[i] % 10 == 0) {
                    pcustom[i] = -1
                }
            }
        }
    ...
    if (p[0] > 0) {
        for (
            i = 1;
            dc.checked && i < fldlines + 1 && !checkpiece(p[0], p[1], p[2] + i * 10);
            i++
        );
        if (i > 1 && i < fldlines + 1) {
            p[2] = p[2] + (i - 1) * 10
            ...
            for (
                i = 1;
                i < fldlines + 1 && checkpiece(p[0], p[1], p[2] - (i - 1) * 10);
                i++
            );
            if (i > 1) {
                if (i < fldlines + 1) {
...
function drawfield (id, down) {
    ...
    for (i = 0; i < 4; i++)
        if (pcustom[i] >= 0) fld[pcustom[i]].style.backgroundColor = '#ffffff'
    if (md >= 0 && md <= 7) {
        // フィールド作成
        if (eventbutton == 1) {
            // 左クリック
            ...
            if (fe >= 0 && fe <= 8) {
                for (i = 0; i < 4; i++) {
                    if (frow.checked) {
                        if (pcustom[i] - (pcustom[i] % 10) == id - (id % 10)) {
                            pcustom[i] = -1
                        }
                    } else {
                        if (pcustom[i] == id) {
                ...
                if (Math.abs(mclx - mcx) > Math.abs(mcly - mcy)) {
                    ...
                    for (
                        ;
                        (mcdx < 0 && mclx >= mcx) || (mcdx > 0 && mclx <= mcx);
                        mclx += mcdx, mcly += mcdy
                    ) {
                        ...
                        if (frow.checked) {
                            ...
                            for (j = 0; j < 10; j++) {
                                ...
                                fld[mcidm + j].style.backgroundColor =
                                    c[
                                        ct * 27 +
                                            f[mcidm + j] +
                                            (mcidm + j < fldblks - 210 && f[mcidm + j] == 0) * 9
                                    ]
                            }
                        } else {
                            f[mcid] = fe
                            fld[mcid].style.backgroundColor =
                                c[
                                    ct * 27 + f[mcid] + (mcid < fldblks - 210 && f[mcid] == 0) * 9
                                ]
                        }
                    }
                } else {
                    ...
                    for (
                        ;
                        (mcdy < 0 && mcly >= mcy) || (mcdy > 0 && mcly <= mcy);
                        mclx += mcdx, mcly += mcdy
                    ) {
        ...
        if (eventbutton != 1) {
            ...
            for (
                i = 0;
                dc.checked && i < fldlines + 1 && !checkpiece(t, r, id + i * 10);
                i++
            );
            if (i > 0 && i < fldlines + 1) {
                p[0] = t
        ...
        if (eventbutton == 1) {
            ...
            if (p[0]) {
                if (down) {
                    ...
                    for (i = 0; i < 4; i++) {
                        pcustom[i] =
                            (pcmy - 1 + b[p[0] * 32 + p[1] * 8 + i * 2 + 1]) * 10 +
                            (pcmx - 1 + b[p[0] * 32 + p[1] * 8 + i * 2 + 0])
                        if (id == pcustom[i]) {
                            pce = 0
                            pcustom[i] = -1
                        }
                ...
                for (i = 0; i < 4; i++) {
                    if (pcustom[i] >= 0) {
                        pcc++
                        if (pcx < 0 || pcustom[i] % 10 < pcx) {
                            pcx = pcustom[i] % 10
                        }
                        if (pcy < 0 || Math.floor(pcustom[i] / 10) < pcy) {
                            pcy = Math.floor(pcustom[i] / 10)
                        }
                    }
                }
                if (pcc == 4) {
                        ...
                        for (pcj = 0; pcj < 2; pcj++)
                            for (pci = 0; pci < 2; pci++) {
                                ...
                                for (j = 0; j < 4; j++) {
                                    ...
                                    for (i = 0; i < 4; i++) {
                                        if (
                                            pcmx == b[k * 8 + i * 2 + 0] - pci &&
                                            pcmy == b[k * 8 + i * 2 + 1] - pcj
                                        ) {
                                            pcmc++
                                        }
                                    }
    ...
    if (p[0] > 0 && (rot == -1 || rot == 1)) {
        ...
        for (rti = 0; rti < (ct ? 5 : 1); rti++) {
            ...
            if (
                ...
                rty >= 0 &&
                rty <= fldlines - 1 &&
                !checkpiece(p[0], (p[1] + rot + 4) % 4, rty * 10 + rtx)
            ) {
                p[1] = (p[1] + rot + 4) % 4
                p[2] = rty * 10 + rtx
                shiftfield(-1)
                    ...
                    if (p[1] == 0 || p[1] == 2) {
                        if (
                            !(
                                ...
                                rtyorg >= 0 &&
                                rtyorg <= fldlines - 1 &&
                                !checkpiece(p[0], p[1], (rtyorg + 1) * 10 + rtxorg)
                            )
                        ) {
                            for (rtj = 1; rtj <= 2; rtj++) {
                                if (
                    ...
                    if (p[1] == 1 || p[1] == 3) {
                        for (rtj = 1; rtj <= 1; rtj++) {
                            if (
                                ...
                                rty >= 0 &&
                                rty <= fldlines - 1 &&
                                !checkpiece(p[0], (p[1] + rot + 4) % 4, rty * 10 + (rtx - rtj))
                            ) {
                                p[1] = (p[1] + rot + 4) % 4
                                p[2] = rty * 10 + (rtx - rtj)
                                shiftfield(-1)
                    ...
                    if ((p[1] + rot + 4) % 4 == 2) {
                        for (rtj = 1; rtj <= 1; rtj++) {
                            if (
                                ...
                                rty - rtj >= 0 &&
                                rty - rtj <= fldlines - 1 &&
                                !checkpiece(p[0], (p[1] + rot + 4) % 4, (rty - rtj) * 10 + rtx)
                            ) {
                                p[1] = (p[1] + rot + 4) % 4
                                p[2] = (rty - rtj) * 10 + rtx
                                shiftfield(-1)
...
function keybuttontext (fcs) {
    ...
    if (!mini) {
        ...
        if (STRING_FRAME_PREVIOUS.substring(1, 2) == ' ') {
            prev.value = fcs
                ? 'V' + STRING_FRAME_PREVIOUS.substring(1)
                : STRING_FRAME_PREVIOUS
        }
...
function previewpiece (id, t, r) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        tmp2 = Math.floor(id / 10) + b[t * 32 + r * 8 + i * 2 + 1] - 1
        if (tmp1 >= 0 && tmp1 <= 9 && tmp2 >= 0 && tmp2 <= fldlines - 2) {
            fld[tmp2 * 10 + tmp1].style.backgroundColor = c[ct * 27 + t + 9]
            tf[i] = tmp2 * 10 + tmp1
        } else {
            tf[i] = -1
        }
