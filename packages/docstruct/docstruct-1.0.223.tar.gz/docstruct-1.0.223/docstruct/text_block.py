...
class Word(TextBlock):
    ...
    def __init__(
        self, text: str, bounding_box: BoundingBox, printed: bool = True
    ):
        super().__init__(bounding_box=bounding_box)
...
class Line(TextBlock):
    ...
    def __init__(
        self, bounding_box: BoundingBox = None, children: list[Word] = None
    ):
...
class Table(Area):
    ...
    def get_grid(self) -> np.ndarray:
        num_rows = (
            max(cell.row_index + cell.row_span - 1 for cell in self.cells) + 1
        )
        num_columns = (
            max(cell.col_index + cell.col_span - 1 for cell in self.cells) + 1
        )
    ...
    def to_csv(self, filepath: str):
        max_row_index = max(
            cell.row_index + cell.row_span - 1 for cell in self.cells
        )
        max_col_index = max(
            cell.col_index + cell.col_span - 1 for cell in self.cells
        )
        table_array = [
