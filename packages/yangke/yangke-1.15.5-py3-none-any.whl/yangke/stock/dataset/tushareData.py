...
from sqlalchemy import create_engine, DATE, BOOLEAN, Float
...
from yangke.dataset.sqlalchemy import SqlOperator, YkColumn
from yangke.dataset.sqlalchemy import Base, Column, String, DATETIME
from yangke.stock.DatasetClass import Holiday
...
class StockData:
    ...
    def __init__(self, project=None):
        ...
        if project is not None:
            self.data_folder = project.data_folder or "stock_data"
            self.code = project.symbol
            self.storage = project.storage
            self.encoding = project.encoding
            self.project = project
        self.engine: Engine | None = None
        self.股票信息表 = "AllStocksBasicInfo"
        self.sql: SqlOperator | None = None  # SqlOperator()  # 使用时初始化
    ...
    def get_ts_code_by_symbol(self, symbol):
        """
        根据股票的symbol获取ts_code
        """
        res = self.sql.select_in_table(table_name='AllStocksBasicInfo',
    ...
    def get_all_stock_basic_info(self) -> pd.DataFrame:
        ...
        """
        self.init_storage()
        if self.storage == "Sqlite数据库":
            update_time = self.sql.get_update_time_of_table(self.股票信息表)
            if self.need_update(update_time):  # 如果需要更新
                ...
                # 保存到Sqlite数据库
                inner_data.to_sql(name=self.股票信息表, con=self.engine, if_exists="replace", index=False)
                self.sql.update_update_time_of_table(self.股票信息表)
            else:
                inner_data = pd.read_sql(sql=self.股票信息表, con=self.engine)
            return inner_data
    ...
    def download_all_stocks(self, codes: list = None, num_works=64, exclude: list = None) -> list:
        ...
        if codes is None:
            ...
            if not self.need_update(state.last_update_time):
                return state.stocks
            codes_available = self.get_all_stock_basic_info()
        ...
        try:
            ...
            logger.error("multi-process error!")
            traceback.print_exc()  # 打印系统错误提示
            import sys
            sys.exit(1)

        logger.info("All stock data downloaded, finish")
    ...
    def download(self, code: str, append=True) -> pd.DataFrame:
        ...
        if '.' in code:  # 如果传入的股票代码是类似于 000544.SZ 格式的，则去掉后面的 .SZ,只保留数字格式代码
            ...
            ts_code = self.get_ts_code_by_symbol(code)
            symbol = code
        ...
        if self.storage == "Sqlite数据库":
            ...
            if not self.need_update(update_time):
                ...
                df = pd.read_sql(f"daily{symbol}", con=self.engine)
                return df
    ...
    def is_holiday(self, day_datetime: datetime.date = date.today()):
        ...
        #                         result_col=['isOpen'])
        is_open = self.sql.select_in_table(table_name="Holiday", condition_dict={"calendarDate": day_datetime},
                                           result_col=['isOpen'])
        ...
        if is_open is None:  # 未查找到相关记录
            ...
            holiday_data["isOpen"] = holiday_data["isOpen"].apply(lambda x: True if x == 1 else False)
            ...
            if holiday_data[holiday_data['calendarDate'] == date_str]['isOpen'].item() == 'Y':
                return False
            else:
                return True
        else:
    ...
    def init_storage(self):
        ...
        if self.storage == "Sqlite数据库":
            if self.engine is None:
                self.engine = create_engine(f'sqlite:///{self.project.db_name}', echo=True, encoding=self.encoding)
