...
class ChalkClient:
    ...
    def delete_features(
        self, namespace: str, features: Optional[List[str]], tags: Optional[List[str]], primary_keys: List[str]
    ) -> FeatureObservationDeletionResponse:
    ...
    def offline_query(
        ...
        branch
            If specified, Chalk will route your request to the relevant branch.
            If specified as None, Chalk will route your request to a non-branch deployment.
            If not specified, Chalk will use the current client's branch info.
        wait
            If True, the query will block until the results are ready.
            If False, the query will return immediately with a `Dataset`.
            The methods of this `Dataset` will block until the results are ready.
        show_progress
