...
from the command line from '{os.getcwd()}'. If you are still having trouble, please contact Chalk support.""",
    ...
    def offline_query(
        ...
        unbranched = self._config.branch is None and branch is None
        ...
        revision._hydrated = unbranched or (isinstance(wait, float) and wait == 0.0)

        if not wait:
            return initialized_dataset

        self._await_operation_completion(revision.revision_id, show_progress=show_progress)
        completed_dataset = self._get_anonymous_dataset(
            revision_id=str(revision.revision_id), environment=environment, branch=branch
        )
        completed_dataset.revisions[-1]._hydrated = True
    ...
    def _create_dataset_request(
        ...
        branched = branch is not None or self._config.branch is not None
        ...
        response = self._request(
            ...
            preview_deployment_id=preview_deployment_id,
            branch=branch,
            api_server_override=server_override,
        )
        return response
