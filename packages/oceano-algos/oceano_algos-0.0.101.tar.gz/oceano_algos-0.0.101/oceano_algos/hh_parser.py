...
__all__ = ['TextPreprocessor', 'preprocess_string', 'preprocess_series', 'HeadHunterScrapper',
           ...
           'get_url_vs_similarity', 'settings_text', 'after_rocket_text', 'welcome_text', 'sql_users',
           'start_search_text', 'stop_search_text']
...
def sql_users(self, chat_id):
    return f"""
            SELECT cvt.chat_id,
            cvt.token,
            settings.cv,
            settings.max_responses,
            settings.format,
            settings.min_salary,
            settings.experience,
            COALESCE(status.status, 0::bigint) AS status
            FROM main_schema.chatid_vs_token cvt
            LEFT JOIN main_schema.chatid_vs_settings settings ON cvt.chat_id = settings.chat_id
            LEFT JOIN main_schema.chatid_vs_status status ON cvt.chat_id = status.chat_id
            WHERE cvt.chat_id={chat_id}
            """
