...
<b>{circle} Соответствие резюме:</b> <i>{similarity} %</i>
    ...
    button   =  InlineKeyboardButton(text="🔍 Перейти на страницу вакансии", url=url)
    keyboard.add(button)
...
def send_recommended_vacancies_to_chatid(self, chat_id):
    ...
    sf = SimilarityFinder(cv, postgres_creds)
    ...
    if min_salary==None:
        ...
        min_salary_filter = (url_vs_similarity['salary_from']>int(min_salary)*1000)
    ...
    sent_vacancies_urls = pc.download(f'SELECT url FROM main_schema.sent_vacancies WHERE chat_id = {chat_id}')['url'].to_list()
    ...
    for _, row in filtered_vacancies.head(10).iterrows():
        ...
        similarity = int(np.sqrt(row['similarity'])*100) # to look more attractive :)
        ...
        try:
            ...
            pc.save(df=pd.DataFrame({"chat_id":[chat_id], 'url':[url]}), table_name='sent_vacancies', schema='main_schema')
        except:
            pass
