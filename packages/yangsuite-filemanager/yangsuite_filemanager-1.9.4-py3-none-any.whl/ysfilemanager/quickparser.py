...
IDENTIFIER_MODULE_SECOND_PART = r"""["']([-a-zA-Z0-9_.]*)["']"""
"""Regular expression matching YANG 'identifier' syntax w/ required quoting."""
...
REVISION_STATEMENT = re.compile(
    r'''^revision[ \t]+['"]?(\d{4}-\d{2}-\d{2})['"]?''' + _SUBSTMT_OPT)
        ...
        while True:
            ...
            if not result['name']:
                if line[0] not in 'ms':
                    ...
                    # Cases for a module/submodel name:
                    # 1. (sub)module test-option {
                    # 2. (sub)module "test-option-with-a-really-long-name"+
                    #      "on-two-lines" {
                    # 3. (sub)module "test-option-with-a-really-long-name" +
                    #      "on-two-lines" {
                    # 4. (sub)module "test-option-with-a-really-long-name"
                    #      "on-two-lines" {
                    #
                    # https://www.rfc-editor.org/rfc/rfc7950#section-7.1
                    #
                    # As per RFC7950, the method of addressing
                    # name splitting is not specified.
                    #
                    # Hence, the approach to consider cases 2, 3, and 4
                    # is undertaken to mitigate the issue.
                    statement = line.strip().split()
                    kind = statement[0]
                    if kind in ['module', 'submodule']:
                        result['kind'] = kind
                        name = statement[1]
                        if len(statement) == 3 and name[0] != '"' \
                                and statement[-1] == '{':
                            result['name'] = name
                        elif name[0] == '"':
                            result['name'] = name.lstrip('"').rstrip('"+')
                            second_part = fp.readline().strip().split()
                            result['name'] += second_part[0].replace('"', '')
