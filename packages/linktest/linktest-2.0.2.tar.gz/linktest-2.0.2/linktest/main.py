...
To use a dataset (currently only supports CSV format), follow these steps:
1. Create a CSV file, where the content of the columns must be separated by commas ",".
2. In the same directory as the CSV file, create a .py file with the same name as the CSV file. This .py file must define a `run_test(self, param...)` method.
...
For example:
    ...
    2. `testLogin.py` file content:
        ...
        def run_test(self, username, password):  # Define the accepted parameters (all parameters defined here must be in the CSV column names)
            tag = "test_login"
    ...
    **Note**:
        The framework will automatically generate a file that conforms to the framework's runtime standards (`*_auto_generated_by_csv.py`) based on `testLogin.csv` & `testLogin.py` (e.g., `testLogin_case_list_auto_generated_by_csv.py`).

    `testLogin_case_list_auto_generated_by_csv.py` content:

        from tests.test_schedule import testLogin
        from linktest.api_testcase import APITestCase


        class testLogin_1(APITestCase):
            tag = "testLogin, test_login"

            def run_test(self):
                username = 'name1'
                password = 'password1'

                testLogin.run_test(self, username, password)


        class testLogin_2(APITestCase):
            tag = "testLogin, test_login"
