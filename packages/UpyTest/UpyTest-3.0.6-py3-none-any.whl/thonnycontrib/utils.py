...
def get_all_tree_childrens(tree: ttk.Treeview, node=None):
    ...
    Args:
        ...
        begin. Defaults to None to starts from the root node.
    """
    def _all_childrens(tree: ttk.Treeview, node: str=None, childrens: list=[]):
...
def format_filename_to_hyperlink(error_message:str):
    ...
    if extracted:
        if os.path.exists(extracted["filename"]):
            ...
            url = "thonny-editor://" + rst_utils.escape(filename).replace(" ", "%20")
...
def get_selected_line(text):
    """
    Get the number of the selected line in the text editor. 
...
def find_node(line, nodes):
    ...
    """
    from .backend.ast_parser import SUPPORTED_TYPES
    if line == None:
        return None
